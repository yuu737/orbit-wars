...
class ProducerLiteConfig:
    ...
    # --- sample65 counter-risk director: do not buy planets with a dead source.
    enable_counter_risk_director_4p: bool = False
    counter_risk_turn_limit: int = 165
    counter_risk_enemy_penalty: float = 1.25
    counter_risk_enemy_hard_margin: float = 9.0
    counter_risk_contested_penalty: float = 0.85
    counter_risk_drain_penalty: float = 0.72
    counter_risk_hold_base: float = 18.0
    counter_risk_hold_prod: float = 4.8
    counter_risk_support_ratio: float = 1.12
...
def _owner_power_vectors(obs, prod: Tensor, *, player_count: int, dtype, device) -> tuple[Tensor, Tensor]:
    ship_power = torch.zeros(int(player_count), dtype=dtype, device=device)
    prod_power = torch.zeros(int(player_count), dtype=dtype, device=device)
    for owner in range(int(player_count)):
        mask = obs.alive & (obs.owner_abs.to(torch.long) == int(owner))
        if bool(mask.any()):
            ship_power[owner] = obs.ships.to(dtype)[mask].sum()
            prod_power[owner] = prod.to(dtype)[mask].sum()
    return ship_power, prod_power


def _apply_counter_risk_director(
    *,
    score: Tensor,
    cand_src: Tensor,
    cand_send: Tensor,
    ...
    if int(player_count) < 4 or not bool(config.enable_counter_risk_director_4p):
        return score
    step = _obs_step(obs_tensors)
    if step > int(config.counter_risk_turn_limit):
        return score
    P = int(obs.P)
    if P <= 0:
        return score
    ...
    active = cand_active.any(dim=-1)
    owners = obs.owner_abs.to(torch.long)
    my_player = int(obs.player_id)

    ship_power, prod_power = _owner_power_vectors(obs, prod, player_count=int(player_count), dtype=dtype, device=device)
    total_power = ship_power + prod_power * 24.0
    my_power = total_power[my_player].clamp(min=1.0)
    enemy_power = total_power.clone()
    enemy_power[my_player] = -1.0
    leader_owner = int(torch.argmax(enemy_power).item()) if int(player_count) > 1 else -1
    living_enemy_power = enemy_power[enemy_power >= 0.0]
    weakest_enemy = living_enemy_power.min() if bool((living_enemy_power >= 0.0).any()) else torch.tensor(0.0, dtype=dtype, device=device)

    target_owner = owners[tgt].clamp(min=-1, max=max(int(player_count) - 1, 0))
    enemy_target = active & obs.is_enemy[tgt]
    neutral_target = active & obs.is_neutral[tgt]
    target_is_leader = enemy_target & (target_owner == int(leader_owner))
    ...
    finish_weak_enemy = enemy_target & (target_power <= weakest_enemy * 1.18 + 80.0) & (target_power <= my_power * 0.72)

    my_support, enemy_pressure, enemy_reach_count = _top_director_neighborhood(
        obs=obs, prod=prod, cache=cache, tgt=tgt, eta=eta,
    )
    target_garrison = obs.ships[tgt].to(dtype)
    source_after = obs.ships[src].to(dtype) - send
    source_hold = (
        torch.full_like(score, float(config.counter_risk_hold_base))
        + prod[src].to(dtype) * float(config.counter_risk_hold_prod)
    )
    source_drain = active & obs.owned[src] & (source_after < source_hold)
    maintainable = (
        my_support
        + send * 0.55
        + prod[tgt].to(dtype) * eta.clamp(max=8.0) * 0.85
        >= enemy_pressure * float(config.counter_risk_support_ratio) + target_garrison * 0.20
    ) & (~source_drain | (source_after >= source_hold - float(config.counter_risk_enemy_hard_margin)))

    unsafe_enemy_attack = enemy_target & (~target_is_leader) & (~finish_weak_enemy) & (~maintainable)
    hard_enemy_block = unsafe_enemy_attack & (
        source_drain
        | (enemy_reach_count >= 2)
        | (enemy_pressure > my_support + send * 0.45 + float(config.counter_risk_enemy_hard_margin))
    )
    contested_neutral = (
        neutral_target
        & (enemy_reach_count >= 2)
        & (my_support + send < enemy_pressure * 1.05 + 5.0)
    )

    penalty = torch.zeros_like(score)
    penalty = penalty + torch.where(
        unsafe_enemy_attack,
        torch.full_like(score, float(config.counter_risk_enemy_penalty))
        + ((enemy_pressure - my_support).clamp(min=0.0) / 70.0).clamp(max=0.85),
        torch.zeros_like(score),
    )
    penalty = penalty + torch.where(
        contested_neutral,
        torch.full_like(score, float(config.counter_risk_contested_penalty)),
        torch.zeros_like(score),
    )
    penalty = penalty + torch.where(
        source_drain & (enemy_target | contested_neutral),
        ((source_hold - source_after).clamp(min=0.0) / 32.0).clamp(max=1.8)
        * float(config.counter_risk_drain_penalty),
        torch.zeros_like(score),
    )
    adjusted = score - penalty
    return torch.where(hard_enemy_block, torch.full_like(score, float("-inf")), adjusted)
...
def plan_lite_waves(
    *,
    ...
    score = _apply_counter_risk_director(
        score=score,
        cand_src=cand_src,
        cand_send=cand_send,
        cand_eta=cand_eta,
...
CONFIG_4P_S7_STABLE = dataclasses.replace(
    ...
    enable_counter_risk_director_4p=True,
)
