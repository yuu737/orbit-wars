...
class ProducerLiteConfig:
    ...
    # --- sample181: 2P sub-floor attrition launches (Phase 2A) ---
    # Phase 1 diagnostic: 74.5% of aimable 2P candidates are dropped by capture-floor
    # gating -> only 2.5% stay valid -> ~0.43 launches/turn (winners launch ~2x more).
    # Allow sub-floor launches at ENEMY targets (favorable attrition trades can score
    # positive even without a one-shot capture), gated to 2P. Neutrals are excluded
    # (sub-floor at a neutral just wastes ships). 4P untouched (player_count>=4 + flag).
    enable_subfloor_2p: bool = False
    subfloor_frac_2p: float = 0.5
...
def _tier_candidates(
    *,
    ...
    if float(subfloor_frac) > 0.0:
        # sample181 Phase 2A: allow sub-floor attrition launches at ENEMY targets only.
        subfloor_ok = (sizes >= floor_at_arr * float(subfloor_frac)) & target_is_enemy.view(1, T)
        clears_floor = clears_floor | subfloor_ok
...
def plan_lite_waves(
    *,
    ...
    target_is_enemy = obs.is_enemy[target_idx.clamp(0, P - 1)]
    # sample181 Phase 2A: sub-floor attrition only for 2P with the flag on (4P -> 0.0).
    subfloor_frac = (
        float(config.subfloor_frac_2p)
        if (int(player_count) < 4 and bool(config.enable_subfloor_2p)) else 0.0
    )
...
# sample181 Phase 2A: enable 2P sub-floor attrition (baked in code; params.json ignored at eval).
_CONFIG_2P_BASE = dataclasses.replace(ProducerLiteConfig(), enable_subfloor_2p=True, subfloor_frac_2p=0.5)
