...
class ProducerLiteConfig:
    ...
    anchor_planner_enabled: bool = False
    anchor_turns: int = 180
    anchor_extra_targets: int = 6
    anchor_near_dist: float = 340.0
...
def _anchor_source_mask(obs, config: ProducerLiteConfig, source_mask: Tensor, player_count: int, step: int) -> Tensor:
    if (
        not bool(config.anchor_planner_enabled)
        or int(player_count) < 4
        or int(step) >= int(config.anchor_turns)
    ):
        return source_mask
    hold_floor = obs.prod * float(config.anchor_hold_prod_mult) + float(config.anchor_hold_base)
    thin_anchor = (
        obs.owned
        & obs.alive
        & (obs.prod >= float(config.anchor_min_prod))
        & (obs.ships < hold_floor)
    )
    guarded = source_mask & ~thin_anchor
    return guarded if bool(guarded.any()) else source_mask


def _add_anchor_targets(
    *,
    obs,
    cache,
    prod: Tensor,
    ...
    anchor_like = (
        ...
        & ((prod.to(dtype) >= float(config.anchor_min_prod)) | (obs.ships.to(dtype) >= float(config.anchor_min_ships)))
    )
    if not bool(anchor_like.any()):
        return target_idx, target_exists

    anchor_idx = torch.nonzero(anchor_like, as_tuple=False).view(-1)
    anchor_to_all = d0[anchor_idx, :] if int(anchor_idx.numel()) > 0 else torch.empty(0, P, dtype=dtype, device=device)
    linked = anchor_to_all <= float(config.anchor_link_dist)
    anchor_value = (prod[anchor_idx].to(dtype) * 5.0 + obs.ships[anchor_idx].to(dtype) * 0.04).view(-1, 1)
    cluster_bonus = torch.where(linked, anchor_value, torch.zeros_like(anchor_value)).amax(dim=0)

    candidate = neutral & (src_dist <= float(config.anchor_near_dist) + float(config.anchor_link_dist))
    pref = (
        prod.to(dtype) * 7.0
        + obs.ships.to(dtype) * 0.05
        + cluster_bonus
        - src_dist.clamp(max=float(config.anchor_near_dist) + float(config.anchor_link_dist)) * 0.012
    )
    pref = torch.where(candidate, pref, torch.full_like(pref, float("-inf")))
    cap = max(1, min(int(config.anchor_extra_targets), P))
    extra_idx, extra_exists = _candidate_indices(pref, candidate, cap)
...
def plan_lite_waves(
    *,
    ...
    source_mask = _anchor_source_mask(obs, config, source_mask, int(player_count), int(step))
    ...
    target_idx, target_exists = _add_anchor_targets(
        obs=obs,
        cache=cache,
        prod=prod,
        config=config,
