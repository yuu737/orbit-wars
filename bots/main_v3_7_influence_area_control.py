...
def local_influence_at(x, y, planets, fleets, player):
    ...
    for planet in planets:
        if planet.owner < 0:
            continue
        dist = max(4.0, distance_xy(x, y, planet.x, planet.y))
        weight = 1.0 / (1.0 + dist / 18.0)
        value = planet.ships * 0.20 + planet.production * 8.5
    ...
    for fleet in fleets:
        dist = max(3.0, distance_xy(x, y, fleet.x, fleet.y))
        weight = 1.0 / (1.0 + dist / 20.0)
        value = fleet.ships * 0.42
        if fleet.owner == player:
            friendly += value * weight
        else:
            enemy += value * weight
    ...
    if target.owner == -1:
        ...
        if target.production >= 4:
            if net > -18.0:
                adjustment += 16.0
            if net > 14.0:
                adjustment += min(18.0, net * 0.18)
            if net < -45.0 and target.ships > 24:
                adjustment -= 18.0
        else:
            if net < -18.0:
                adjustment -= min(26.0, (-net) * 0.20)
            if net > 22.0 and target.ships <= 18:
                adjustment += 10.0

    if source.production >= 4 and current_step >= 110 and target.production >= 3:
        src_friendly, src_enemy = local_influence_at(source.x, source.y, planets, fleets, player)
        if src_friendly + 10.0 > src_enemy:
            adjustment += 7.0
