...
class ProducerLiteConfig:
    ...
    reactive_target_weight: float = 0.12
    reactive_source_weight: float = 0.08
    reactive_response_turns: float = 8.0
    reactive_penalty_cap: float = 240.0
    terminal_reactive_scale: float = 0.35
...
def _apply_phase_config(config: ProducerLiteConfig, step: int) -> ProducerLiteConfig:
    if int(step) >= TOTAL_STEPS - int(config.terminal_phase_turns):
        return dataclasses.replace(
            ...
            reactive_target_weight=float(config.reactive_target_weight) * float(config.terminal_reactive_scale),
            reactive_source_weight=float(config.reactive_source_weight) * float(config.terminal_reactive_scale),
        )
    return config
    ...
    *,
    ...
    if bool(config.reactive_eval) and int(player_count) <= 2 and int(obs.P) > 0:
        ...
        if bool(enemy_mask.any()):
            ...
            penalty = penalty.clamp(max=float(config.reactive_penalty_cap))
