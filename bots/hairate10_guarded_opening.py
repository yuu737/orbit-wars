...
class ProducerLiteConfig:
    ...
    opening_turns: int = 45
    opening_race_weight: float = 0.08
    opening_contest_weight: float = 0.035
    opening_cost_weight: float = 0.04
    opening_bonus_cap: float = 0.45
    opening_late_contest_penalty_weight: float = 0.16
    opening_source_floor: float = 4.0
    opening_thin_source_penalty_weight: float = 0.10
...
def _tier_candidates(
    *,
    ...
    if int(player_count) <= 2 and int(step) < int(config.opening_turns) and int(obs.P) > 0:
        P = int(obs.P)
        tgt_safe = target_idx.clamp(0, P - 1)
        src_safe = source_idx.clamp(0, P - 1)
        neutral_t = obs.is_neutral[tgt_safe] & target_exists
        if bool(neutral_t.any()):
            target_prod = prod[tgt_safe].to(dtype)
            dist_self = cache.cross_dist[0, src_safe, :][:, tgt_safe].to(dtype)  # [S, T]
            self_speed = fleet_speed(sizes.clamp(min=1.0)).clamp(min=1e-6)
            self_eta = dist_self / self_speed
            ...
            if bool(enemy_mask.any()):
                ...
                dist_enemy = cache.cross_dist[0, :, tgt_safe].to(dtype)  # [P, T]
                enemy_eta_all = dist_enemy / enemy_speed.view(P, 1)
                enemy_eta_all = torch.where(
                    enemy_mask.view(P, 1),
                    enemy_eta_all,
                    torch.full_like(enemy_eta_all, float("inf")),
                )
                enemy_eta = enemy_eta_all.min(dim=0).values  # [T]
            else:
                enemy_eta = torch.full((T,), float("inf"), dtype=dtype, device=device)

            defender_cost = floor[:, 0].to(dtype).clamp(min=1.0)
            arrival_advantage = (enemy_eta.view(1, T) - self_eta).clamp(min=-8.0, max=8.0)
            first_bonus = (arrival_advantage.clamp(min=0.0) / 8.0) * target_prod.view(1, T)
            contest_window = (enemy_eta.view(1, T) - self_eta).abs() <= 3.0
            contest_bonus = contest_window.to(dtype) * target_prod.view(1, T)
            cost_penalty = (defender_cost.view(1, T) / (target_prod.view(1, T).clamp(min=1.0) + 1.0))
            late_contest_penalty = (
                (self_eta - enemy_eta.view(1, T) - 1.0).clamp(min=0.0, max=8.0) / 8.0
            ) * target_prod.view(1, T)
            source_after = obs.ships[src_safe].to(dtype).view(S, 1) - sizes
            thin_source_penalty = (
                float(config.opening_source_floor) - source_after
            ).clamp(min=0.0)

            opening_bonus = (
                float(config.opening_race_weight) * first_bonus
                + float(config.opening_contest_weight) * contest_bonus
                - float(config.opening_cost_weight) * cost_penalty
                - float(config.opening_late_contest_penalty_weight) * late_contest_penalty
                - float(config.opening_thin_source_penalty_weight) * thin_source_penalty
            )
            opening_bonus = opening_bonus.clamp(min=-0.25, max=float(config.opening_bonus_cap))
            opening_bonus = torch.where(neutral_t.view(1, T), opening_bonus, torch.zeros_like(opening_bonus))
            score = score + opening_bonus.reshape(C)
