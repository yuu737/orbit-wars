...
def my_economy_ready(my_planets, is_2p):
    production = sum(int(planet.production) for planet in my_planets)
    if is_2p:
        return len(my_planets) >= 3 or production >= 10
    return len(my_planets) >= 3 or production >= 12


def selective_enemy_threat(target, planets, fleets, player, is_2p):
    horizon = 16 if is_2p else 12
    threat = 0.0
    fastest_eta = None
    ...
    for enemy in planets:
        ...
        if eta > horizon:
            continue
        if distance(enemy, target) > (24.0 if is_2p else 18.0):
            continue
        weight = 1.0 - eta / max(1.0, horizon)
        threat += int(enemy.ships) * (0.32 + 0.58 * weight)
        threat += int(enemy.production) * (1.8 if is_2p else 2.4)
    ...
    for fleet in fleets:
        ...
        if eta > horizon:
            continue
        threat += int(fleet.ships) * max(0.25, 1.0 - eta / max(1.0, horizon))
        fastest_eta = eta if fastest_eta is None else min(fastest_eta, eta)

    return threat, fastest_eta


def build_selective_hybrid_defense_candidates(
    sources,
    my_planets,
    planets,
    fleets,
    ...
    if current_step < 32:
        return []
    if not my_economy_ready(my_planets, is_2p):
        return []
    ...
    for target in my_planets:
        if target.production < (4 if is_2p else 5):
            continue
        threat, fastest_eta = selective_enemy_threat(target, planets, fleets, player, is_2p)
        if fastest_eta is None:
            continue

        local_hold = int(target.ships) + int(target.production) * int(max(1.0, fastest_eta))
        danger = threat - local_hold
        margin = int(target.production) * (2 if is_2p else 3) + 5
        if danger < margin:
            continue

        need = min(22 if is_2p else 16, max(config.min_ships_to_launch, int(danger + target.production + 2)))
        for source in sources:
            if source.id == target.id:
                continue
            budget = budgets.get(source.id, 0)
            ...
            if dist > (16.0 if is_2p else 11.0):
                continue
            send = min(budget, need)
            eta = dist / fleet_speed(send)
            if eta > fastest_eta + (2.5 if is_2p else 1.8):
                continue
            angle = math.atan2(target.y - source.y, target.x - source.x)
            score = (
                target.production * (42.0 if is_2p else 34.0)
                + danger * 1.8
                - send * 0.78
                - eta * 2.5
            )
...
def agent(obs):
    ...
    candidates.extend(
        build_selective_hybrid_defense_candidates(
            sources,
            my_planets,
            planets,
            fleets,
