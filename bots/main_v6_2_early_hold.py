...
Experimental V6.2 settings:
- Keep the V6.1 frontier-gated unified planner structure.
- Raise the value of early high-production hold inside the shared pool.
- Spend a little more to survive the first counter window after important captures.
"""
...
def hold_priority_bonus(target, loss_turn, current_step, is_2p):
    bonus = target.production * (12.0 if is_2p else 9.0)
    if target.production >= 5:
        bonus += 42.0 if is_2p else 30.0
    elif target.production >= 4:
        bonus += 20.0 if is_2p else 14.0
    if loss_turn <= (4 if is_2p else 3):
        bonus += 34.0 if is_2p else 24.0
    if current_step <= (140 if is_2p else 100):
        bonus += target.production * (5.0 if is_2p else 3.5)
    return bonus
...
def score_candidate(candidate, target, projection, planets, fleets, player, current_step, is_2p, powers):
    ...
    if candidate.kind == "defense":
        ...
        saved += hold_priority_bonus(target, max(1, int(math.ceil(candidate.eta))), current_step, is_2p) * 0.45
    ...
    if candidate.kind == "attack" and target.production >= 4:
        ...
        if retake_risk_after_capture(target, planets, player, candidate.eta, post_capture, is_2p):
            score -= 28.0 if target.production >= 5 else 16.0
        else:
            score += 18.0 + target.production * 4.0
    if target.production >= 5:
        score += 34.0
    elif target.production >= 4:
        score += 16.0
        ...
        for target in targets:
            ...
            if target.owner == -1 and current_step < 120:
                ...
                if target.production <= 2 and target_pressure > needed + 4:
                    continue
            send = int(needed)
            if target.production >= (4 if is_2p else 5) and current_step < (180 if is_2p else 130):
                hold_margin = 2 + max(0, int(target.production) - 3)
                padded = int(needed + hold_margin)
                if padded <= budget:
                    if retake_risk_after_capture(target, planets, player, eta, padded - int(needed) + 1, is_2p):
                        continue
                    send = padded
...
def build_defense_candidates(sources, my_planets, projection, budgets, config, player, current_step):
    candidates = []
    is_2p = config.defense_horizon >= 20
    for target in my_planets:
        ...
        if loss_turn is None or loss_turn > config.horizon:
            continue
        if target.production < 3 and loss_turn > 6:
            continue
        ...
        if target.production >= 4 and loss_turn <= (6 if is_2p else 4):
            need += 2 + max(0, int(target.production) - 4)
        ...
        for source in sources:
            ...
            saved += hold_priority_bonus(target, loss_turn, current_step, is_2p)
...
def build_urgent_hold_candidates(my_planets, projection, budgets, config, player, current_step, is_2p):
    ...
    for target in my_planets:
        ...
        if target.production >= 4:
            desired += 2 + max(0, int(target.production) - 4)
        for source in my_planets:
            if source.id == target.id:
                continue
            safe_budget = budgets.get(source.id, 0)
            reserve_break_budget = int(source.ships * (0.38 if loss_turn > 3 else 0.54))
            ...
            score += hold_priority_bonus(target, loss_turn, current_step, is_2p)
            if safe_budget < send:
                score -= (send - safe_budget) * 0.42
            if target.production >= 4:
                score += 32.0
...
def agent(obs):
    ...
    planner_candidates.extend(
        build_defense_candidates(sources, my_planets, projection, reserve_budgets, config, player, current_step)
    )
    planner_candidates.extend(
