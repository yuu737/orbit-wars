...
from orbit_lite.movement import PlanetMovement, MovementConfig
...
def agent(obs):
    ...
    t_obs = parse_obs(obs_tensors)
    movement_cfg = MovementConfig(
        movement_horizon=18 if is_2p else 13,
        drift_epsilon=1e-3,
        track_fleets=True,
        player_count=2 if is_2p else 4,
        max_tracked_fleets=128,
    )
    movement = ensure_planet_movement(
        obs_tensors=obs_tensors,
        expected_cfg=movement_cfg,
        cached_movement=_MEMORY.movement,
    )
    _MEMORY.movement = movement
    H = 18 if is_2p else 13
    ...
    if selected:
        ...
        max_L = 1
        for cand in selected:
            if isinstance(cand, MultiCandidate):
                max_L = max(max_L, len(cand.orders))
                
        try:
            import numpy as np
            
            src_arr = np.zeros((C, max_L), dtype=np.int64)
            tgt_arr = np.zeros((C, max_L), dtype=np.int64)
            ships_arr = np.zeros((C, max_L), dtype=np.float32)
            eta_arr = np.ones((C, max_L), dtype=np.float32)
            owner_arr = np.full((C, max_L), player_id, dtype=np.int64)
            valid_arr = np.zeros((C, max_L), dtype=np.bool_)

            for i, cand in enumerate(selected):
                if isinstance(cand, MultiCandidate):
                    for j, order in enumerate(cand.orders):
                        src_arr[i, j] = order.source_id
                        tgt_arr[i, j] = order.target_id
                        ships_arr[i, j] = order.ships
                        eta_arr[i, j] = order.eta if order.eta is not None else 1.0
                        valid_arr[i, j] = True
                else:
                    src_arr[i, 0] = cand.source_id
                    tgt_arr[i, 0] = cand.target_id
                    ships_arr[i, 0] = cand.ships
                    eta_arr[i, 0] = cand.eta if cand.eta is not None else 1.0
                    valid_arr[i, 0] = True
                    
            source_slots = torch.from_numpy(src_arr).to(device)
            target_slots = torch.from_numpy(tgt_arr).to(device)
            req_ships = torch.from_numpy(ships_arr).to(device)
            eta_tensor = torch.from_numpy(eta_arr).to(device)
            owner_tensor = torch.from_numpy(owner_arr).to(device)
            valid_tensor = torch.from_numpy(valid_arr).to(device)
            ...
            diff = sparse_launch_flow_delta(
                ...
                player_count=2 if is_2p else 4, launches=launches, player_id=player_id
            )
            scores = competitive_score(diff, player_id=player_id)
            
            for i, cand in enumerate(selected):
                sim_score = scores[i].item()
                target_owner = -1
                for p in planets:
                    if p.id == cand.target_id:
                        target_owner = p.owner
                        break
                        
                # Accept if simulation score is >= -0.01
                # Or if it's a neutral target (investment pays off beyond horizon)
                # Or if it's our own target (regrouping/defense)
                if sim_score >= -0.01 or target_owner == -1 or target_owner == player_id:
                    filtered_selected.append(cand)
        except Exception as e:
            with open("C:/tmp/v8_4_error.log", "a") as f:
                import traceback
                f.write(f"Exception on step {current_step}:\n{traceback.format_exc()}\n")

    final_moves = []
    for cand in filtered_selected:
        if isinstance(cand, MultiCandidate):
            for order in cand.orders:
                if order.ships > 0:
                    final_moves.append([order.source_id, order.angle, int(order.ships)])
        else:
            if cand.ships > 0:
                final_moves.append([cand.source_id, cand.angle, int(cand.ships)])
