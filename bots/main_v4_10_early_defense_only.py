...
def projected_defense_need(target, projection, loss_turn, is_2p):
    ...
    need = max(4, -projected_ships + int(target.production) * 2 + (4 if is_2p else 6))
    if target.production >= 5:
        need += 4 if is_2p else 6
    elif target.production >= 4:
        need += 2 if is_2p else 4
    if loss_turn <= 5:
        need += 3
    return int(need)
...
def build_defense_candidates(sources, my_planets, projection, budgets, config, player, current_step, is_2p):
    ...
    for target in my_planets:
        ...
        if loss_turn is None or loss_turn > config.horizon:
            continue
        if current_step < 95 and target.production <= 2 and loss_turn > 6:
            continue
        ...
        for source in sources:
            ...
            saved = target.production * 52.0 + urgency * 12.0
            if current_step < 105 and target.production >= 4:
                saved += 42.0
            score = saved - need * (0.82 if is_2p else 0.95) - eta * 2.2
