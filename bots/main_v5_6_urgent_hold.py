...
def agent(obs):
    ...
    budgets = {
        source.id: max(0, budgets.get(source.id, 0) - urgent_spent.get(source.id, 0))
        for source in sources
    }
    ...
    urgent_sources = {cand.source_id for cand in urgent_selected}
    urgent_targets = {cand.target_id for cand in urgent_selected if cand.kind != "regroup"}
    if urgent_selected:
        candidates = [
