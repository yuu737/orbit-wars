...
ADVANTAGE_PROD_WEIGHT = 1.0
ADVANTAGE_SHIP_WEIGHT = 0.015
ADVANTAGE_WIN = 2.4
ADVANTAGE_LOSS = -2.4
ADVANTAGE_SCALE = 4.0
...
def _advantage_state(obs, *, dtype):
    ...
    my_mask = alive & (owner == me)
    my_prod = prod[my_mask].sum()
    my_ships = ships[my_mask].sum()

    enemy_prod = torch.zeros((), dtype=dtype, device=obs.device)
    enemy_ships = torch.zeros((), dtype=dtype, device=obs.device)
    for pid in range(4):
        if pid == me:
            continue
        mask = alive & (owner == pid)
        if bool(mask.any()):
            enemy_prod = torch.maximum(enemy_prod, prod[mask].sum())
            enemy_ships = torch.maximum(enemy_ships, ships[mask].sum())

    raw = (
        (my_prod - enemy_prod) * float(ADVANTAGE_PROD_WEIGHT)
        + (my_ships - enemy_ships) * float(ADVANTAGE_SHIP_WEIGHT)
    )
    adv = (raw / float(ADVANTAGE_SCALE)).clamp(min=-1.0, max=1.0)
    return adv, raw


def _adaptive_knobs(obs, *, dtype):
    adv, raw = _advantage_state(obs, dtype=dtype)
    winning = (raw >= float(ADVANTAGE_WIN)).to(dtype)
    losing = (raw <= float(ADVANTAGE_LOSS)).to(dtype)
    neutral = 1.0 - torch.maximum(winning, losing)

    # Winning: take borders/outside more freely. Losing: preserve the home shell.
    outside_penalty = (
        neutral * float(OUTSIDE_ATTACK_PENALTY)
        + winning * 0.03
        + losing * 0.34
    )
    border_bonus = (
        neutral * float(BORDER_ATTACK_BONUS)
        + winning * 0.30
        + losing * 0.06
    )
    home_defense_bonus = (
        neutral * float(HOME_DEFENSE_BONUS)
        + winning * 0.22
        + losing * 0.78
    )
    source_drain_penalty = (
        neutral * float(SOURCE_DRAIN_PENALTY)
        + winning * 0.08
        + losing * 0.42
    )
    home_attack_bonus = (
        neutral * float(HOME_ATTACK_BONUS)
        + winning * 0.18
        + losing * 0.34
    )
    return {
        ...
        "source_drain_penalty": source_drain_penalty,
...
def _home_shortlist_score(obs, cache, home_sector, border, *, dtype):
    ...
    knobs = _adaptive_knobs(obs, dtype=dtype)
    ...
    attack_score = (
        ...
        + border.to(dtype) * (0.8 + prod * 0.20 + knobs["border_bonus"] * 2.0)
        + obs.is_enemy.to(dtype) * 0.85
        - (~home_sector & ~border).to(dtype) * (0.85 + knobs["outside_penalty"] * 1.7)
    )
    ...
    hold_score = (
        ...
        + deficit * 1.2
        + thin * prod * 24.0
        + home_sector.to(dtype) * prod * (14.0 + knobs["home_defense_bonus"] * 6.0)
...
def _home_build_target_shortlist(obs, obs_tensors, garrison_status, cache, *, config, K_eta, H, prod, source_mask):
    ...
    a_k = max(1, min(int(HOME_EXTRA_ATTACK_TARGETS), P))
...
def _home_adjustment(*, obs, cache, cand_src, cand_send, cand_tgt_slot, cand_eta, cand_is_def, score):
    ...
    attack_adjust = (
        target_home.to(dtype) * (knobs["home_attack_bonus"] + (prod[tgt] * float(HOME_HIGH_PROD_BONUS)).clamp(max=0.55))
        + target_border.to(dtype) * knobs["border_bonus"]
        - target_outside.to(dtype) * knobs["outside_penalty"]
    ) * is_attack.to(dtype) * float(phase)

    defense_adjust = (
        knobs["home_defense_bonus"]
        + target_thin * 1.2
