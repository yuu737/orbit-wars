...
- V4.22: conservative defense need sizing; avoids V4.21 over-defending.
"""
...
def projected_light_defense_need(target, projection, player, loss_turn, is_2p):
    """Conservative replacement for the V4.15 fixed defense size.

    The goal is not to make the bot more defensive.  It only avoids obvious
    waste when the projected loss is caused by a small incoming margin, while
    preserving V4.15's fixed send size in most normal situations.
    """
    fixed_need = max(4, int(target.production * 2 + 4))
    ...
    if before_owner != player or enemy_incoming <= 0:
        return fixed_need

    margin_need = enemy_incoming - friendly_incoming - int(before_ships) - int(target.production) + 1
    margin_need = max(4, int(margin_need + (1 if is_2p else 2)))

    # Keep the strong V4.15 behavior unless the projection says a smaller
    # reinforcement is enough.  Only high-production planets under immediate
    # loss are allowed to require more than the old fixed amount.
    if target.production >= 5 and loss_turn <= 4:
        cap = 26 if is_2p else 20
        return min(cap, max(fixed_need, margin_need))

    return min(fixed_need, margin_need)
...
def build_defense_candidates(sources, my_planets, projection, budgets, config, player, current_step, is_2p):
    ...
    for target in my_planets:
        ...
        if loss_turn is None or loss_turn > config.horizon:
            continue
        need = projected_light_defense_need(target, projection, player, loss_turn, is_2p)
        for source in sources:
            if source.id == target.id:
                continue
            budget = budgets.get(source.id, 0)
