"""
Orbit Wars - V4.19 Midgame Stabilizer SafePlus
...
- V4.19: minimal-risk improvements over V4.15 midgame stabilizer.
"""
...
CONFIG_2P = PlannerConfig(horizon=20, max_targets=14, max_actions=7, roi_threshold=2.0)
...
def choose_config(is_2p, current_step, powers, player):
    """Small dynamic adjustment only; keep V4.15 behavior as the default."""
    base = CONFIG_2P if is_2p else CONFIG_4P
    my_prod = powers.get(int(player), {}).get("production", 0)
    enemy_prod = max(
        (stats.get("production", 0) for owner, stats in powers.items() if int(owner) != int(player)),
        default=0,
    )
    ...
    if is_2p:
        if current_step < 70:
            return PlannerConfig(
                ...
                max_targets=15,
                max_actions=7,
                roi_threshold=1.65 if my_prod < enemy_prod + 4 else 1.9,
        ...
        if 70 <= current_step <= 145 and my_prod + 3 < enemy_prod:
            return PlannerConfig(
                horizon=20,
                max_sources=base.max_sources,
                max_targets=15,
...
def target_shortlist(my_planets, targets, planets, config):
    ...
    for target in targets:
        ...
        value = target.production * 70.0 - int(target.ships) * 1.35 - nearest * 2.0
...
def build_attack_candidates(
    ...
    for source in sources:
        ...
        for target in targets:
            ...
            hold_extra = 0
            if target.production >= 5:
                hold_extra = 2 if target.owner == -1 else 1
            elif target.production >= 4 and target.owner == -1:
                hold_extra = 1
            if current_step > 260 and target.owner == -1:
                hold_extra = 0
            needed = min(budget, needed + hold_extra)
...
def build_defense_candidates(sources, my_planets, projection, budgets, config, player, current_step):
    ...
    for target in my_planets:
        ...
        projected_ships = projection.ships_by_id[target.id][max(0, loss_turn - 1)]
        need = max(4, int(enemy_incoming - friendly_incoming - projected_ships + target.production + 3))
        need = max(need, int(target.production * 2 + 3))
        for source in sources:
            if source.id == target.id:
                continue
            budget = budgets.get(source.id, 0)
...
def agent(obs):
    ...
    # Conservative fallback: only when planner selects nothing, take a nearby high-production neutral.
    if not selected and current_step < (115 if is_2p else 85):
        fallback = []
        for source in sources:
            budget = budgets.get(source.id, 0)
            if budget < config.min_ships_to_launch:
                continue
            for target in targets:
                if target.owner != -1 or target.id in comet_ids or target.production < (4 if is_2p else 5):
                    continue
                target_x, target_y, eta = predict_intercept_position(
                    source, target, initial_planets.get(target.id), current_step, angular_velocity, comet_ids, budget
                )
                if eta > (12.0 if is_2p else 8.0) or crosses_sun(source, target_x, target_y):
                    continue
                needed = capture_floor(target, projection, eta, player, overhead=2)
                if needed > budget or needed < config.min_ships_to_launch:
                    continue
                angle = math.atan2(target_y - source.y, target_x - source.x)
                score = target.production * 55.0 - needed * 1.4 - eta * 4.0
                fallback.append(Candidate("attack", source.id, target.id, angle, int(needed), eta, score))
        selected = greedy_select(fallback, dict(budgets), config)
