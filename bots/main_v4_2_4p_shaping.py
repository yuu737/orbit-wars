"""
Orbit Wars - V4.2 4P-Shaped Planner Branch
...
- Shape 4P enemy attacks to avoid feeding leaders and prefer cheap weak targets.
...
def score_candidate(candidate, target, projection, planets, fleets, player, current_step, is_2p, powers):
    ...
    if target.owner != -1:
        ...
        if not is_2p:
            enemy_entries = [
                entry for owner, entry in powers.items()
            ...
            if enemy_entries:
                ...
                if span >= 28:
                    ...
                    if owner_power >= strongest - 18:
                        if target.production >= 5 and candidate.ships <= 24 and candidate.eta < 10:
                            denial += 10.0
                        else:
                            risk_cost += 18.0 + max(0, candidate.ships - 18) * 0.35
                            if target.production <= 3:
                                risk_cost += 10.0
