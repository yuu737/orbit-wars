...
def _counter_snipe_pass(
    ...
    drain = safe_drain(
        garrison_status, source_idx=cs_src_idx, source_ships=source_ships,
    ...
    valid = (
        ...
        & (sizes >= min_send) & (sizes >= sniper_size.view(1, T_cs)) & src_neq_tgt
...
def plan_lite_waves(
    *,
    ...
    # --- V11.2 Preemptive Defense: Use sniper_size for flipping defensive targets ---
    tgt_clamped = target_idx.clamp(0, P - 1)
    is_defensive = garrison_status.owner[tgt_clamped, 0] == int(pid)
    owner_future = garrison_status.owner[tgt_clamped, 1:H+1]
    is_enemy_future = (owner_future >= 0) & (owner_future != pid)
    enemy_flips = is_defensive.unsqueeze(-1) & is_enemy_future
    any_flip = enemy_flips.any(dim=-1)
    
    flip_turn = enemy_flips.to(torch.long).argmax(dim=-1) + 1
    flip_turn = torch.where(any_flip, flip_turn, torch.full_like(flip_turn, H + 1))
    
    deficit = garrison_status.ships[tgt_clamped].gather(-1, flip_turn.clamp(0, H).unsqueeze(-1)).squeeze(-1)
    min_send = float(config.min_ships_to_launch)
    sniper_size_def = (deficit + 1.0).clamp(min=min_send)
    ...
    # --- V11.1 Retake Risk Filter ---
    tgt_safe_idx = target_idx.clamp(0, P - 1)
    tgt_is_enemy = obs.is_enemy[tgt_safe_idx]
    tgt_is_neutral = obs.is_neutral[tgt_safe_idx]
    tgt_prod = prod[tgt_safe_idx].to(dtype)
    needs_risk_check = (tgt_is_enemy | (tgt_is_neutral & (tgt_prod >= 4.0))) & target_exists # [T]
    ...
    if bool(needs_risk_check.any()):
        ...
        # 距離は time=0 のスナップショットを使用 [P, T]
        dist_to_targets = cache.cross_dist[0, :, tgt_safe_idx] 
        
        # 緩和されたヒューリスティック: 敵が実際に脅威となるには、距離に応じた十分なマージンが必要
        # 敵の艦隊数 > 30 + 生産力 * 距離 (到着するまでの生産量と防御補正)
        threat_threshold = 30.0 + tgt_prod.view(1, T) * dist_to_targets
        
        is_close = dist_to_targets <= float(H) * 1.5 # [P, T]
        eye = torch.eye(P, device=device, dtype=torch.bool)
        is_not_target = ~eye[:, tgt_safe_idx] # [P, T]
        is_threat = is_close & (enemy_ships.view(P, 1) > threat_threshold) & is_not_target
        
        has_threat = is_threat.any(dim=0) # [T]
        is_risky = needs_risk_check & has_threat
    else:
