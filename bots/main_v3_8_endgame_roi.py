"""
Orbit Wars - V3.6 4P Targeting Agent
...
def endgame_roi_adjustment(target, needed, travel_time, remaining, is_2p):
    if remaining > 170:
        return 0.0

    # Approximate how many turns a captured planet can actually pay production
    # before the episode ends, with a small setup buffer after arrival.
    setup_turns = 6.0 if target.owner == -1 else 10.0
    active_turns = remaining - travel_time - setup_turns
    roi = target.production * active_turns - needed
    adjustment = 0.0

    if target.owner == -1:
        if active_turns <= 0:
            adjustment -= 70.0 if is_2p else 90.0
        elif roi < 0:
            adjustment -= min(55.0, -roi * (0.9 if is_2p else 1.15))
        elif remaining < 110 and target.production <= 2:
            adjustment -= 18.0
        elif remaining < 90 and target.production >= 4 and travel_time <= 24:
            adjustment += 10.0
    else:
        score_window = remaining - travel_time
        if score_window <= 0:
            adjustment -= 60.0 if is_2p else 80.0
        elif remaining < 95 and target.production <= 3 and travel_time > 18:
            adjustment -= 28.0

        # Enemy attacks can still be good late if they directly swing score soon.
        pressure_value = target.production * max(0.0, score_window - 6.0) + target.ships * 0.45
        strike_roi = pressure_value - needed * 1.2
        if strike_roi < 0:
            adjustment -= min(42.0, -strike_roi * (0.32 if is_2p else 0.45))
        elif remaining < 80 and target.production >= 4 and travel_time <= 20:
            adjustment += 12.0
...
def target_score(
    ...
    score = value - capture_cost - time_cost
    score += endgame_roi_adjustment(target, needed, travel_time, remaining, is_2p)
    return score
