...
def target_shortlist(my_planets, targets, planets, config):
    ...
    for target in targets:
        ...
        value = target.production * 69.0 - int(target.ships) * 1.35 - nearest * 2.1
        if target.owner != -1:
            value += target.production * 30.0
...
def attack_quality_ok(source, target, needed, eta, current_step, is_2p):
    if not is_2p:
        return True
    if current_step < 90:
        if target.production <= 1 and eta > 7.0:
            return False
        if target.owner != -1 and target.production <= 2 and needed > max(12, int(source.ships * 0.45)):
            return False
    if 90 <= current_step < 230:
        if target.production <= 2 and eta > 10.0:
            return False
        if target.owner != -1 and target.production <= 3 and needed > max(18, int(source.ships * 0.52)):
            return False
    if current_step >= 230 and target.owner == -1 and target.production <= 2:
        return False
    return True
    ...
    for source in sources:
        ...
        for target in targets:
            ...
            if needed > budget or needed < config.min_ships_to_launch:
                continue
            if not attack_quality_ok(source, target, needed, eta, current_step, is_2p):
                continue
