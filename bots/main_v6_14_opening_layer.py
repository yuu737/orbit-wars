...
def opening_neutral_candidates(my_planets, targets, initial_planets, current_step, is_2p):
    if current_step >= opening_turn_limit(is_2p):
        return []
    ...
    for target in targets:
        ...
        is_static = bool(initial is not None and not is_rotating(initial))
        score = target.production * 100.0 - int(target.ships) * 1.6 - travel * 1.8
        if is_static:
            score += 20.0
...
def opening_target_allowed(target, source, opening_rows, is_2p):
    if target.owner != -1:
        return True
    row = next((entry for entry in opening_rows if entry["target"].id == target.id and entry["source"].id == source.id), None)
    if row is None:
        return False

    better_exists = any(entry["target"].production >= 2 for entry in opening_rows)
    if better_exists and int(target.production) < 2:
        return False
    if row["travel"] > (18.0 if is_2p else 13.0):
        return False
    if row["support_dist"] > (16.0 if is_2p else 12.0):
        return False
    if int(target.production) <= 1 and int(target.ships) >= 12:
        return False
    if int(target.production) >= 4:
        return True
    return row["is_static"] or row["support_dist"] <= (12.0 if is_2p else 9.0)


def target_shortlist(my_planets, targets, planets, config, initial_planets, current_step, is_2p):
    ranked = []
    opening_rows = opening_neutral_candidates(my_planets, targets, initial_planets, current_step, is_2p)
    for target in targets:
        source = min(my_planets, key=lambda s: distance(s, target))
        ...
        if current_step < opening_turn_limit(is_2p) and target.owner == -1:
            if opening_target_allowed(target, source, opening_rows, is_2p):
                value += 48.0
            else:
                value -= 120.0
        ranked.append((value, target))
    ranked.sort(key=lambda item: item[0], reverse=True)
    max_targets = config.max_targets + (4 if current_step < opening_turn_limit(is_2p) and is_2p else 0)
    ...
    for source in sources:
        ...
        for target in targets:
            if target.id in comet_ids:
                continue
            if current_step < opening_turn_limit(is_2p) and not opening_target_allowed(target, source, opening_rows, is_2p):
                continue
