...
from sample_lite.geometry import fleet_speed
from sample_lite.intercept_aim import intercept_angle
from sample_lite.movement import MovementConfig, PlanetMovement
from sample_lite.movement_step import (
...
from sample_lite.obs import parse_obs
from sample_lite.distance_cache import build_distance_cache
from sample_lite.planner_core import (
    _candidate_indices,
    _empty_entries,
    _greedy_select,
    _plan_regroup,
...
from sample_lite.adapter import single_obs_to_tensor, sparse_action_row_to_moves
...
class ProducerLiteConfig:
    ...
    reactive_target_weight: float = 0.14
    ...
    *,
    ...
    if bool(config.reactive_eval) and int(player_count) <= 2 and int(obs.P) > 0:
        ...
        if bool(enemy_mask.any()):
            ...
            response_window = eta + float(config.reactive_response_turns)
            tgt_decay = (
                1.0 - eta_to_tgt.unsqueeze(0) / response_window.clamp(min=1.0).unsqueeze(1)
            ).clamp(min=0.0)
            ...
            score = score - float(config.reactive_target_weight) * target_deficit.reshape(C)
