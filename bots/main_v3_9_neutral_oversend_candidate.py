"""
Orbit Wars - V3.13 Frontline Reserve Agent
...
def recommended_neutral_send(
    ...
    if target.production >= 5:
        extra += 2 if is_2p else 1
    elif target.production >= 4:
        extra += 1

    if enemy_race_eta is not None:
        race_gap = enemy_race_eta - travel_time
        if race_gap <= (8.0 if is_2p else 6.0):
            extra += 2 if target.production >= 4 else 1
        elif race_gap <= (14.0 if is_2p else 10.0) and target.production >= 4:
            extra += 1
    ...
    if target.ships <= 8 and target.production >= 4:
        extra += 1

    extra_cap = 5 if is_2p else 3
    extra = min(extra, extra_cap, max(0, int(available) - send))
    return send + extra
...
def opening_target_score(
    ...
    send = recommended_neutral_send(
        target,
        needed,
        max(needed, int(source.ships)),
        travel_time,
        enemy_race_eta,
        current_step,
        mode is MODE_2P,
    ...
    if not capture_holds_against_snipe(
        target, planets, player, travel_time, send, needed, mode is MODE_2P
    ):
        return float("-inf"), None
    ...
    return score, (target_x, target_y, send)
...
def target_score(
    ...
    if target.owner == -1:
        ships_sent = recommended_neutral_send(
            target,
            needed,
            max(needed, int(source.ships)),
            travel_time,
        ...
        capture_cost += max(0, ships_sent - needed) * 1.2
    else:
...
def agent(obs):
    ...
    for mine in ordered_my_planets:
        ...
        for target in targets:
            committed = target_commits.get(target.id, 0)
            _, _, travel_time, needed = effective_capture_plan(
                mine,
                target,
                current_step,
                angular_velocity,
