"""
Orbit Wars - V2.8 Frontline Hold Agent
...
- Add stronger frontline holding after the opening to avoid collapse in real matches.
...
def nearest_enemy_planet_distance(planet, planets, player):
    best = float("inf")
    for other in planets:
        if other.owner >= 0 and other.owner != player:
            d = distance(planet, other)
            if d < best:
                best = d
    return best


def central_ring_bonus(planet):
    # Planets near the sun-side lanes are easy to trade back and forth.
    # Keep more ships there once the opening has ended.
    d = distance_xy(planet.x, planet.y, CENTER_X, CENTER_Y)
    if 18.0 <= d <= 42.0:
        return 6
    if d < 18.0:
        return 4
...
def frontline_bonus(planet, planets, player):
    enemy_d = nearest_enemy_planet_distance(planet, planets, player)
    if enemy_d < 24.0:
        return 10
    if enemy_d < 34.0:
        return 6
...
def reserve_for_planet(planet, current_step, fleets, player, planets):
    ...
    # V2.8: after the opening, captured production planets must be held.
    # Real-match loss showed many thin garrisons being recaptured one by one.
    if current_step >= 95:
        if planet.production >= 5:
            reserve += 14
        elif planet.production >= 4:
            reserve += 10
        elif planet.production >= 3:
            reserve += 6

        reserve += central_ring_bonus(planet)
        reserve += frontline_bonus(planet, planets, player)

    friendly, enemy, soon_enemy = estimate_incoming_to_planet(planet, fleets, player)
    pressure = max(soon_enemy, int(enemy * 0.75))
    if pressure > 0:
        reserve = max(reserve, pressure + planet.production * 3 + 5 - friendly)

    # Avoid draining newly captured planets completely.
    if current_step >= 110 and planet.production >= 3 and planet.ships < reserve + 8:
        reserve = max(reserve, planet.ships - 3)
...
def find_reinforcement_target(source, my_planets, fleets, player, current_step, available):
    # Reinforcement is expensive. Use it only after early expansion, and mostly for real threats.
    if current_step < 115:
        return None, 0
    ...
    for ally in my_planets:
        ...
        # Mostly reinforce planets that are actually being attacked.
        if soon_enemy <= 0 and current_step < 190:
            continue
        desired = max(base_reserve(ally, current_step), ally.production * 4 + 8)
        deficit = desired + max(soon_enemy, int(enemy * 0.45)) - (ally.ships + friendly)
        if deficit <= 3:
            continue

        send = min(available // 2, int(deficit + 3))
        if send < 5:
            continue
    ...
    if best is not None and best_score > 28.0:
...
def agent(obs):
    ...
    for mine in ordered_my_planets:
        ...
        reserve = reserve_for_planet(mine, current_step, fleets, player, planets)
        available = int(mine.ships - already_launched - reserve)
        if available <= 0:
            continue
        if current_step >= 100 and mine.production >= 3 and available < 7:
            continue
