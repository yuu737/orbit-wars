...
class ProducerLiteConfig:
    ...
    opening_turns: int = 60
    opening_race_weight: float = 0.10
    opening_contest_weight: float = 0.06
    opening_cost_weight: float = 0.03
    opening_bonus_cap: float = 0.75
    ...
    *,
    ...
    if int(player_count) <= 2 and int(step) < int(config.opening_turns) and int(obs.P) > 0:
        ...
        if bool(neutral_t.any()):
            ...
            contest_bonus = ((enemy_eta.view(1, T) <= self_eta + 3.0).to(dtype)) * target_prod.view(1, T)
