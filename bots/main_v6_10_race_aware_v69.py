"""
Orbit Wars - V6.10 Race-Aware V6.9 Evolution Candidate
...
V6.10 direction:
- Use V6.9 as the parent because it beat V6.17/V6.19 in current local tests.
- Add only a small early neutral-race adjustment.
- Avoid V6.17/V6.19 broad opening score stacking.
"""
...
def neutral_race_adjustment(source, target, eta, needed, my_planets, planets, fleets, player, current_step, is_2p):
    """Small, isolated neutral-race heuristic for V6.10.

    V6.17/V6.19 changed broad opening target values and regressed in local tests.
    This keeps V6.9's scoring intact and only adjusts early neutral attacks when
    the target is likely to be contested soon.  It should help against strong
    expanders without turning into a hard opening script.
    """
    if not is_2p or target.owner != -1 or current_step >= 120:
        return 0.0
    if target.production <= 1:
        return 0.0

    enemy_etas = []
    enemy_pressure_ships = 0
    for enemy in planets:
        if enemy.owner in (-1, player):
            continue
        enemy_ships = max(1, int(enemy.ships))
        enemy_eta = distance(enemy, target) / fleet_speed(enemy_ships)
        if enemy_eta <= 28.0:
            enemy_etas.append(enemy_eta)
            enemy_pressure_ships += enemy_ships

    if not enemy_etas:
        return 0.0

    best_enemy_eta = min(enemy_etas)
    support_dist = min(
    ...
    # Prefer arriving clearly before the enemy on valuable neutral planets.
    race_gap = best_enemy_eta - eta
    if race_gap >= 2.0:
        adjustment += min(18.0, 5.0 + race_gap * 2.2 + production * 2.0)
    elif race_gap <= -1.0:
        adjustment -= min(26.0, 6.0 + (-race_gap) * 3.0 + production * 1.4)

    # Do not chase unsupported low-value neutrals near the enemy.
    if support_dist > 18.0 and best_enemy_eta < eta + 4.0:
        adjustment -= min(16.0, (support_dist - 18.0) * 1.3 + production)

    # If the enemy has enough nearby mass to retake immediately, require value.
    if enemy_pressure_ships > needed + 10 and production <= 3:
        adjustment -= min(14.0, (enemy_pressure_ships - needed - 10) * 0.18)

    return max(-30.0, min(24.0, adjustment))
...
def build_attack_candidates(
    ...
    for source in sources:
        ...
        for target in targets:
            ...
            cand.score += neutral_race_adjustment(
                source, target, eta, needed, my_planets, planets, fleets, player, current_step, is_2p
            )
