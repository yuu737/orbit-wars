...
def is_behind_for_breakout(powers, player, is_2p, current_step, has_large_idle=False):
    ...
    if current_step >= 220 and enemy_production > 0 and my_production < enemy_production * 0.95:
        return True
    if current_step >= 255 and has_large_idle and enemy_production >= my_production:
        return True
...
def build_desperation_breakout(
    ...
    for source in sources:
        ...
        if int(source.ships) >= 55:
            reserve_break_budget = int(source.ships * (0.54 if current_step < 300 else 0.74))
        ...
        for target in targets:
            ...
            if can_capture:
                ...
                if target.production < 3 or budget < 24:
                    continue
                send = max(22, min(budget, int(budget * (0.78 if current_step >= 275 else 0.58))))
            ...
            score = (
                ...
                - send * (0.34 if can_capture else 0.12)
                - eta * 1.6
            )
            if can_capture:
                score += 42.0
