...
CLUSTER_FULL_TURN = 85
CLUSTER_END_TURN = 145
...
def _cluster_phase_weight(step: int) -> float:
    if int(step) < int(HOME_START_TURN):
        return 0.0
    if int(step) <= int(CLUSTER_FULL_TURN):
        return 1.0
    return max(0.0, 1.0 - (float(step) - float(CLUSTER_FULL_TURN)) / max(float(CLUSTER_END_TURN - CLUSTER_FULL_TURN), 1.0))
...
def _cluster_features(obs, cache, home_sector, border, *, dtype):
    ...
    step = int(obs.step.reshape(-1)[0].item())
    cluster_phase = torch.full((), float(_cluster_phase_weight(step)), dtype=dtype, device=device)
    expansion_value = (
