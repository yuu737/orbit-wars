...
CLUSTER_POTENTIAL_ON = 10.0
CLUSTER_POTENTIAL_FULL = 16.0
...
def _cluster_features(obs, cache, home_sector, border, *, dtype):
    ...
    neutral_home = obs.is_neutral & home_or_border & obs.alive
    ...
    neutral_prod_total = prod[neutral_home].sum() if bool(neutral_home.any()) else torch.zeros((), dtype=dtype, device=device)
    neutral_high_count = (neutral_home & (prod >= 2.0)).sum().to(dtype)
    near_density = (home_neutral_near.sum(dim=0).to(dtype).clamp(max=4.0) * home_attack.to(dtype)).max()
    potential = neutral_prod_total + neutral_high_count * 2.0 + near_density * 1.5
    gate = ((potential - float(CLUSTER_POTENTIAL_ON)) / max(float(CLUSTER_POTENTIAL_FULL - CLUSTER_POTENTIAL_ON), 1e-6)).clamp(min=0.0, max=1.0)
...
def _home_build_target_shortlist(obs, obs_tensors, garrison_status, cache, *, config, K_eta, H, prod, source_mask):
    ...
    cluster_active = bool(cluster["gate"].item() > 0.0)
    target_cap = CLUSTER_EXTRA_TARGETS if cluster_active else HOME_EXTRA_ATTACK_TARGETS
    a_k = max(1, min(int(target_cap), P))
...
def _home_adjustment(*, obs, cache, cand_src, cand_send, cand_tgt_slot, cand_eta, cand_is_def, score):
    ...
    target_cluster_value = cluster["expansion_value"][tgt]
    source_core = cluster["core_factor"][src] > 0.35
    ...
    attack_adjust = (
        ...
        + target_cluster_value.clamp(min=0.0, max=float(CLUSTER_MAX_ADJUST))
        + source_core.to(dtype) * (target_home | target_border).to(dtype) * float(CLUSTER_SOURCE_BONUS)
