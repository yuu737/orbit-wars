...
Experimental V5.3 settings:
- Keep the V5.1 multi-source scaffold.
- Add stronger direct-threat defense so urgent saves compete with expansion.
- Gate risky neutral expansion earlier to reduce defense-collapse and overexpand losses.
"""
...
def attack_exposure_penalty(source, target, my_planets, planets, fleets, player, current_step, is_2p):
    if target.owner != -1:
        return 0.0
    ...
    if current_step < 140:
        if target.production <= 3:
            penalty += max(0.0, support_dist - (14.0 if is_2p else 10.0)) * 3.2
        if source_pressure > int(source.ships) + int(source.production) * 2:
            penalty += 34.0 if target.production <= 4 else 16.0
        if target_pressure > int(target.ships) + int(target.production) * 3:
            penalty += (target_pressure - int(target.ships)) * 0.42
    elif current_step < 220:
        if target.production <= 2:
            penalty += max(0.0, support_dist - 12.0) * 2.4
        if source_pressure > int(source.ships) + int(source.production) * 3:
            penalty += 20.0
    return penalty
    ...
    for source in sources:
        ...
        for target in targets:
            ...
            if needed > budget or needed < config.min_ships_to_launch:
                continue
            if target.owner == -1 and current_step < 160:
                target_pressure = enemy_pressure(target, planets, fleets, player, 16 if is_2p else 10)
                source_pressure = enemy_pressure(source, planets, fleets, player, 16 if is_2p else 10)
                if target.production <= 3 and target_pressure > needed + int(target.production) * 2 + 2:
                    continue
                if source_pressure > int(source.ships) + int(source.production) * 2 and target.production <= 4:
                    continue
            ...
            cand.score -= attack_exposure_penalty(
...
def build_defense_candidates(sources, my_planets, projection, budgets, config, player, current_step):
    ...
    for target in my_planets:
        ...
        if loss_turn is None or loss_turn > config.defense_horizon:
            continue
...
def direct_enemy_threat(target, planets, fleets, player, is_2p):
    ...
    for enemy in planets:
        ...
        if eta > horizon:
            continue
        if distance(enemy, target) > (26.0 if is_2p else 18.0):
            continue
        weight = 1.0 - eta / max(1.0, horizon)
        threat += int(enemy.ships) * (0.34 + 0.60 * weight)
        threat += int(enemy.production) * (2.0 if is_2p else 2.6)
        fastest_eta = eta if fastest_eta is None else min(fastest_eta, eta)

    for fleet in fleets:
        if fleet.owner in (-1, player):
            continue
        eta = fleet_eta_to_planet(fleet, target)
        if eta > horizon:
            continue
        aimed = fleet_points_toward_planet(fleet, target)
        if not aimed and eta > 8:
            continue
        weight = 1.0 - eta / max(1.0, horizon)
        threat += int(fleet.ships) * (0.82 if aimed else 0.30) * max(0.2, weight)
        fastest_eta = eta if fastest_eta is None else min(fastest_eta, eta)

    return threat, fastest_eta


def build_direct_threat_defense_candidates(
    sources,
    my_planets,
    planets,
    fleets,
    ...
    candidates = []
    for target in my_planets:
        threat, fastest_eta = direct_enemy_threat(target, planets, fleets, player, is_2p)
        if fastest_eta is None:
            continue

        local_hold = int(target.ships) + int(target.production) * int(max(1.0, fastest_eta))
        margin = int(target.production) * (3 if is_2p else 4) + (6 if is_2p else 10)
        danger = threat - local_hold
        if danger < margin:
            continue
        if current_step < 80 and target.production <= 2 and danger < margin + 8:
            continue

        need = max(config.min_ships_to_launch, int(danger + int(target.production) + 3))
        need = min(36 if is_2p else 26, need)
        for source in sources:
            if source.id == target.id:
                continue
            budget = budgets.get(source.id, 0)
            if budget < config.min_ships_to_launch:
                continue
            send = min(budget, need)
            eta = distance(source, target) / fleet_speed(send)
            if eta > fastest_eta + (3.5 if is_2p else 2.5):
                continue
            angle = math.atan2(target.y - source.y, target.x - source.x)
            urgency = max(0.0, (22 if is_2p else 15) - fastest_eta)
            score = (
                target.production * (55.0 if is_2p else 44.0)
                + danger * 2.4
                + urgency * 5.0
                - send * 0.85
                - eta * 2.4
            )
            if target.production >= 4 and current_step < 120:
                score += 35.0
            candidates.append(Candidate("defense", source.id, target.id, angle, int(send), eta, score))
    return candidates
...
def agent(obs):
    ...
    candidates.extend(
        build_direct_threat_defense_candidates(
            sources,
            my_planets,
            planets,
            fleets,
