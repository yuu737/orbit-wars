...
def path_has_intercept_conflict(
    source,
    target,
    planets,
    initial_planets,
    ...
    for other in planets:
        if other.id in (source.id, target.id):
            continue
        other_init = initial_planets.get(other.id)
        for frac in (0.20, 0.40, 0.60, 0.80):
            ...
            if distance_xy(fx, fy, ox, oy) > other.radius + 0.45:
                continue
            other_dist = distance_xy(source.x, source.y, ox, oy)
            if other_dist + other.radius < target_dist - target.radius - 0.6:
                return True
    return False
            ...
            if selective_hold_target(target, my_planets, current_step, is_2p):
                ...
                if padded <= budget:
                    ...
                    if not retake_risk_after_capture(target, planets, player, eta, post_capture, is_2p):
                        send = padded
            if path_has_intercept_conflict(
                source,
                target,
                planets,
                initial_planets,
