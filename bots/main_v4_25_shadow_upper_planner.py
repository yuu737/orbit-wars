...
# ---------------------------------------------------------------------------
# V4.25 conservative shadow layer
#
...
def build_shadow_emergency_defense(
    ...
    for target in my_planets:
        if target.id in defended_targets:
            continue
        loss_turn = projection.first_loss_turn_by_id.get(target.id)
        if loss_turn is None or loss_turn > min(config.horizon, 12 if is_2p else 9):
            continue
        ...
        if target.production < (4 if is_2p else 5) and loss_turn > 5:
            continue
...
def build_shadow_opening_expansion(
    ...
    if current_step > (80 if is_2p else 58):
        return []
    used_sources, taken_targets, _ = selected_ids(selected)
    # Do not over-expand if the baseline already made several commitments.
    if len([cand for cand in selected if cand.kind in ("attack", "counter_snipe")]) >= (2 if is_2p else 1):
        return []
    ...
    for source in sources:
        ...
        for target in neutral_targets:
            ...
            if eta_probe > (11.5 if is_2p else 8.0) or crosses_sun(source, tx, ty):
                continue
            ...
            if needed > budget or needed < config.min_ships_to_launch:
                continue
            if needed > max(12, int(budget * (0.55 if is_2p else 0.45))):
                continue
            ...
            if eta > (11.5 if is_2p else 8.0) or crosses_sun(source, tx, ty):
                continue
            ...
            if pressure > needed + (8 if is_2p else 5):
                continue
            angle = math.atan2(ty - source.y, tx - source.x)
            score = target.production * 62.0 - needed * 2.4 - eta * 4.0 - pressure * 0.4
            if target.production >= 5:
                score += 28.0
