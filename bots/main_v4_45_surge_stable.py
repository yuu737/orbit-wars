...
def target_shortlist(my_planets, targets, planets, config):
    ...
    for target in targets:
        ...
        value = target.production * 72.0 - int(target.ships) * 1.25 - nearest * 2.0
        if target.owner != -1:
...
def score_candidate(candidate, target, projection, planets, fleets, player, current_step, is_2p, powers):
    ...
    prod_value = target.production * horizon_payoff * (0.60 if is_2p else 0.38)
    pressure = enemy_pressure(target, planets, fleets, player, 24 if is_2p else 16)
    risk_cost = max(0.0, pressure - candidate.ships) * (0.24 if is_2p else 0.18)
    ship_cost = candidate.ships * (1.02 if candidate.kind == "attack" else 0.72)
    time_cost = candidate.eta * (2.45 if candidate.kind == "attack" else 1.4)
    ...
    if target.owner != -1:
        ...
        denial += target.production * (28.0 if is_2p else 14.0)
        if owner_stats.get("power", 0) > my_stats.get("power", 0):
            denial += min(48.0, (owner_stats.get("power", 0) - my_stats.get("power", 0)) * 0.10)
...
def build_desperation_breakout(
    ...
    if current_step < 150 or current_step > 490:
        return []
    has_large_idle = any(int(source.ships) >= 80 for source in sources)
    ...
    if attack_count >= 1 and current_step < 260:
        return []
    ...
    targets = [
        ...
        and planet.production >= (2 if current_step >= 260 else 3)
    ]
    if not targets:
        return []
    ...
    for source in sources:
        ...
        if int(source.ships) >= 60:
            reserve_break_budget = int(source.ships * (0.52 if current_step < 300 else 0.72))
        ...
        for target in targets:
            ...
            if eta_probe > 54.0 or crosses_sun(source, tx, ty):
                continue
            needed = capture_floor(target, projection, eta_probe, player, overhead=3)
            can_capture = needed <= budget
            if can_capture:
                send = min(budget, needed + max(8, min(34, int(target.production * 6))))
            else:
                ...
                if target.production < 3 or budget < 24:
                    continue
                send = max(22, min(budget, int(budget * (0.78 if current_step >= 280 else 0.58))))

            tx, ty, eta = predict_intercept_position(
                source, target, initial_planets.get(target.id), current_step, angular_velocity, comet_ids, send
            )
            if eta > 54.0 or crosses_sun(source, tx, ty):
                continue
            angle = math.atan2(ty - source.y, tx - source.x)
            score = (
                target.production * (96.0 if can_capture else 62.0)
                + min(70, int(target.ships)) * 0.46
                - send * (0.36 if can_capture else 0.12)
                - eta * 1.4
            )
            if can_capture:
                score += 42.0
