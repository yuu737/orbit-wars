...
Experimental V6.0 settings:
- Keep the V5.6 urgent-hold planner as the base.
- Move relay, urgent hold, defense, counter-snipe, and attack into one shared candidate pool.
- Let one greedy selector choose across the whole pool instead of layering defense patches before attacks.
"""
...
def build_frontline_relay_candidates(my_planets, planets, fleets, budgets, config, player, current_step, is_2p):
    if current_step > (115 if is_2p else 85):
        return []
    ...
    for source in my_planets:
        ...
        for target in my_planets:
            ...
            if dist > (18.0 if is_2p else 13.0):
                continue
            target_pressure = pressures.get(target.id, 0.0)
            gap = target_pressure - source_pressure
            if gap < (18.0 if is_2p else 13.0):
                continue
            send = min(int(budget), max(config.min_ships_to_launch, int(gap * 0.26)))
            eta = dist / fleet_speed(max(1, send))
            if eta > (8.0 if is_2p else 5.5):
                continue
            angle = math.atan2(target.y - source.y, target.x - source.x)
            score = (
                target.production * (28.0 if is_2p else 24.0)
                + gap * 1.7
                - send * 0.55
                - eta * 1.5
            )
