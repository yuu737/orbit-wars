...
def midgame_collapse_reserve_bonus(planet, planets, player, current_step, is_2p):
    if not is_2p or current_step < 65 or current_step > 135 or planet.production < 4:
        return 0
    ...
    for enemy in planets:
        ...
        if dist > 24.0:
            continue
        nearest_enemy = dist if nearest_enemy is None else min(nearest_enemy, dist)
        enemy_local_power += max(0, int(enemy.ships) - int(dist * 0.35))
        enemy_local_power += int(enemy.production) * 2

    if nearest_enemy is None:
        return 0

    local_hold = int(planet.production) + 2
    if nearest_enemy < 16.0:
        local_hold += 2
    if enemy_local_power > int(planet.ships) + int(planet.production) * 2:
        local_hold += 3
    return min(local_hold, 8)
...
def safe_drain(source, projection, planets, player, config, current_step, is_2p):
    ...
    reserve += midgame_collapse_reserve_bonus(source, planets, player, current_step, is_2p)
