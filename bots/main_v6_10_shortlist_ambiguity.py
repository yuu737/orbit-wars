...
def shortlist_ambiguity_penalty(target, source, planets, initial_planets, current_step, angular_velocity, comet_ids):
    ...
    for other in planets:
        if other.id in (source.id, target.id):
            continue
        other_x, other_y = predicted_planet_position(
            other,
            initial_planets.get(other.id),
            current_step,
            angular_velocity,
        ...
        other_dist = distance_xy(source.x, source.y, other_x, other_y)
        if other_dist >= target_dist - 0.75:
            continue
        other_heading = math.atan2(other_y - source.y, other_x - source.x)
        heading_gap = angle_diff(target_heading, other_heading)
        lane_width = math.atan2(other.radius + 0.35, max(1.0, other_dist))
        overlap = lane_width - heading_gap
        if overlap <= 0.0:
            continue
        weight = 1.2 if other.owner not in (-1, source.owner) else (0.85 if other.owner == -1 else 0.65)
        penalty += weight * (overlap * 170.0 + max(0.0, target_dist - other_dist) * 0.28)
    if target.production >= 5:
        penalty *= 0.7
    return penalty


def target_shortlist(my_planets, targets, planets, config, initial_planets, current_step, angular_velocity, comet_ids):
    ranked = []
    for target in targets:
        ...
        if target.production <= 1:
            value -= 45.0
        value -= shortlist_ambiguity_penalty(
            target,
            source,
            planets,
            initial_planets,
