"""
Orbit Wars - V4.20 Midgame Stabilizer Baseline Plus
...
- V4.20: exact V4.15 behavior plus a very conservative no-action expansion fallback.
"""
...
def agent(obs):
    ...
    # V4.20 minimal safe addition:
    # Keep V4.15 decisions unchanged. Only when the planner emits no action,
    # try one nearby high-production neutral capture. This avoids changing the
    # proven midgame stabilizer behavior while reducing idle opening turns.
    if not selected and current_step < (90 if is_2p else 70):
        fallback_candidates = []
        for source in sources:
            budget = budgets.get(source.id, 0)
            if budget < config.min_ships_to_launch:
            ...
            for target in targets:
                ...
                if eta > (10.5 if is_2p else 7.5) or crosses_sun(source, target_x, target_y):
                    continue
                needed = capture_floor(target, projection, eta, player, overhead=1)
                if needed > budget or needed < config.min_ships_to_launch:
                    continue
                angle = math.atan2(target_y - source.y, target_x - source.x)
                score = target.production * 48.0 - needed * 1.7 - eta * 4.5
                if score <= config.roi_threshold:
                    continue
                fallback_candidates.append(
                    Candidate("attack", source.id, target.id, angle, int(needed), eta, score)
                )
        selected = greedy_select(fallback_candidates, dict(budgets), config)
