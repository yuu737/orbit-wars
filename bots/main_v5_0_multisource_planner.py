...
def build_multisource_capture_candidates(
    ...
    if not is_2p or current_step < 45 or current_step > 260:
        return []
    ...
    valuable_targets = [
        ...
        and target.production >= (4 if current_step < 180 else 5)
    ]
    ...
    for target in valuable_targets:
        ...
        for source in sorted(sources, key=lambda item: distance(item, target)):
            ...
            if eta > (18.0 if current_step < 140 else 24.0) or crosses_sun(source, tx, ty):
                continue
        ...
        for eta, source, tx, ty, budget in parts[:3]:
            ...
            if remaining_need <= 0:
                break
            send = min(budget, max(config.min_ships_to_launch, min(remaining_need, int(budget * 0.72))))
        ...
        if total_send > max(18, int(total_budget * 0.82)):
            continue
        score = (
