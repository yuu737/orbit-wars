...
def build_shadow_micro_regroup(
    ...
    if current_step < (50 if is_2p else 38) or current_step > (170 if is_2p else 120):
        return []
    ...
    for target in my_planets:
        ...
        desired = int(target.production * (3.0 if is_2p else 3.5) + target_pressure * 0.08)
        gap = desired - int(target.ships)
        if gap < config.min_ships_to_launch + 2:
            continue
        for source in sources:
            ...
            if budget < config.min_ships_to_launch:
                continue
            source_pressure = pressures.get(source.id, 0.0)
            if source_pressure + (6 if is_2p else 4) >= target_pressure:
                continue
            dist = distance(source, target)
            if dist > (12.0 if is_2p else 8.0):
                continue
            if crosses_sun(source, target.x, target.y):
                continue
            send = min(6 if is_2p else 5, budget, max(config.min_ships_to_launch, gap // 2))
