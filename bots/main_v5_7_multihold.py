...
Experimental V5.6 settings:
- Keep the V5.4 selective-hold planner as the base.
- Add a narrow urgent-hold defense layer for planets projected to fall soon.
- Allow limited reserve breaking only for near-term saves on important planets.
"""
...
def build_multisource_hold_candidates(my_planets, projection, budgets, config, player, current_step, is_2p):
    candidates = []
    for target in my_planets:
        loss_turn = projection.first_loss_turn_by_id.get(target.id)
        if loss_turn is None or loss_turn > (7 if is_2p else 5):
            continue
        ...
        desired = max(config.min_ships_to_launch + 2, shortfall + int(target.production) + 4)
        parts = []
        for source in sorted(my_planets, key=lambda planet: distance(planet, target)):
            if source.id == target.id:
                continue
            safe_budget = budgets.get(source.id, 0)
            reserve_break_budget = int(source.ships * (0.28 if loss_turn > 3 else 0.40))
            budget = max(safe_budget, reserve_break_budget)
            if budget < config.min_ships_to_launch:
                continue
            eta = distance(source, target) / fleet_speed(max(1, budget))
            if eta > loss_turn + (1.0 if is_2p else 0.6):
                continue
            parts.append((eta, source, safe_budget, budget))
        ...
        arrival_eta = 0.0
        planned = []
        for eta, source, safe_budget, budget in parts[:2]:
            remaining = desired - total_send
            if remaining <= 0:
                break
            send = min(int(budget), max(config.min_ships_to_launch, remaining))
            if send < config.min_ships_to_launch:
                continue
            angle = math.atan2(target.y - source.y, target.x - source.x)
            order = Candidate("multi_hold", source.id, target.id, angle, int(send), eta, 0.0)
            planned.append((order, safe_budget))
            total_send += int(send)
            arrival_eta = max(arrival_eta, eta)

        if len(planned) < 2 or total_send < desired:
            continue

        score = (
            target.production * (94.0 if is_2p else 72.0)
            + shortfall * 4.2
            + max(0.0, 8.0 - loss_turn) * 14.0
            - total_send * 1.25
            - arrival_eta * 4.0
        )
        orders = []
        for order, safe_budget in planned:
            if order.ships > safe_budget:
                score -= (order.ships - safe_budget) * 0.45
            orders.append(order)
        candidates.append(DefenseMultiCandidate(target.id, orders, arrival_eta, score))
    return candidates


def append_multisource_hold(selected, multi_candidates, source_budgets):
    ...
    for multi in sorted(multi_candidates, key=lambda item: item.score, reverse=True):
        if multi.target_id in defended_targets or multi.target_id in taken_targets:
            continue
        if any(order.source_id in used_sources for order in multi.orders):
            continue
        if any(order.ships > source_budgets.get(order.source_id, 0) for order in multi.orders):
            continue
        for order in multi.orders:
            selected.append(order)
            source_budgets[order.source_id] = max(0, source_budgets.get(order.source_id, 0) - order.ships)
        break
    return selected
...
def agent(obs):
    ...
    urgent_budgets = {
        source.id: max(
            ...
            int(source.ships * (0.34 if current_step >= 55 else 0.42))
        )
        for source in my_planets
    }
    multi_hold_selected = []
    multi_hold_selected = append_multisource_hold(
        multi_hold_selected,
        build_multisource_hold_candidates(
            my_planets,
            projection,
            urgent_budgets,
            config,
    ...
    multi_hold_spent = {}
    for cand in multi_hold_selected:
        multi_hold_spent[cand.source_id] = multi_hold_spent.get(cand.source_id, 0) + cand.ships
    urgent_selected = greedy_select_limited(
        ...
        dict(urgent_budgets),
        max_actions=2 if is_2p else 1,
        roi_threshold=max(8.0, config.roi_threshold),
    )
    urgent_spent = {}
    for cand in urgent_selected:
        urgent_spent[cand.source_id] = urgent_spent.get(cand.source_id, 0) + cand.ships
    budgets = {
        source.id: max(0, budgets.get(source.id, 0) - multi_hold_spent.get(source.id, 0) - urgent_spent.get(source.id, 0))
        for source in sources
    }
    ...
    committed_sources = {cand.source_id for cand in multi_hold_selected + urgent_selected}
    committed_targets = {cand.target_id for cand in multi_hold_selected + urgent_selected if cand.kind != "regroup"}
    if committed_sources:
        candidates = [
            cand for cand in candidates
            if cand.source_id not in committed_sources
            and (cand.kind == "regroup" or cand.target_id not in committed_targets)
        ]

    selected = list(multi_hold_selected)
    selected.extend(urgent_selected)
    selected.extend(greedy_select(candidates, dict(budgets), config))
