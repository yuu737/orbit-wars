...
Experimental V5.5 settings:
- Keep the V5.4 selective-hold planner as the base.
- Emit optional per-turn JSONL instrumentation when `OW_V5_LOG` is set.
- Record candidate counts, rejection reasons, budgets, shortlist state, and final selected actions.
"""
...
def debug_enabled():
    return bool(os.environ.get(DEBUG_LOG_ENV))


def make_debug_stats(player, current_step, is_2p):
    return {
        "player": int(player),
        "step": int(current_step),
        "mode": "2p" if is_2p else "4p",
        "counts": {},
        "reasons": {},
        "notes": {},
    }
...
def debug_bump(debug_stats, bucket, key, amount=1):
    if debug_stats is None:
        return
    inner = debug_stats.setdefault(bucket, {})
    inner[key] = inner.get(key, 0) + amount


def debug_set(debug_stats, bucket, key, value):
    if debug_stats is None:
        return
    debug_stats.setdefault(bucket, {})[key] = value


def emit_debug(debug_stats):
    if debug_stats is None:
        return
    ...
    with open(path, "a", encoding="utf-8") as handle:
        handle.write(json.dumps(debug_stats, separators=(",", ":"), sort_keys=True) + "\n")
...
def build_attack_candidates(
    ...
    for source in sources:
        ...
        if budget < config.min_ships_to_launch:
            debug_bump(debug_stats, "reasons", "attack_budget_too_small")
            continue
        for target in targets:
            if target.id in comet_ids:
                debug_bump(debug_stats, "reasons", "attack_comet_target")
                continue
            ...
            if eta > config.horizon:
                debug_bump(debug_stats, "reasons", "attack_eta_too_long")
                continue
            if crosses_sun(source, target_x, target_y):
                debug_bump(debug_stats, "reasons", "attack_crosses_sun")
                continue
            needed = capture_floor(target, projection, eta, player)
            if needed > budget:
                debug_bump(debug_stats, "reasons", "attack_needed_gt_budget")
                continue
            if needed < config.min_ships_to_launch:
                debug_bump(debug_stats, "reasons", "attack_needed_below_min_launch")
                continue
            if target.owner == -1 and current_step < 120:
                target_pressure = enemy_pressure(target, planets, fleets, player, 16 if is_2p else 10)
                if target.production <= 2 and target_pressure > needed + 4:
                    debug_bump(debug_stats, "reasons", "attack_neutral_early_pressure_gate")
                    continue
            ...
            exposure_penalty = light_neutral_overexpand_penalty(
                source, target, my_planets, planets, fleets, player, current_step, is_2p
            )
            cand.score -= exposure_penalty
            if exposure_penalty > 0:
                debug_bump(debug_stats, "reasons", "attack_light_overexpand_penalty")
            candidates.append(cand)
            debug_bump(debug_stats, "counts", "attack_candidates")
    return candidates
...
def build_defense_candidates(sources, my_planets, projection, budgets, config, player, current_step, debug_stats=None):
    candidates = []
    for target in my_planets:
        ...
        if loss_turn is None or loss_turn > config.horizon:
            continue
        debug_bump(debug_stats, "counts", "defense_loss_planets")
        if target.production < 4 and loss_turn > 8:
            debug_bump(debug_stats, "reasons", "defense_low_prod_deferred")
            continue
        ...
        for source in sources:
            ...
            budget = budgets.get(source.id, 0)
            if budget < need:
                debug_bump(debug_stats, "reasons", "defense_budget_too_small")
                continue
            eta = distance(source, target) / fleet_speed(need)
            if eta > loss_turn + 1.25:
                debug_bump(debug_stats, "reasons", "defense_eta_too_late")
                continue
            ...
            debug_bump(debug_stats, "counts", "defense_candidates")
    return candidates
...
def build_multisource_capture_candidates(
    ...
    if not is_2p or current_step < 95 or current_step > 285:
        debug_bump(debug_stats, "reasons", "multi_outside_time_window")
        return []

    used_sources, taken_targets, _ = selected_ids(selected)
    if sum(1 for cand in selected if cand.kind in ("attack", "counter_snipe")) >= 2:
        debug_bump(debug_stats, "reasons", "multi_existing_attack_commitments")
        return []
    candidates = []
    valuable_targets = [
    ...
    for target in valuable_targets:
        ...
        for source in sorted(sources, key=lambda item: distance(item, target)):
            if source.id in used_sources:
                debug_bump(debug_stats, "reasons", "multi_used_source_block")
                continue
            budget = budgets.get(source.id, 0)
            if budget < config.min_ships_to_launch:
                debug_bump(debug_stats, "reasons", "multi_budget_too_small")
                continue
            ...
            if eta > 18.0:
                debug_bump(debug_stats, "reasons", "multi_eta_too_long")
                continue
            if crosses_sun(source, tx, ty):
                debug_bump(debug_stats, "reasons", "multi_crosses_sun")
                continue
            parts.append((eta, source, tx, ty, budget))

        if len(parts) < 2:
            debug_bump(debug_stats, "reasons", "multi_not_enough_parts")
            continue
        ...
        for eta, source, tx, ty, budget in parts[:3]:
            ...
            if send < config.min_ships_to_launch:
                debug_bump(debug_stats, "reasons", "multi_send_below_min_launch")
                continue
        ...
        if len(planned) < 2:
            debug_bump(debug_stats, "reasons", "multi_planned_sources_lt2")
            continue
        ...
        if total_send < needed_final + max(3, int(target.production)):
            debug_bump(debug_stats, "reasons", "multi_total_send_insufficient")
            continue
        total_budget = sum(budgets.get(order.source_id, 0) for order in planned)
        if total_send > max(18, int(total_budget * 0.62)):
            debug_bump(debug_stats, "reasons", "multi_total_send_too_greedy")
            continue
        score = (
        ...
        debug_bump(debug_stats, "counts", "multi_candidates")
    return candidates


def append_multisource_capture(selected, multi_candidates, budgets, debug_stats=None):
    used_sources, taken_targets, _ = selected_ids(selected)
    for multi in sorted(multi_candidates, key=lambda item: item.score, reverse=True):
        if multi.score <= 35.0:
            debug_bump(debug_stats, "reasons", "multi_score_floor_stop")
            break
        if multi.target_id in taken_targets:
            debug_bump(debug_stats, "reasons", "multi_target_taken")
            continue
        if any(order.source_id in used_sources for order in multi.orders):
            debug_bump(debug_stats, "reasons", "multi_source_conflict")
            continue
        if any(order.ships > budgets.get(order.source_id, 0) for order in multi.orders):
            debug_bump(debug_stats, "reasons", "multi_budget_conflict")
            continue
        for order in multi.orders:
            selected.append(order)
            budgets[order.source_id] = max(0, budgets.get(order.source_id, 0) - order.ships)
        debug_bump(debug_stats, "counts", "multi_selected")
        break
    return selected
...
def agent(obs):
    ...
    is_2p = num_players == 2
    debug_stats = make_debug_stats(player, current_step, is_2p) if debug_enabled() else None
    ...
    debug_set(debug_stats, "notes", "my_planets", len(my_planets))
    debug_set(debug_stats, "notes", "targets", len(targets))
    debug_set(debug_stats, "notes", "total_budget", sum(budgets.values()))
    debug_set(debug_stats, "notes", "top_source_budgets", sorted(budgets.items(), key=lambda item: item[1], reverse=True)[:5])
    ...
    debug_set(
        ...
        "shortlist",
        [
            {"id": int(target.id), "owner": int(target.owner), "prod": int(target.production), "ships": int(target.ships)}
            for target in shortlisted_targets[:6]
        ],
    )
    candidates = []
    candidates.extend(build_defense_candidates(sources, my_planets, projection, budgets, config, player, current_step, debug_stats))
    candidates.extend(
    ...
    debug_set(debug_stats, "notes", "candidate_counts_pre_select", {
        "defense": sum(1 for cand in candidates if cand.kind == "defense"),
        "counter_snipe": sum(1 for cand in candidates if cand.kind == "counter_snipe"),
        "attack": sum(1 for cand in candidates if cand.kind == "attack"),
    })

    selected = greedy_select(candidates, dict(budgets), config)
    debug_set(debug_stats, "notes", "selected_pre_multi", [
        {"kind": cand.kind, "source": int(cand.source_id), "target": int(cand.target_id), "ships": int(cand.ships), "score": round(float(cand.score), 2)}
    ...
    final_actions = [[cand.source_id, cand.angle, int(cand.ships)] for cand in selected if cand.ships > 0]
    debug_set(debug_stats, "notes", "selected_final", [
        {"kind": cand.kind, "source": int(cand.source_id), "target": int(cand.target_id), "ships": int(cand.ships), "score": round(float(cand.score), 2)}
        for cand in selected if cand.ships > 0
    ])
    debug_set(debug_stats, "notes", "selected_kind_counts", {
        "defense": sum(1 for cand in selected if cand.kind == "defense"),
        "counter_snipe": sum(1 for cand in selected if cand.kind == "counter_snipe"),
        "attack": sum(1 for cand in selected if cand.kind == "attack"),
        "multi_capture": sum(1 for cand in selected if cand.kind == "multi_capture"),
        "regroup": sum(1 for cand in selected if cand.kind == "regroup"),
        "shadow": sum(1 for cand in selected if cand.kind.startswith("shadow")),
        "breakout": sum(1 for cand in selected if cand.kind == "breakout"),
    })
    debug_set(debug_stats, "notes", "final_action_count", len(final_actions))
    emit_debug(debug_stats)
