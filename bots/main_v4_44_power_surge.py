...
def target_shortlist(my_planets, targets, planets, config):
    ...
    for target in targets:
        ...
        value = target.production * 78.0 - int(target.ships) * 1.05 - nearest * 1.8
        if target.owner != -1:
            value += target.production * 42.0
            value += min(60, int(target.ships)) * 0.25
...
def score_candidate(candidate, target, projection, planets, fleets, player, current_step, is_2p, powers):
    ...
    prod_value = target.production * horizon_payoff * (0.68 if is_2p else 0.42)
    pressure = enemy_pressure(target, planets, fleets, player, 24 if is_2p else 16)
    risk_cost = max(0.0, pressure - candidate.ships) * (0.20 if is_2p else 0.16)
    ship_cost = candidate.ships * (0.92 if candidate.kind == "attack" else 0.72)
    time_cost = candidate.eta * (2.2 if candidate.kind == "attack" else 1.4)
    ...
    if target.owner != -1:
        ...
        denial += target.production * (34.0 if is_2p else 16.0)
        if owner_stats.get("power", 0) > my_stats.get("power", 0):
            denial += min(70.0, (owner_stats.get("power", 0) - my_stats.get("power", 0)) * 0.12)
    ...
    for source in sources:
        ...
        for target in targets:
            ...
            if eta > config.horizon or crosses_sun(source, target_x, target_y):
                continue
            overhead = 2 if (is_2p and target.production >= 4) else 1
            needed = capture_floor(target, projection, eta, player, overhead=overhead)
...
def is_behind_for_breakout(powers, player, is_2p, current_step, has_large_idle=False):
    ...
    if enemy_power <= 0:
        return False
    if current_step >= 300 and has_large_idle:
        return True
...
def build_desperation_breakout(
    ...
    if current_step < 120 or current_step > 490:
        return []
    has_large_idle = any(int(source.ships) >= 75 for source in sources)
    ...
    if attack_count >= 2 and current_step < 240:
        return []
    ...
    targets = [
        ...
        and planet.production >= (2 if current_step >= 240 else 3)
    ]
    if not targets:
        return []
    ...
    for source in sources:
        ...
        if int(source.ships) >= 50:
            reserve_break_budget = int(source.ships * (0.58 if current_step < 300 else 0.78))
        ...
        for target in targets:
            ...
            if eta_probe > 60.0 or crosses_sun(source, tx, ty):
                continue
            needed = capture_floor(target, projection, eta_probe, player, overhead=3)
            can_capture = needed <= budget
            if can_capture:
                send = min(budget, needed + max(8, min(40, int(target.production * 7))))
            else:
                # When we are far behind, a large idle stack should still apply
                # pressure instead of waiting forever for a perfect capture.
                if target.production < 2 or budget < 22:
                    continue
                send = max(22, min(budget, int(budget * (0.82 if current_step >= 260 else 0.62))))

            tx, ty, eta = predict_intercept_position(
                source, target, initial_planets.get(target.id), current_step, angular_velocity, comet_ids, send
            )
            if eta > 60.0 or crosses_sun(source, tx, ty):
                continue
            angle = math.atan2(ty - source.y, tx - source.x)
            score = (
                target.production * (104.0 if can_capture else 66.0)
                + min(80, int(target.ships)) * 0.48
                - send * (0.32 if can_capture else 0.10)
                - eta * 1.25
            )
            if can_capture:
                score += 42.0
