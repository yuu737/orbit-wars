...
class ProducerLiteConfig:
    ...
    safe_terminal_enabled: bool = True
    safe_terminal_phase_turns: int = 60
    safe_terminal_close_ratio: float = 0.75
    safe_terminal_lead_ratio: float = 0.95
    safe_terminal_close_roi_drop: float = 0.10
    safe_terminal_behind_roi_drop: float = 0.20
    safe_terminal_close_roi_floor: float = 1.35
    safe_terminal_behind_roi_floor: float = 1.30
    safe_terminal_neutral_eta_margin: float = 8.0
    safe_terminal_enemy_eta_margin: float = 4.0
    safe_terminal_neutral_decay_scale: float = 80.0
...
def _apply_phase_config(config: ProducerLiteConfig, step: int) -> ProducerLiteConfig:
    if bool(config.safe_terminal_enabled):
        return config
...
def owner_strength(obs, prod: Tensor, *, player_count: int, dtype, device) -> Tensor:
...
def apply_safe_terminal_config(
    ...
    *,
    ...
    if not bool(config.safe_terminal_enabled):
        return _apply_phase_config(config, step)
    remaining = TOTAL_STEPS - int(step)
    if remaining > int(config.safe_terminal_phase_turns):
        return config

    dtype = obs.ships.dtype
    device = obs.device
    pid = int(obs.player_id)
    strength = owner_strength(obs, prod, player_count=int(player_count), dtype=dtype, device=device)
    if pid < 0 or pid >= int(player_count) or strength.numel() == 0:
        return config
    my_strength = strength[pid]
    leader_strength = strength.max().clamp(min=1e-6)

    if bool(my_strength >= float(config.safe_terminal_lead_ratio) * leader_strength):
        return dataclasses.replace(config, enable_regroup=True)

    if bool(my_strength >= float(config.safe_terminal_close_ratio) * leader_strength):
        new_roi = max(
            float(config.safe_terminal_close_roi_floor),
            float(config.roi_threshold) - float(config.safe_terminal_close_roi_drop),
        )
    else:
        new_roi = max(
            float(config.safe_terminal_behind_roi_floor),
            float(config.roi_threshold) - float(config.safe_terminal_behind_roi_drop),
        )
...
def apply_safe_terminal_candidate_adjustments(
    *,
    score: Tensor,
    obs,
    prod: Tensor,
    ...
    too_late_neutral = is_neutral & (
        eta > max(1.0, float(remaining) - float(config.safe_terminal_neutral_eta_margin))
    )
    too_late_enemy = is_enemy & (
        eta > max(1.0, float(remaining) - float(config.safe_terminal_enemy_eta_margin))
    )
    invalid_late = too_late_neutral | too_late_enemy

    neutral_factor = (
        (float(remaining) - eta) / max(1.0, float(config.safe_terminal_neutral_decay_scale))
    ).clamp(min=0.20, max=1.0)
...
def _tier_candidates(
    *,
    ...
    score = apply_safe_terminal_candidate_adjustments(
        score=score,
        obs=obs,
        prod=prod,
...
def run_turn(obs_tensors: dict, *, config: ProducerLiteConfig, player_count: int, memory) -> dict:
    ...
    step = int(obs_tensors.get("step", torch.zeros(1, device=device)).reshape(-1)[0].item())
    config = apply_safe_terminal_config(
        config,
        obs=obs,
