...
# ---------------------------------------------------------------------------
# V4.27 counter-snipe guarded shadow layer
#
...
def build_shadow_counter_snipe(
    ...
    """High-confidence leftover counter-snipe.

    The baseline already has counter-snipe in the main candidate pool.  This
    shadow version only fires after baseline + regroup, with unused sources and
    leftover safe budget.  It targets the narrow case where the opponent is
    visibly taking a valuable neutral and the baseline did not spend a source on
    it.  This is safer than reopening ordinary attacks because the enemy fleet
    direction gives a concrete short-horizon tactical fact.
    """
    if current_step < (32 if is_2p else 48) or current_step > (210 if is_2p else 160):
        return []

    used_sources, taken_targets, _ = selected_ids(selected)
    candidates = []
    min_prod = 4 if is_2p else 5
    max_delay = 10.0 if is_2p else 7.0
    enemy_eta_cap = 20.0 if is_2p else 14.0
    max_cost = 18 if is_2p else 14
    ...
    for source in sources:
        if source.id in used_sources:
            continue
        budget = remaining_budgets.get(source.id, 0)
        if budget < config.min_ships_to_launch + 2:
            continue
        for target in neutral_targets:
            for fleet in fleets:
                if fleet.owner in (-1, player):
                ...
                # Keep this as a small tactical parasitic move, not a second main attack.
                if needed < config.min_ships_to_launch or needed > budget or needed > max_cost:
                    continue
                if needed > int(budget * (0.58 if is_2p else 0.50)):
                    continue
                if budget - needed < 2:
                    continue
                ...
                if not capture_holds_after_counter_snipe(target, planets, player, travel, needed, needed, is_2p):
                    continue
                # Extra local risk filter against large enemy planets just behind the target.
                ...
                if pressure > needed + (5 if is_2p else 3):
                    continue
                angle = math.atan2(ty - source.y, tx - source.x)
                score = (
                    target.production * (62.0 if is_2p else 45.0)
                    + max(0.0, max_delay - delay) * 5.0
                    - needed * 2.8
                    - travel * 2.2
                    - pressure * 0.55
                )
...
def build_shadow_finish_attack(
    ...
    # Disabled in V4.27: V4.25 gained placement wins but lost average score,
