...
def opening_max_travel(is_2p):
    return 36.0 if is_2p else 23.0


def opening_neutral_shape_score(source, target, my_planets, initial_planets, targets, current_step, is_2p):
    ...
    if support_dist <= (14.5 if is_2p else 10.5):
        score += 10.0
    elif support_dist > (22.0 if is_2p else 15.0):
        score -= 10.0
    return score


def early_enemy_pressure_bonus(target, current_step, is_2p, powers, player):
    if not is_2p or current_step >= 55 or target.owner in (-1, player):
        return 0.0
...
def target_shortlist(my_planets, targets, planets, config, initial_planets, current_step, is_2p, powers, player):
    ...
    for target in targets:
        ...
        if target.production <= 1:
            value -= 45.0
        value += opening_neutral_shape_score(
            source, target, my_planets, initial_planets, targets, current_step, is_2p
        )
        value += early_enemy_pressure_bonus(target, current_step, is_2p, powers, player)
        ranked.append((value, target))
    ranked.sort(key=lambda item: item[0], reverse=True)
    max_targets = config.max_targets + (2 if current_step < opening_turn_limit(is_2p) and is_2p else 0)
    ...
    for source in sources:
        ...
        for target in targets:
            if target.id in comet_ids:
                continue
            if current_step < opening_turn_limit(is_2p) and target.owner == -1:
                if distance(source, target) > opening_max_travel(is_2p):
                    continue
            ...
            cand.score += opening_neutral_shape_score(
                source, target, my_planets, initial_planets, targets, current_step, is_2p
            )
            cand.score += early_enemy_pressure_bonus(target, current_step, is_2p, powers, player)
