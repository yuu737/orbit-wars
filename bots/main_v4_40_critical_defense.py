...
def build_critical_defense_candidates(sources, my_planets, planets, fleets, budgets, config, player, current_step, is_2p):
    if not is_2p or current_step < 78 or current_step > 118:
        return []

    pressures = {
        planet.id: enemy_pressure(planet, planets, fleets, player, 16)
    ...
    for target in my_planets:
        ...
        hold_target = int(target.production * 4 + target_pressure * 0.12)
        ...
        for source in sources:
            ...
            if source_pressure + 12.0 >= target_pressure:
                continue
            budget = budgets.get(source.id, 0)
            if budget < config.min_ships_to_launch:
                continue
            dist = distance(source, target)
            if dist > 10.5 or crosses_sun(source, target.x, target.y):
                continue
            send = min(budget, 6, max(config.min_ships_to_launch, deficit // 3))
            eta = dist / fleet_speed(send)
            if eta > 4.8:
                continue
            angle = math.atan2(target.y - source.y, target.x - source.x)
            score = (
                target.production * 13.0
                + min(deficit, 18) * 0.9
                + (target_pressure - source_pressure) * 0.18
                - send * 1.6
                - eta * 3.0
            )
...
def agent(obs):
    ...
    candidates.extend(
        build_critical_defense_candidates(
            sources,
            my_planets,
            planets,
            fleets,
