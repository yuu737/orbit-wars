...
# ---------------------------------------------------------------------------
# V4.28 counter-snipe guarded shadow layer
#
...
def build_shadow_opening_expansion(
    ...
    # Do not over-expand if the baseline already made several commitments.
    if len([cand for cand in selected if cand.kind in ("attack", "counter_snipe")]) >= 1:
        return []
    ...
    for source in sources:
        if source.id in used_sources:
            # Preserve the baseline's source-level plan; only unused sources can
            # add a shadow expansion.
            continue
        budget = remaining_budgets.get(source.id, 0)
        if budget < config.min_ships_to_launch:
            continue
        for target in neutral_targets:
            ...
            if eta_probe > (7.5 if is_2p else 6.0) or crosses_sun(source, tx, ty):
                continue
            ...
            if needed > budget or needed < config.min_ships_to_launch:
                continue
            if needed > min(10, int(budget * (0.42 if is_2p else 0.35))):
                continue
            ...
            if eta > (7.5 if is_2p else 6.0) or crosses_sun(source, tx, ty):
                continue
            ...
            score = target.production * 50.0 - needed * 3.4 - eta * 5.5 - pressure * 0.8
            if target.production >= 5:
                score += 18.0
...
def build_shadow_counter_snipe(
    sources,
    planets,
    fleets,
    remaining_budgets,
    ...
    """Very narrow leftover counter-snipe.

    V4.27 improved placement but hurt average score because shadow counter-snipe
    sometimes displaced safer shadow expansion/defense or spent too much reserve
    on volatile neutral fights.  V4.28 keeps this only as a last-priority extra:
    high-production neutral, short delay, small cost, low local pressure, and
    enough leftover ships after launch.
    """
    if current_step < (38 if is_2p else 52) or current_step > (145 if is_2p else 120):
        return []

    used_sources, taken_targets, _ = selected_ids(selected)
    candidates = []
    min_prod = 5 if is_2p else 5
    max_delay = 7.0 if is_2p else 5.5
    enemy_eta_cap = 15.0 if is_2p else 11.0
    max_cost = 12 if is_2p else 10

    neutral_targets = [
        p for p in planets
        if p.owner == -1 and p.id not in comet_ids and p.id not in taken_targets and p.production >= min_prod
    ]
    ...
    for source in sources:
        if source.id in used_sources:
            continue
        budget = remaining_budgets.get(source.id, 0)
        if budget < config.min_ships_to_launch + 5:
            continue
        for target in neutral_targets:
            for fleet in fleets:
                if fleet.owner in (-1, player):
                ...
                probe = max(config.min_ships_to_launch, enemy_surplus + 1)
                tx, ty, travel = predict_intercept_position(
                    source, target, initial_planets.get(target.id), current_step, angular_velocity, comet_ids, probe
                )
                delay = travel - enemy_eta
                if delay < 1.0 or delay > max_delay or crosses_sun(source, tx, ty):
                    continue

                needed = enemy_surplus + int(target.production) * int(delay) + (2 if is_2p else 3)
                if needed < config.min_ships_to_launch or needed > budget or needed > max_cost:
                    continue
                if needed > int(budget * (0.42 if is_2p else 0.36)):
                    continue
                # Do not drain a source just to win a neutral skirmish.
                if budget - needed < max(5 if is_2p else 6, int(source.production) + 3):
                    continue
                ...
                if delay < 1.0 or delay > max_delay or crosses_sun(source, tx, ty):
                    continue
                if not capture_holds_after_counter_snipe(target, planets, player, travel, needed, needed, is_2p):
                    continue

                post_capture = max(1, needed - enemy_surplus - int(target.production) * int(delay))
                if retake_risk_after_capture(target, planets, player, travel, post_capture, is_2p):
                    continue

                pressure = enemy_pressure(target, planets, fleets, player, 14 if is_2p else 9)
                if pressure > needed + (1 if is_2p else 0):
                    continue
                angle = math.atan2(ty - source.y, tx - source.x)
                score = (
                    target.production * (54.0 if is_2p else 40.0)
                    + max(0.0, max_delay - delay) * 4.0
                    - needed * 4.2
                    - travel * 3.2
                    - pressure * 1.1
                )
                if target.production >= 6:
                    score += 14.0
                candidates.append(Candidate("shadow_counter_snipe", source.id, target.id, angle, int(needed), travel, score))
    return candidates
...
def build_shadow_finish_attack(
    ...
    # Disabled in V4.28: V4.25 gained placement wins but lost average score,
...
def greedy_append_shadow(selected, candidates, remaining_budgets, max_extra):
    if max_extra <= 0:
        return selected

    # Priority order is intentional:
    # 1. emergency defense prevents direct losses,
    # 2. opening expansion was the stable V4.26 improvement,
    # 3. counter-snipe is opportunistic and only used when the safer classes
    #    have no acceptable candidate.
    priority = {
        "shadow_defense": 0,
        "shadow_expand": 1,
        "shadow_counter_snipe": 2,
        "shadow_finish": 3,
    }
    thresholds = {
        "shadow_defense": 95.0,
        "shadow_expand": 95.0,
        "shadow_counter_snipe": 145.0,
        "shadow_finish": 9999.0,
    }

    used_sources, taken_targets, defended_targets = selected_ids(selected)
    extra = 0
    ordered = sorted(candidates, key=lambda item: (priority.get(item.kind, 9), -item.score))
    for cand in ordered:
        if extra >= max_extra:
            break
        if cand.score <= thresholds.get(cand.kind, 95.0):
            continue
...
def agent(obs):
    ...
    selected = greedy_append_shadow(selected, shadow_candidates, shadow_budgets, max_extra=1)
