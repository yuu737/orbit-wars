...
Experimental V7.0 settings:
- Keep the V6.3 unified planner and selective hold-aware attack as the base.
- Replace the simple target shortlist with support-aware shape control.
- Prefer captures that extend a connected high-production outer lane and avoid unsupported early overextension.
"""
...
def nearest_support_distance(target, my_planets, exclude_id=None):
    return min(
        (distance(friend, target) for friend in my_planets if friend.id != exclude_id),
        default=99.0,
    )


def shape_control_value(target, my_planets, planets, player, current_step, is_2p):
    nearest = min(distance(source, target) for source in my_planets)
    support_dist = nearest_support_distance(target, my_planets)
    support = friendly_support_count(target, my_planets, 17.0 if is_2p else 12.5)
    enemy_dist = min(
        (distance(enemy, target) for enemy in planets if enemy.owner not in (-1, player)),
        default=99.0,
    )
    ring = orbital_ring_value(target)

    value = 0.0
    value += target.production * (28.0 if is_2p else 20.0)
    value += max(0.0, ring - 24.0) * (1.2 if is_2p else 0.8)
    value += support * (18.0 if is_2p else 12.0)
    value -= nearest * 1.6
    value -= max(0.0, support_dist - (18.0 if is_2p else 13.0)) * 4.2

    if target.production >= 5 and support_dist <= (18.0 if is_2p else 13.0):
        value += 38.0 if is_2p else 24.0
    if target.owner == -1 and target.production <= 2 and current_step < (130 if is_2p else 90):
        value -= 42.0
    if enemy_dist + 3.0 < nearest and support == 0:
        value -= 55.0
    if target.owner != -1:
        value += target.production * (18.0 if is_2p else 11.0)
    return value
...
def target_shortlist(my_planets, targets, planets, config, player, current_step, is_2p):
    ranked = []
    for target in targets:
        nearest = min(distance(source, target) for source in my_planets)
        value = target.production * 48.0 - int(target.ships) * 1.25 - nearest * 1.6
        value += shape_control_value(target, my_planets, planets, player, current_step, is_2p)
        if target.owner != -1:
            value += target.production * 20.0
...
def unsupported_capture_risk(source, target, my_planets, planets, fleets, player, current_step, is_2p):
    if target.owner != -1:
        return 0.0
    if current_step > (155 if is_2p else 105):
        return 0.0
    support_dist = nearest_support_distance(target, my_planets, exclude_id=source.id)
    support = friendly_support_count(target, my_planets, 17.0 if is_2p else 12.5)
    pressure = enemy_pressure(target, planets, fleets, player, 18 if is_2p else 12)
    nearest_enemy = min(
        (distance(enemy, target) for enemy in planets if enemy.owner not in (-1, player)),
        default=99.0,
    )
    nearest_friend = min(distance(friend, target) for friend in my_planets)

    risk = 0.0
    if support == 0:
        risk += max(0.0, support_dist - (14.0 if is_2p else 10.0)) * 4.5
        risk += 28.0
    if nearest_enemy + 2.0 < nearest_friend:
        risk += 35.0
    if target.production <= 3:
        risk += max(0.0, pressure - int(target.ships) - 4) * 0.55
    elif target.production >= 5 and support_dist <= (18.0 if is_2p else 13.0):
        risk -= 22.0
    return risk
        ...
        for target in targets:
            ...
            if target.owner == -1 and current_step < 120:
                ...
                if target.production <= 2 and target_pressure > needed + 4:
                    continue
                if target.production <= 3 and unsupported_capture_risk(
            ...
            cand.score += shape_control_value(target, my_planets, planets, player, current_step, is_2p) * 0.22
            cand.score -= unsupported_capture_risk(source, target, my_planets, planets, fleets, player, current_step, is_2p)
...
def build_urgent_hold_candidates(my_planets, projection, budgets, config, player, current_step, is_2p):
    candidates = []
    for target in my_planets:
        loss_turn = projection.first_loss_turn_by_id.get(target.id)
        ...
        for source in my_planets:
            ...
            eta = distance(source, target) / fleet_speed(max(1, send))
...
def build_frontline_relay_candidates(my_planets, planets, fleets, budgets, config, player, current_step, is_2p):
    ...
    for source in my_planets:
        ...
        for target in my_planets:
            ...
            if send < max(5, config.min_ships_to_launch):
                continue
            eta = dist / fleet_speed(max(1, send))
...
def agent(obs):
    ...
    shortlisted_targets = target_shortlist(my_planets, targets, planets, config, player, current_step, is_2p)
    reserve_budgets = {
        source.id: max(
    ...
    planner_candidates.extend(
        build_defense_candidates(sources, my_planets, projection, budgets, config, player, current_step)
    )
    planner_candidates.extend(
