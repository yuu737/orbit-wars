...
class ProducerLiteConfig:
    ...
    opening_bonus_cap: float = 0.35
    opening_roi_margin: float = 0.3
    opening_min_arrival_advantage: float = 1.5
    opening_min_source_after: float = 5.0
    source_collapse_guard: bool = True
    source_collapse_turns: float = 8.0
    source_collapse_floor: float = 5.0
    source_collapse_pressure_fraction: float = 0.25
    ...
    *,
    ...
    if bool(config.reactive_eval) and int(player_count) <= 2 and int(obs.P) > 0:
        ...
        if bool(enemy_mask.any()):
            ...
            enemy_reachable_pressure = torch.where(
                eta_to_src <= float(config.source_collapse_turns),
            ...
            if int(step) < int(config.opening_turns):
                ...
                ).clamp(min=0.0, max=float(config.opening_bonus_cap))
                opening_allowed = (
                    neutral_t.view(1, T)
                    & (base_score.reshape(S, T) >= float(config.roi_threshold) - float(config.opening_roi_margin))
                    & (arrival_advantage >= float(config.opening_min_arrival_advantage))
                    & (source_after >= float(config.opening_min_source_after))
                )
                score = score + torch.where(opening_allowed, opening_bonus, torch.zeros_like(opening_bonus)).reshape(C)

            if bool(config.source_collapse_guard):
                source_floor = torch.maximum(
                    torch.full_like(enemy_reachable_pressure, float(config.source_collapse_floor)),
                    enemy_reachable_pressure * float(config.source_collapse_pressure_fraction),
                )
                collapse_risk = (
                    target_is_attack
                    & (enemy_eta_to_src.view(S, 1) <= float(config.source_collapse_turns))
                    & (source_after < source_floor.view(S, 1))
                )
                score = torch.where(collapse_risk.reshape(C), torch.full_like(score, float("-inf")), score)
