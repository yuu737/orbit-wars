    ...
    for source in sources:
        ...
        for target in targets:
            if target.id in comet_ids:
                continue
            probe_x, probe_y, eta = predict_intercept_position(
                source, target, initial_planets.get(target.id), current_step, angular_velocity, comet_ids, budget
            )
            if eta > config.horizon or crosses_sun(source, probe_x, probe_y):
                continue
            ...
            if not validate_intercept_window(
                source, target, initial_planets.get(target.id), current_step, angular_velocity, comet_ids, send, target_x, target_y, eta
            ):
                continue
