...
def opening_max_travel(is_2p):
    return 35.0 if is_2p else 22.0


def opening_neutral_bonus(source, target, my_planets, initial_planets, current_step, is_2p):
    ...
    if target.production >= 4:
        bonus += 28.0 + target.production * 8.0
    ...
    if support_dist <= (14.0 if is_2p else 10.0):
        bonus += 10.0
    elif support_dist > (20.0 if is_2p else 14.0):
        bonus -= 12.0
    return bonus


def opening_low_prod_penalty(target, targets, current_step, is_2p):
    if current_step >= opening_turn_limit(is_2p) or target.owner != -1:
        return 0.0
    if int(target.production) >= 2:
        return 0.0
    if any(other.owner == -1 and int(other.production) >= 2 for other in targets):
        return 42.0
    return 0.0
...
def target_shortlist(my_planets, targets, planets, config, initial_planets, current_step, is_2p):
    ...
    for target in targets:
        ...
        if target.production <= 1:
            value -= 45.0
        value += opening_neutral_bonus(source, target, my_planets, initial_planets, current_step, is_2p)
        value -= opening_low_prod_penalty(target, targets, current_step, is_2p)
        if current_step < opening_turn_limit(is_2p) and target.owner != -1 and int(target.production) < 4:
            value -= 28.0
...
def build_attack_candidates(
    ...
    for source in sources:
        ...
        for target in targets:
            ...
            cand.score += opening_neutral_bonus(
                source, target, my_planets, initial_planets, current_step, is_2p
            )
            cand.score -= opening_low_prod_penalty(target, targets, current_step, is_2p)
            if current_step < opening_turn_limit(is_2p) and target.owner != -1 and int(target.production) < 4:
                cand.score -= 24.0
