...
V7.1 attack-confidence branch:
- Keep V7.0/V6.9 architecture as the baseline.
- Add only the V6.12 attack_confidence_bonus signal to attack candidate scoring.
- Do not add opening bias, early enemy pressure, or forward evaluation yet.
"""
...
class PlannerContext:
    ...
    reserve_budgets: dict[int, int]
    unified_budgets: dict[int, int]
    shortlisted_targets: list[Planet]
...
def attack_confidence_bonus(
    ...
    """V6.12 signal: reward clear lanes, nearby support, and high-value targets.

    This is intentionally local and additive. It does not alter target shortlist,
    opening rules, or source budgets.
    """
    target_heading = math.atan2(target_y - source.y, target_x - source.x)
    target_dist = distance_xy(source.x, source.y, target_x, target_y)
    nearest_gap = math.pi
...
def make_planner_context(obs):
    ...
    return PlannerContext(
        ...
        raw_initial_planets=raw_initial_planets,
        ...
        sources=sources,
        budgets=budgets,
        reserve_budgets=reserve_budgets,
        unified_budgets=unified_budgets,
        shortlisted_targets=shortlisted_targets,
    )


def generate_core_candidates(ctx):
    """Generate the main candidate pool.

    V7.0 keeps the V6.9 candidate builders unchanged, but centralizes them in
    one function.  Future branches should add candidate families here before
...
def select_core_plan(ctx, candidates):
    """Select the first-pass plan.

    This remains greedy in V7.0.  V7.2 should replace or wrap this with a
    two-stage selector: rule score shortlist -> forward evaluation.
    """
    return greedy_select(candidates, dict(ctx.unified_budgets), ctx.config)


def spent_by_source(selected, sources):
...
def remaining_budgets(base_budgets, selected, sources):
    spent = spent_by_source(selected, sources)
    return {
        source.id: max(0, base_budgets.get(source.id, 0) - spent.get(source.id, 0))
        for source in sources
    }


def append_multisource_phase(ctx, selected):
    budgets = remaining_budgets(ctx.budgets, selected, ctx.sources)
    return append_multisource_capture(
        selected,
...
def append_regroup_phase(ctx, selected):
    budgets = remaining_budgets(ctx.budgets, selected, ctx.sources)
    ...
    if regroup_candidates:
        selected.extend(greedy_select(regroup_candidates, budgets, ctx.config))
    return selected


def generate_shadow_candidates(ctx, budgets, selected):
    candidates = []
    candidates.extend(
...
def append_shadow_phase(ctx, selected):
    budgets = remaining_budgets(ctx.budgets, selected, ctx.sources)
    return greedy_append_shadow(
        selected,
        generate_shadow_candidates(ctx, budgets, selected),
        budgets,
        max_extra=shadow_extra_limit(selected, ctx.powers, ctx.player, ctx.current_step, ctx.is_2p),
        score_floor=shadow_score_floor(selected, ctx.powers, ctx.player, ctx.current_step, ctx.is_2p),
    )


def append_breakout_phase(ctx, selected):
    budgets = remaining_budgets(ctx.budgets, selected, ctx.sources)
    return append_breakout(
        selected,
...
def selected_to_actions(selected):
    return [[cand.source_id, cand.angle, int(cand.ships)] for cand in selected if cand.ships > 0]


def build_plan(ctx):
    candidates = generate_core_candidates(ctx)
    selected = select_core_plan(ctx, candidates)
    selected = append_multisource_phase(ctx, selected)
    selected = append_regroup_phase(ctx, selected)
    selected = append_shadow_phase(ctx, selected)
    selected = append_breakout_phase(ctx, selected)
    return selected


def agent(obs):
    ctx = make_planner_context(obs)
    if ctx is None:
        return []
    return selected_to_actions(build_plan(ctx))
