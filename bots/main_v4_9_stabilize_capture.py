...
def build_capture_stabilize_candidates(
    selected,
    sources,
    planets,
    fleets,
    ...
    if current_step > (170 if is_2p else 130):
        return []

    planets_by_id = {planet.id: planet for planet in planets}
    attack_targets = []
    for cand in selected:
        if cand.kind not in ("attack", "counter_snipe"):
            continue
        target = planets_by_id.get(cand.target_id)
        if target is None or target.owner == player or target.production < (4 if is_2p else 5):
            continue
        attack_targets.append((cand, target))

    if not attack_targets:
        return []

    candidates = []
    for attack, target in attack_targets:
        pressure = enemy_pressure(target, planets, fleets, player, 26 if is_2p else 16)
        hold_need = max(
            config.min_ships_to_launch,
            int(target.production * (2.0 if is_2p else 2.6) + pressure * 0.22),
        )
        if attack.ships >= hold_need + int(target.production):
            continue

        support_need = min(24 if is_2p else 16, hold_need + int(target.production) - attack.ships)
        for source in sources:
            if source.id == attack.source_id:
                continue
            budget = budgets.get(source.id, 0)
            if budget < config.min_ships_to_launch:
                continue
            ...
            if dist > (18.0 if is_2p else 13.0):
                continue
            send = min(budget, max(config.min_ships_to_launch, support_need))
            eta = dist / fleet_speed(send)
            if eta > attack.eta + (7.0 if is_2p else 4.5):
                continue
            angle = math.atan2(target.y - source.y, target.x - source.x)
            score = (
                target.production * 38.0
                + max(0.0, support_need - send) * -1.4
                - eta * 2.0
                - send * 0.55
            )
            if target.production >= 5:
...
def agent(obs):
    ...
    regroup_candidates.extend(
        build_capture_stabilize_candidates(
            selected,
            sources,
            planets,
            fleets,
