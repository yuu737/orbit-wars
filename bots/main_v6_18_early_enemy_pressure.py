...
def early_enemy_pressure_bonus(source, target, my_planets, planets, current_step, is_2p, powers, player):
    if not is_2p or current_step >= 95 or target.owner in (-1, player):
        return 0.0
    if target.production < 3:
        return 0.0

    support = friendly_support_count(target, my_planets, 17.0)
    nearest_enemy_support = min(
        (distance(enemy, target) for enemy in planets if enemy.owner not in (-1, player, target.owner)),
        default=99.0,
    )
    owner_stats = powers.get(int(target.owner), {})
    my_stats = powers.get(int(player), {})

    bonus = target.production * 9.0
    if target.production >= 5:
        bonus += 18.0
    if support > 0:
        bonus += 10.0
    if distance(source, target) <= 22.0:
        bonus += 8.0
    if owner_stats.get("power", 0) >= my_stats.get("power", 0):
        bonus += 14.0
    if nearest_enemy_support < 12.0:
        bonus -= 10.0
    return bonus


def target_shortlist(my_planets, targets, planets, config, current_step, is_2p, powers, player):
    ranked = []
    for target in targets:
        ...
        if target.production <= 1:
            value -= 45.0
        value += early_enemy_pressure_bonus(
            source, target, my_planets, planets, current_step, is_2p, powers, player
        )
...
def build_attack_candidates(
    ...
    for source in sources:
        ...
        for target in targets:
            ...
            cand.score += early_enemy_pressure_bonus(
                source, target, my_planets, planets, current_step, is_2p, powers, player
            )
