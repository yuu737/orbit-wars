...
def build_pressure_defense_candidates(sources, my_planets, planets, fleets, budgets, config, player, current_step, is_2p):
    if not is_2p or current_step < 62 or current_step > 122:
        return []

    pressures = {
        planet.id: enemy_pressure(planet, planets, fleets, player, 20)
    ...
    for target in my_planets:
        ...
        hold_target = int(target.production * 4 + target_pressure * 0.18)
        deficit = hold_target - int(target.ships)
        if deficit < 5:
            continue
        ...
        for source in sources:
            ...
            if source_pressure + 8.0 >= target_pressure:
                continue
            budget = budgets.get(source.id, 0)
            if budget < config.min_ships_to_launch:
                continue
            dist = distance(source, target)
            if dist > 16.0 or crosses_sun(source, target.x, target.y):
                continue
            send = min(budget, max(config.min_ships_to_launch, min(10, int(deficit * 0.55))))
            ...
            angle = math.atan2(target.y - source.y, target.x - source.x)
            score = (
                target.production * 30.0
                + min(deficit, 18) * 1.6
                + (target_pressure - source_pressure) * 0.35
                - send * 1.05
                - eta * 2.4
            )
...
def agent(obs):
    ...
    candidates.extend(
        build_pressure_defense_candidates(
            sources,
            my_planets,
            planets,
            fleets,
