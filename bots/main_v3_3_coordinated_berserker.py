"""
Orbit Wars - V3.3 Coordinated Berserker Agent
...
- V3.3: coordinate partial launches from multiple strong planets to the same valuable target.
"""
...
def base_reserve(planet, current_step):
    ...
    if current_step < 90:
        return max(1, planet.production // 2 + 1)
    if current_step < 160:
        return max(3, planet.production + 1)
    if current_step > 430:
        return max(7, planet.production * 3)
...
def reserve_for_planet(planet, current_step, fleets, player):
    ...
    # V3.2: no global high-production holding tax. Previous defensive variants slowed expansion.
    if current_step >= 220 and planet.production >= 5:
        reserve += 1
...
def target_score(source, target, committed_ships, current_step, angular_velocity, initial_planet, comet_ids, player):
    ...
    if target.owner == -1:
        ...
        if target.production <= 2 and current_step < 300:
            capture_cost += 18.0
...
def find_reinforcement_target(source, my_planets, fleets, player, current_step, available):
    ...
    if best is not None and best_score > 48.0:
...
def agent(obs):
    ...
    # V3.3: estimate how much attack budget remains after each source planet.
    # This allows several planets to combine fleets against one valuable target instead
    # of skipping it just because each individual source cannot capture it alone.
    raw_available_by_id = {}
    for p in ordered_my_planets:
        raw_reserve = reserve_for_planet(p, current_step, fleets, player)
        raw_available_by_id[p.id] = max(0, int(p.ships - raw_reserve))

    suffix_available = [0] * (len(ordered_my_planets) + 1)
    for i in range(len(ordered_my_planets) - 1, -1, -1):
        suffix_available[i] = suffix_available[i + 1] + raw_available_by_id.get(ordered_my_planets[i].id, 0)

    for source_index, mine in enumerate(ordered_my_planets):
        ...
        if current_step < 92:
            ...
            if best_target is not None and best_score > -24.0:
        ...
        # V3.3: if a valuable target cannot be captured by this planet alone, allow a
        # partial committed launch when later planets can plausibly finish the capture.
        best_target = None
        best_needed = 0
        best_send = 0
        best_score = float("-inf")

        future_available = suffix_available[source_index + 1] if source_index + 1 < len(suffix_available) else 0
        ...
        for target in targets:
            ...
            can_complete_now = needed <= available
            valuable_target = target.production >= 4 or (target.owner != -1 and target.production >= 3)
            enough_group_power = available + future_available >= needed
            partial_minimum = max(6, min(18, int(needed * 0.28)))
            can_coordinate = (
                not can_complete_now
                and valuable_target
                and enough_group_power
                and available >= partial_minimum
                and 70 <= current_step <= 390
            )

            if not can_complete_now and not can_coordinate:
                continue
            ...
            if can_coordinate:
                # Partial attacks are only worth it for high-value targets. Penalize
                # them enough that ordinary complete captures still win when close.
                score -= 18.0
                score -= max(0, needed - available) * 0.12
                if target.production >= 5:
                    score += 18.0
                if committed > 0:
                    score += 22.0
        ...
        min_score = -3.0
        if current_step >= 120 and best_target is not None and best_target.owner != -1:
            # V3.2: allow earlier pressure on production-4/5 enemy planets.
            min_score = -10.0 if best_target.production >= 4 else 10.0
        if current_step >= 390 and best_target is not None and best_target.owner == -1:
            min_score = 8.0
        if best_target is None or best_send <= 0 or (best_score <= min_score and not early_expand):
            continue
        ...
        angle = math.atan2(target_y - mine.y, target_x - mine.x)
        moves.append([mine.id, angle, int(best_send)])
        launched_from[mine.id] = launched_from.get(mine.id, 0) + int(best_send)
        target_commits[best_target.id] = target_commits.get(best_target.id, 0) + int(best_send)
