...
def prod1_neutral_gate(source, target, my_planets, planets, fleets, player, current_step, is_2p):
    if target.owner != -1 or int(target.production) != 1:
        return True
    if current_step >= (125 if is_2p else 95):
        return False
    ...
    if support_dist > (12.0 if is_2p else 9.0):
        return False
    if target_pressure > int(target.ships) + (1 if is_2p else 0):
        return False
    return True
    ...
    for source in sources:
        ...
        for target in targets:
            if target.id in comet_ids:
                continue
            if not prod1_neutral_gate(source, target, my_planets, planets, fleets, player, current_step, is_2p):
                continue
            ...
            if target.owner == -1 and int(target.production) == 1:
                cand.score -= 18.0 + max(0.0, eta - 4.5) * 7.0
