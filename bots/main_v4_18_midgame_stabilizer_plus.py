"""
Orbit Wars - V4.18 Midgame Stabilizer Plus
...
V4.18 settings:
- Planner-first branch; no V2/V3 fallback.
- Adds dynamic midgame stabilization and more exact defensive reinforcement.
- Keeps expansion active when production is behind.
"""
...
def config_with(**kwargs):
    base = dict(
        horizon=CONFIG_2P.horizon,
        max_sources=CONFIG_2P.max_sources,
        max_targets=CONFIG_2P.max_targets,
        max_actions=CONFIG_2P.max_actions,
        roi_threshold=CONFIG_2P.roi_threshold,
        min_ships_to_launch=CONFIG_2P.min_ships_to_launch,
        regroup_enabled=CONFIG_2P.regroup_enabled,
        regroup_distance=CONFIG_2P.regroup_distance,
        regroup_threshold=CONFIG_2P.regroup_threshold,
        reserve_margin=CONFIG_2P.reserve_margin,
    )
    base.update(kwargs)
    return PlannerConfig(**base)


def choose_config(is_2p, current_step, powers, player):
    base = CONFIG_2P if is_2p else CONFIG_4P
    my_stats = powers.get(int(player), {})
    enemy_stats = [v for k, v in powers.items() if int(k) != int(player)]
    best_enemy_prod = max((e.get("production", 0) for e in enemy_stats), default=0)
    my_prod = my_stats.get("production", 0)
    prod_gap = best_enemy_prod - my_prod
    ...
    data = dict(
        ...
        roi_threshold=base.roi_threshold,
        min_ships_to_launch=base.min_ships_to_launch,
        regroup_enabled=base.regroup_enabled,
        regroup_distance=base.regroup_distance,
        regroup_threshold=base.regroup_threshold,
        reserve_margin=base.reserve_margin,
    )

    if is_2p:
        if current_step < 90:
            data.update(horizon=22, max_targets=15, max_actions=7, roi_threshold=1.15, reserve_margin=1)
        elif current_step < 230:
            data.update(horizon=24, max_targets=15, max_actions=7, roi_threshold=1.35, regroup_distance=8.5, regroup_threshold=7.0, reserve_margin=2)
        else:
            data.update(horizon=20, max_targets=13, max_actions=6, roi_threshold=1.7, regroup_distance=8.0, regroup_threshold=8.0, reserve_margin=3)
        if prod_gap >= 4:
            data["roi_threshold"] = min(data["roi_threshold"], 1.05)
            data["max_actions"] = max(data["max_actions"], 7)
            data["reserve_margin"] = max(0, data["reserve_margin"] - 1)
    else:
        if current_step < 80:
            data.update(horizon=16, max_targets=12, max_actions=6, roi_threshold=1.55, reserve_margin=2)
        elif current_step < 230:
            data.update(horizon=17, max_targets=12, max_actions=6, roi_threshold=1.75, regroup_distance=6.5, regroup_threshold=8.0)
        if prod_gap >= 5:
            data["roi_threshold"] = min(data["roi_threshold"], 1.45)
...
def score_candidate(candidate, target, projection, planets, fleets, player, current_step, is_2p, powers):
    ...
    # Midgame stabilizer: prefer nearby productive targets that can be held,
    # and avoid spending turns on low-production neutrals after the opening.
    if candidate.kind == "attack":
        if current_step < 230 and target.production >= 4 and candidate.eta <= 13.0:
            score += 18.0
        if target.owner == -1 and current_step > 135 and target.production <= 2:
            score -= 28.0
        if target.owner != -1 and candidate.eta <= 11.0:
    ...
    for source in sources:
        ...
        for target in targets:
            ...
            if target.owner == -1 and target.production >= 4:
                hold_buffer = 1 if current_step < 120 else 2
            elif target.owner != -1:
                hold_buffer = 2 + max(0, int(target.production) - 3)
            send = min(budget, int(needed + hold_buffer))
            if send > budget or send < config.min_ships_to_launch:
                continue
...
def build_defense_candidates(sources, my_planets, projection, budgets, config, player, current_step):
    ...
    for target in my_planets:
        ...
        enemy_incoming = sum(v for owner, v in incoming.items() if int(owner) != int(player))
        friendly_incoming = sum(v for owner, v in incoming.items() if int(owner) == int(player))
        projected_before = max(0, int(target.ships) + int(target.production) * max(0, loss_turn - 1))
        margin = max(3, int(target.production) + (2 if current_step < 220 else 4))
        need = max(
            config.min_ships_to_launch,
            enemy_incoming - projected_before - friendly_incoming + margin,
            int(target.production * 2 + 3),
        )
        ...
        for source in sources:
            ...
            saved = target.production * 58.0 + remaining * target.production * 0.16 + int(target.ships) * 0.55
            urgency = max(0.0, config.horizon - loss_turn) * 3.0
            score = saved + urgency - need * 0.72 - eta * 2.2
...
def agent(obs):
    ...
    config = choose_config(is_2p, current_step, powers, player)
