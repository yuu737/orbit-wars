"""
Orbit Wars - V4.4b Selective Defense Planner Branch
...
- Size defense candidates from projected loss severity, gated to valuable/urgent planets.
...
def projected_defense_need(target, projection, loss_turn, player):
    ...
    return max(4, estimated_shortfall + int(target.production) * 2 + 3)
...
def build_defense_candidates(sources, my_planets, projection, budgets, config, player, current_step):
    ...
    for target in my_planets:
        ...
        if target.production < 4 and loss_turn > 8:
            continue
        need = projected_defense_need(target, projection, loss_turn, player)
