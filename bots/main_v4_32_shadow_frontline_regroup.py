...
def build_shadow_opening_expansion(
    ...
    for source in sources:
        ...
        for target in neutral_targets:
            ...
            if eta_probe > (14.0 if is_2p else 10.0) or crosses_sun(source, tx, ty):
                continue
            ...
            if eta > (14.0 if is_2p else 10.0) or crosses_sun(source, tx, ty):
                continue
            ...
            if target.production >= 4 and retake_risk_after_capture(target, planets, player, eta, post_capture, is_2p):
                continue
...
def build_shadow_frontline_regroup(
    sources,
    my_planets,
    planets,
    fleets,
    ...
    if current_step < (45 if is_2p else 35) or current_step > (180 if is_2p else 125):
        return []
    used_sources, _, _ = selected_ids(selected)
    pressures = {
        planet.id: enemy_pressure(planet, planets, fleets, player, 22 if is_2p else 15)
        for planet in my_planets
    }
    candidates = []
    for target in my_planets:
        ...
        desired = int(target.production * (4 if is_2p else 5) + target_pressure * 0.16)
        ...
        for source in sources:
            ...
            if budget < config.min_ships_to_launch:
                continue
            dist = distance(source, target)
            if dist > (22.0 if is_2p else 15.0):
                continue
            source_pressure = pressures.get(source.id, 0.0)
            if source.id not in used_sources and source_pressure > target_pressure + 8:
                continue
            send = min(budget, max(config.min_ships_to_launch, min(gap, 18 if is_2p else 12)))
            eta = dist / fleet_speed(send)
            angle = math.atan2(target.y - source.y, target.x - source.x)
            score = target.production * 34.0 + gap * 1.3 + target_pressure * 0.4 - send * 0.55 - eta * 1.8
            candidates.append(Candidate("shadow_regroup", source.id, target.id, angle, int(send), eta, score))
    return candidates
...
def agent(obs):
    ...
    shadow_candidates.extend(
        build_shadow_frontline_regroup(
            sources,
            my_planets,
            planets,
            fleets,
