"""
Orbit Wars - V3.14 Projected Planner Agent
...
def find_projected_planner_target(
    ...
    threshold = 18.0 if is_2p else 24.0
...
def agent(obs):
    ...
    for mine in my_planets:
        available_by_id[mine.id] = min(
            available_by_id.get(mine.id, 0),
            safe_drain_from_projection(mine, projected_states, player, planner_horizon, safe_margin),
        )
