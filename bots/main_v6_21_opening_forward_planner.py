...
def attack_confidence_bonus(
    ...
    for other in planets:
        ...
        other_heading = math.atan2(oy - source.y, ox - source.x)
        nearest_gap = min(nearest_gap, angle_diff(target_heading, other_heading))
        other_dist = distance_xy(source.x, source.y, ox, oy)
        if other_dist < blocking_dist:
            blocking_dist = other_dist

    support_dist = min(
        (distance(friend, target) for friend in my_planets if friend.id != source.id),
        default=99.0,
    )
    bonus = min(22.0, nearest_gap * 55.0)
    if support_dist <= 12.0:
        bonus += 10.0
    elif support_dist <= 16.0:
        bonus += 4.0
    if target.production >= 5:
        bonus += 12.0
    elif target.production >= 4:
        bonus += 6.0
    if initial_planets.get(target.id) is not None and not is_rotating(initial_planets[target.id]):
        bonus += 4.0
    if blocking_dist >= target_dist - 0.75:
        bonus += 5.0
    return bonus
...
def opening_turn_limit(is_2p):
    return 14 if is_2p else 10


def soft_opening_neutral_score(source, target, my_planets, initial_planets, targets, current_step, is_2p):
    if current_step >= opening_turn_limit(is_2p) or target.owner != -1:
        return 0.0
    ...
    if int(target.production) <= 1 and int(target.ships) >= 14:
        score -= 12.0
    if int(target.production) <= 1 and any(other.owner == -1 and int(other.production) >= 2 for other in targets):
        score -= 14.0

    initial = initial_planets.get(target.id)
    if initial is not None and not is_rotating(initial):
        score += 6.0
    if support_dist <= (14.5 if is_2p else 10.5):
        score += 5.0
    elif support_dist > (24.0 if is_2p else 16.0):
        score -= 5.0
    return score


def early_highprod_enemy_bonus(target, current_step, is_2p, powers, player):
    if not is_2p or current_step >= 90 or target.owner in (-1, player):
        return 0.0
...
def target_shortlist(my_planets, targets, planets, config, initial_planets, current_step, is_2p, powers, player):
    ranked = []
    for target in targets:
        ...
        if target.production <= 1:
            value -= 45.0
        value += soft_opening_neutral_score(
            source, target, my_planets, initial_planets, targets, current_step, is_2p
        )
        value += early_highprod_enemy_bonus(target, current_step, is_2p, powers, player)
        ranked.append((value, target))
    ranked.sort(key=lambda item: item[0], reverse=True)
    max_targets = config.max_targets + (1 if current_step < opening_turn_limit(is_2p) and is_2p else 0)
...
def build_attack_candidates(
    ...
    for source in sources:
        ...
        for target in targets:
            ...
            cand.score += attack_confidence_bonus(
                source,
                target,
                my_planets,
                planets,
            ...
            cand.score += soft_opening_neutral_score(
                source, target, my_planets, initial_planets, targets, current_step, is_2p
            )
            cand.score += early_highprod_enemy_bonus(target, current_step, is_2p, powers, player)
...
# ---------------------------------------------------------------------------
# V6.21 opening-forward-only layer
#
# V6.20 proved that short-horizon evaluation has useful signal against strong
# opening bots, but applying it broadly damaged the stable V6.19 behavior.
# This layer is intentionally narrow: only 2-player, only the opening, only
# neutral attack candidates, and only the top few original candidates receive a
# small clamped adjustment.
# ---------------------------------------------------------------------------


def opening_forward_eval_score(planets, fleets, player, horizon, orders):
    owner_by_id = {}
    ships_by_id = {}
    production_by_id = {}
    incoming_by_id = {}
    ...
    for planet in planets:
        owner_by_id[planet.id] = int(planet.owner)
        ships_by_id[planet.id] = int(planet.ships)
        production_by_id[planet.id] = int(planet.production)
        incoming_by_id[planet.id] = [dict() for _ in range(horizon + 1)]

    planet_ids = list(owner_by_id.keys())
    for order in orders:
        if order.source_id in ships_by_id and owner_by_id.get(order.source_id) == int(player):
            ships_by_id[order.source_id] = max(0, ships_by_id[order.source_id] - int(order.ships))
        if order.target_id in incoming_by_id:
            turn = max(1, min(horizon, int(math.ceil(order.eta))))
            arrivals = incoming_by_id[order.target_id][turn]
            arrivals[int(player)] = arrivals.get(int(player), 0) + int(order.ships)
    ...
    for turn in range(1, horizon + 1):
        for planet_id in planet_ids:
            owner = owner_by_id[planet_id]
            ships = ships_by_id[planet_id]
            if owner >= 0:
                ships += production_by_id[planet_id]
            owner, ships = resolve_planet_combat(owner, ships, incoming_by_id[planet_id][turn])
            owner_by_id[planet_id] = owner
            ships_by_id[planet_id] = ships
    ...
    for planet_id in planet_ids:
        ...
        if owner < 0:
            continue
        entry = stats.setdefault(owner, {"production": 0, "ships": 0, "planets": 0})
        entry["production"] += production_by_id[planet_id]
        entry["ships"] += ships_by_id[planet_id]
        entry["planets"] += 1

    def value(entry):
        return (
            entry.get("production", 0) * 90.0
            + entry.get("ships", 0) * 0.65
            + entry.get("planets", 0) * 16.0
        )

    my_value = value(stats.get(int(player), {}))
    enemy_value = max(
        (value(entry) for owner, entry in stats.items() if int(owner) != int(player)),
        default=0.0,
    )
    return my_value - enemy_value


def apply_opening_forward_to_candidates(candidates, planets, fleets, player, current_step, is_2p):
    if not candidates or not is_2p:
        return candidates
    if current_step >= 14:
        return candidates
    ...
    eligible = [
        ...
        if cand.kind == "attack"
        and planet_by_id.get(cand.target_id) is not None
        and int(planet_by_id[cand.target_id].owner) == -1
    ]
    if not eligible:
        return candidates

    horizon = 18
    baseline = opening_forward_eval_score(planets, fleets, player, horizon, [])
    top_ids = {
        id(cand)
        for cand in sorted(eligible, key=lambda item: item.score, reverse=True)[:10]
    }

    for cand in eligible:
        if id(cand) not in top_ids:
            continue
        target = planet_by_id.get(cand.target_id)
        if target is None:
            continue
        delta = opening_forward_eval_score(planets, fleets, player, horizon, [cand]) - baseline
        bonus = delta * 0.038

        # Small tactical nudges: prefer high-production neutrals that the
        # forward evaluator says are safe, and penalize slow/expensive low-prod
        # captures that only look good by static score.
        if target.production >= 5 and bonus > 0:
            bonus += 4.0
        if target.production <= 2 and cand.eta > 8.0:
            bonus -= 4.0

        cand.score += max(-20.0, min(30.0, bonus))
    return candidates
