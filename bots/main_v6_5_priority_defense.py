...
Experimental V6.5 settings:
- Keep the V6.3 selective hold-aware attack branch as the base.
- Add narrow priority-defense candidates for collapsing production-5 planets.
- Let only those priority defenses see a limited reserve budget inside the unified pool.
"""
...
def build_priority_defense_candidates(sources, my_planets, projection, safe_budgets, reserve_budgets, config, player, current_step, is_2p):
    candidates = []
    for target in my_planets:
        loss_turn = projection.first_loss_turn_by_id.get(target.id)
        if loss_turn is None or loss_turn > (6 if is_2p else 4):
            continue
        if target.production < 5:
            continue
        ...
        need = max(config.min_ships_to_launch, shortfall + int(target.production) + 4)
        for source in sources:
            if source.id == target.id:
                continue
            safe_budget = safe_budgets.get(source.id, 0)
            budget = max(safe_budget, reserve_budgets.get(source.id, 0))
            if budget < need:
                continue
            eta = distance(source, target) / fleet_speed(max(1, need))
            if eta > loss_turn + 0.75:
                continue
            angle = math.atan2(target.y - source.y, target.x - source.x)
            saved = (
                target.production * 92.0
                + max(0, END_STEP - current_step - loss_turn) * target.production * 0.16
                + shortfall * 2.2
                + max(0.0, 7 - loss_turn) * 16.0
            )
            score = saved - need * 1.02 - eta * 2.7
            if safe_budget < need:
                score -= (need - safe_budget) * 0.38
            candidates.append(Candidate("priority_defense", source.id, target.id, angle, int(need), eta, score))
    return candidates
...
def greedy_select(candidates, budgets, config):
    ...
    for cand in sorted(candidates, key=lambda item: item.score, reverse=True):
        ...
        if cand.source_id in defended_targets:
            continue
        if cand.kind in ("defense", "urgent_hold", "priority_defense") and cand.target_id in used_sources:
            continue
        ...
        if cand.kind in ("defense", "urgent_hold", "priority_defense"):
...
def agent(obs):
    ...
    planner_candidates.extend(
        build_priority_defense_candidates(
            sources,
            my_planets,
            projection,
            budgets,
