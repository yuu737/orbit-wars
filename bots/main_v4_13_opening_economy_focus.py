...
def target_shortlist(my_planets, targets, planets, config, current_step, is_2p):
    ranked = []
    my_production = sum(int(planet.production) for planet in my_planets)
    for target in targets:
        ...
        if target.production <= 1:
            value -= 45.0
        if current_step <= (85 if is_2p else 65) and target.owner == -1:
            if target.production >= 5:
                ...
                value -= 120.0
            elif target.production <= 2 and nearest > 13.0:
                value -= 72.0
            if my_production < (10 if is_2p else 12) and target.production >= 4:
                value += 45.0
...
def score_candidate(candidate, target, projection, planets, fleets, player, current_step, is_2p, powers):
    ...
    if candidate.kind == "attack" and target.owner == -1 and current_step <= (85 if is_2p else 65):
        ...
        if my_prod < (10 if is_2p else 12) and target.production >= 4:
            score += 55.0
        if candidate.eta > (9.5 if is_2p else 7.0) and target.production <= 2:
            score -= 55.0
    if target.production >= 5:
        score += 34.0
    elif target.production >= 4:
...
def agent(obs):
    ...
    shortlisted_targets = target_shortlist(my_planets, targets, planets, config, current_step, is_2p)
    candidates = []
