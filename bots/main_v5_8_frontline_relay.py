...
Experimental V5.8 settings:
- Keep the V5.6 urgent-hold planner as the base.
- Add a narrow frontline-relay layer to pre-position ships before collapses happen.
- Use at most one early/midgame relay from a low-pressure rear planet into a threatened high-value ally.
"""
...
def build_frontline_relay_candidates(my_planets, planets, fleets, budgets, config, player, current_step, is_2p):
    if current_step > (130 if is_2p else 95):
        return []
    ...
    for source in my_planets:
        ...
        for target in my_planets:
            ...
            if dist > (20.0 if is_2p else 14.0):
                continue
            target_pressure = pressures.get(target.id, 0.0)
            gap = target_pressure - source_pressure
            if gap < (16.0 if is_2p else 12.0):
                continue
            send = min(int(budget), max(config.min_ships_to_launch, int(gap * 0.32)))
            eta = dist / fleet_speed(max(1, send))
            if eta > (9.0 if is_2p else 6.0):
                continue
            angle = math.atan2(target.y - source.y, target.x - source.x)
            score = (
                target.production * (34.0 if is_2p else 28.0)
                + gap * 2.1
                - send * 0.62
                - eta * 1.8
            )
...
def agent(obs):
    ...
    relay_selected = greedy_select_limited(
        ...
        dict(budgets),
        max_actions=1,
        roi_threshold=max(12.0, config.roi_threshold),
    )
    relay_spent = {}
    for cand in relay_selected:
        relay_spent[cand.source_id] = relay_spent.get(cand.source_id, 0) + cand.ships
    urgent_budgets = {
        source.id: max(0, urgent_budgets.get(source.id, 0) - relay_spent.get(source.id, 0))
        for source in my_planets
    }
    ...
    budgets = {
        source.id: max(0, budgets.get(source.id, 0) - relay_spent.get(source.id, 0) - urgent_spent.get(source.id, 0))
        for source in sources
    }
    ...
    committed = relay_selected + urgent_selected
    urgent_sources = {cand.source_id for cand in committed}
    urgent_targets = {cand.target_id for cand in committed if cand.kind != "regroup"}
    if committed:
        candidates = [
            cand for cand in candidates
            if cand.source_id not in urgent_sources
            and (cand.kind == "regroup" or cand.target_id not in urgent_targets)
        ]

    selected = list(relay_selected)
