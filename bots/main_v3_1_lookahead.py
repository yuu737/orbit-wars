"""
Orbit Wars - V3.1 One-Step Lookahead Agent
...
- Keep V2.7/V2.9 speed, with selective high-production recapture pressure.
- Avoid V2.8/V3.0-style global defense slowdown.
- Add soft one-step lookahead to avoid obviously disposable captures.
...
def one_step_safety_adjust(source, target, target_x, target_y, needed, current_step, planets, player):
    """Small 1-step lookahead.

    Estimate whether a captured target is immediately exposed to nearby enemy planets.
    This is intentionally a soft score adjustment, not a hard defense mode, so it should
    avoid V2.8/V3.0-style slowdown.
    """
    if current_step < 105:
        return 0.0

    # Cheap low-production targets are allowed to be expendable.
    if target.production <= 1 and target.owner == -1:
        return 0.0
    ...
    for p in planets:
        if p.id == source.id or p.id == target.id:
            continue
        d = distance_xy(p.x, p.y, target_x, target_y)
        if d > 46:
            continue

        # Larger nearby garrisons can counterattack faster; production matters in midgame.
        local_power = max(0.0, p.ships - base_reserve(p, current_step)) + p.production * 6.0
        proximity = max(0.0, 1.0 - d / 46.0)

        if p.owner == player:
            friendly_cover += local_power * proximity
        elif p.owner >= 0:
            enemy_threat += local_power * proximity

    # Current incoming fleets near this target also matter, but keep the weight low.
    # We only want to filter obviously bad captures, not become defensive.
    safety_margin = friendly_cover + target.production * 10.0
    if target.owner == -1:
        safety_margin += 12.0
    if target.production >= 5:
        safety_margin += 18.0
    ...
    danger = enemy_threat - safety_margin
    if danger <= 0:
        # Mild bonus for attacks that are naturally supported by nearby owned planets.
        return min(14.0, friendly_cover * 0.05)

    # Soft penalty. Enemy planets and low-production targets are penalized more.
    mult = 0.22
    if target.owner != -1:
        mult = 0.34
    ...
    # Never fully block high-production opportunities through this heuristic alone.
    if target.production >= 5:
        penalty = min(penalty, 26.0)
    elif target.production >= 4:
        penalty = min(penalty, 34.0)

    return -penalty


def target_score(source, target, committed_ships, current_step, angular_velocity, initial_planet, comet_ids, player, planets):
    needed = ships_needed_to_capture(target) - committed_ships
    if needed <= 0:
        return float("-inf")
    ...
    # V3.1: one-step lookahead. Penalize targets that are likely to be recaptured
    # immediately by nearby enemy planets, while preserving V2.9's aggressive economy.
    value += one_step_safety_adjust(
        source, target, target_x, target_y, needed, current_step, planets, player
    )
