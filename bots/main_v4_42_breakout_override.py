...
def is_behind_for_breakout(powers, player, is_2p, current_step):
    if not is_2p:
        return False
    my_stats = powers.get(int(player), {})
...
def build_desperation_breakout(
    ...
    if current_step < 145 or current_step > 470:
        return []
    if not is_behind_for_breakout(powers, player, is_2p, current_step):
        return []
    ...
    for source in sources:
        ...
        if int(source.ships) >= 70:
            reserve_break_budget = int(source.ships * (0.42 if current_step < 300 else 0.55))
        ...
        for target in targets:
            ...
            if eta_probe > 35.0 or crosses_sun(source, tx, ty):
                continue
            needed = capture_floor(target, projection, eta_probe, player, overhead=3)
            can_capture = needed <= budget
            if can_capture:
                send = min(budget, needed + max(4, min(18, int(target.production * 3))))
            else:
                # When we are far behind, a large idle stack should still apply
                # pressure instead of waiting forever for a perfect capture.
                if target.production < 4 or budget < 28:
                    continue
                send = max(18, min(budget, int(budget * (0.62 if current_step >= 260 else 0.48))))

            tx, ty, eta = predict_intercept_position(
                source, target, initial_planets.get(target.id), current_step, angular_velocity, comet_ids, send
            )
            if eta > 35.0 or crosses_sun(source, tx, ty):
                continue
            angle = math.atan2(ty - source.y, tx - source.x)
            score = (
                target.production * (72.0 if can_capture else 48.0)
                + min(40, int(target.ships)) * 0.35
                - send * (0.55 if can_capture else 0.30)
                - eta * 2.0
            )
            if can_capture:
                score += 42.0
