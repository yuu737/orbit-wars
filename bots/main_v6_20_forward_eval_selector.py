...
def forward_eval_score(planets, fleets, player, horizon, orders):
    owner_by_id = {}
    ships_by_id = {}
    production_by_id = {}
    incoming_by_id = {}
    ...
    planet_by_id = {planet.id: planet for planet in planets}
    for order in orders:
        if order.source_id in ships_by_id and owner_by_id.get(order.source_id) == player:
    ...
    stats = {}
    for planet in planets:
        owner = owner_by_id[planet.id]
        if owner < 0:
            continue
        entry = stats.setdefault(owner, {"production": 0, "ships": 0, "planets": 0})
        entry["production"] += production_by_id[planet.id]
        entry["ships"] += ships_by_id[planet.id]
        entry["planets"] += 1

    def value(entry):
        return entry.get("production", 0) * 85.0 + entry.get("ships", 0) * 0.9 + entry.get("planets", 0) * 18.0
...
def apply_forward_eval_to_candidates(candidates, planets, fleets, player, current_step, is_2p):
    if not candidates:
        return candidates
    if current_step > (285 if is_2p else 190):
        return candidates

    horizon = 22 if is_2p else 14
    baseline = forward_eval_score(planets, fleets, player, horizon, [])
    top_ids = {
        id(cand)
        for cand in sorted(candidates, key=lambda item: item.score, reverse=True)[:18 if is_2p else 12]
    }
    for cand in candidates:
        if id(cand) not in top_ids:
            continue
        delta = forward_eval_score(planets, fleets, player, horizon, [cand]) - baseline
        if cand.kind in ("defense", "urgent_hold"):
            weight = 0.060
        elif cand.kind in ("attack", "counter_snipe"):
            weight = 0.052
        else:
            weight = 0.035
        cand.score += max(-85.0, min(115.0, delta * weight))
    return candidates
...
def agent(obs):
    ...
    planner_candidates = apply_forward_eval_to_candidates(
