...
def apply_forward_eval_to_candidates(candidates, planets, fleets, player, current_step, is_2p):
    ...
    horizon = 20 if is_2p else 12
    baseline = forward_eval_score(planets, fleets, player, horizon, [])
    top_ids = {
        id(cand)
        for cand in sorted(candidates, key=lambda item: item.score, reverse=True)[:12 if is_2p else 8]
    }
    for cand in candidates:
        ...
        if cand.kind in ("defense", "urgent_hold"):
            ...
            weight = 0.012
        cand.score += max(-32.0, min(44.0, delta * weight))
    return candidates
