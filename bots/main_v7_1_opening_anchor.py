...
Experimental V7.1 settings:
- Keep the V6.3 unified planner and selective hold-aware attack as the base.
- Add explicit opening-anchor candidates for early production-4/5 neutral captures.
- Try to create a strong high-production foothold before hairate's pressure cycle starts.
"""
...
def build_opening_anchor_candidates(
    sources,
    my_planets,
    planets,
    fleets,
    ...
    if not is_2p or current_step > 82:
        return []
    ...
    neutral_targets = [
        ...
        and planet.production >= 4
    ]
    for target in neutral_targets:
        support_dist = nearest_support_distance(target, my_planets)
        pressure = enemy_pressure(target, planets, fleets, player, 18)
        for source in sources:
            budget = budgets.get(source.id, 0)
            if budget < config.min_ships_to_launch:
                continue
            ...
            if eta > 15.0 or crosses_sun(source, tx, ty):
                continue
            needed = capture_floor(target, projection, eta, player, overhead=2)
            hold_margin = 2 + max(0, int(target.production) - 4)
            send = int(needed + hold_margin)
            if send > budget or send < config.min_ships_to_launch:
                continue
            post_capture = max(1, send - int(needed) + 1)
            if target.production == 4 and support_dist > 18.0 and pressure > int(target.ships) + 9:
                continue
            if retake_risk_after_capture(target, planets, player, eta, post_capture, is_2p):
                continue

            ring_bonus = max(0.0, orbital_ring_value(target) - 24.0)
            score = (
                target.production * 86.0
                + ring_bonus * 1.3
                + max(0.0, 82 - current_step) * 0.45
                - send * 1.05
                - eta * 3.1
                - max(0.0, support_dist - 18.0) * 1.8
                - max(0.0, pressure - send) * 0.24
            )
            if target.production >= 5:
                score += 52.0
            angle = math.atan2(ty - source.y, tx - source.x)
            candidates.append(Candidate("opening_anchor", source.id, target.id, angle, int(send), eta, score))
    return candidates
...
def agent(obs):
    ...
    planner_candidates.extend(
        build_opening_anchor_candidates(
            sources,
            my_planets,
            planets,
            fleets,
