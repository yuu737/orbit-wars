...
Experimental V6.4 settings:
- Keep the V6.3 selective hold-aware attack branch as the base.
- Add a very narrow 2-source hold rescue for collapsing production-5 planets.
- Gate the branch first on the focused hairate benchmark before broad evaluation.
"""
...
def build_multisource_hold_candidates(sources, my_planets, projection, budgets, config, player, current_step, is_2p, selected):
    used_sources, _, defended_targets = selected_ids(selected)
    candidates = []
    if not is_2p:
        return candidates
    ...
    for target in my_planets:
        ...
        if target.production < 5:
            continue
        loss_turn = projection.first_loss_turn_by_id.get(target.id)
        if loss_turn is None or loss_turn > 6:
            continue

        previous_turn = max(0, loss_turn - 1)
        incoming = projection.incoming_by_id[target.id][loss_turn]
        enemy_incoming = sum(ships for owner, ships in incoming.items() if owner not in (-1, player))
        friendly_incoming = incoming.get(player, 0)
        pre_loss_ships = projection.ships_by_id[target.id][previous_turn]
        produced = int(target.production) if target.owner >= 0 else 0
        shortfall = max(0, enemy_incoming - friendly_incoming - pre_loss_ships - produced)
        ...
        for source in sorted(sources, key=lambda item: distance(item, target)):
            ...
            if eta > loss_turn + 0.8:
                continue
            send = min(int(budget), max(config.min_ships_to_launch, int(shortfall * 0.65) + 2))
            if send < config.min_ships_to_launch:
                continue
            parts.append((eta, source, send))
        ...
        planned = []
        total_send = 0
        for eta, source, send in parts[:2]:
            angle = math.atan2(target.y - source.y, target.x - source.x)
            planned.append(Candidate("multi_hold", source.id, target.id, angle, int(send), eta, 0.0))
            total_send += int(send)

        hold_need = shortfall + int(target.production) + 3
        if total_send < hold_need:
            continue

        score = (
            target.production * 96.0
            + max(0.0, 7 - loss_turn) * 16.0
            + shortfall * 2.8
            - total_send * 1.05
            - max(order.eta for order in planned) * 2.6
        )
        candidates.append(MultiCandidate("multi_hold", target.id, planned, max(order.eta for order in planned), score))
    return candidates


def append_multisource_hold(selected, multi_candidates, budgets):
    used_sources, taken_targets, defended_targets = selected_ids(selected)
    for multi in sorted(multi_candidates, key=lambda item: item.score, reverse=True):
        if multi.score <= 70.0:
            break
        if multi.target_id in taken_targets or multi.target_id in defended_targets:
            continue
        if any(order.source_id in used_sources for order in multi.orders):
            continue
        if any(order.ships > budgets.get(order.source_id, 0) for order in multi.orders):
...
def agent(obs):
    ...
    selected = append_multisource_hold(
        selected,
