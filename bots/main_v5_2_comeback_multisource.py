...
Experimental V5.2 settings:
- Keep V5.1's guarded planner as the base.
- Restrict multi-source capture to comeback-only attacks against valuable enemy planets.
- Limit multi-source plans to two sources and reject obvious quick retakes.
"""
...
def build_multisource_capture_candidates(
    ...
    if not is_2p or current_step < 105 or current_step > 260:
        return []

    used_sources, taken_targets, _ = selected_ids(selected)
    if sum(1 for cand in selected if cand.kind in ("attack", "counter_snipe", "breakout")) >= 2:
        return []

    my_stats = powers.get(int(player), {})
    enemy_stats = [
        stats for owner, stats in powers.items()
        if int(owner) != int(player)
    ]
    if not enemy_stats:
        return []
    strongest_enemy = max(enemy_stats, key=lambda stats: stats.get("power", 0))
    if my_stats.get("power", 0) >= strongest_enemy.get("power", 0) * 0.98:
        return []
    ...
    for target in valuable_targets:
        pressure = enemy_pressure(target, planets, fleets, player, 22)
        parts = []
        total_send = 0
        arrival_eta = 0.0
        ...
        planned = []
        for eta, source, tx, ty, budget in parts[:2]:
            needed_at_eta = capture_floor(target, projection, eta, player, overhead=3)
            remaining_need = max(0, needed_at_eta + max(6, int(target.production * 2.5)) - total_send)
            if remaining_need <= 0:
                break
            send = min(budget, max(config.min_ships_to_launch, min(remaining_need, int(budget * 0.48))))
        ...
        hold_margin = max(6, int(target.production * 2.2), int(pressure * 0.18))
        if total_send < needed_final + hold_margin:
            continue
        total_budget = sum(budgets.get(order.source_id, 0) for order in planned)
        if total_send > max(18, int(total_budget * 0.54)):
            continue
        projected_garrison = projection.ships_by_id[target.id][
            max(1, min(len(projection.ships_by_id[target.id]) - 1, int(math.ceil(arrival_eta))))
        ]
        post_capture = max(1, total_send - int(projected_garrison))
        if retake_risk_after_capture(target, planets, player, arrival_eta, post_capture, is_2p):
            continue
        score = (
