...
def build_opening_combo_candidates(
    sources,
    targets,
    planets,
    fleets,
    ...
    if current_step > (95 if is_2p else 70):
        return []

    candidates = []
    combo_limit = 3 if is_2p else 2
    max_eta = min(config.horizon + (4 if is_2p else 2), 22 if is_2p else 16)

    high_value_targets = [
        target for target in targets
        if target.owner == -1
        and target.id not in comet_ids
        and target.production >= (4 if is_2p else 5)
    ]
    ...
    for target in high_value_targets:
        ...
        for source in sources:
            ...
            target_x, target_y, eta = predict_intercept_position(
                ...
                angular_velocity,
                comet_ids,
                min(budget, max(config.min_ships_to_launch, int(target.ships))),
            )
            if eta > max_eta or crosses_sun(source, target_x, target_y):
                continue
            pieces.append((eta, distance(source, target), source, target_x, target_y, budget))
        ...
        total_budget = 0
        max_arrival = 0.0
        for eta, _, source, target_x, target_y, budget in pieces[:combo_limit]:
            selected.append((source, target_x, target_y, budget, eta))
            total_budget += budget
            max_arrival = max(max_arrival, eta)
            needed = capture_floor(target, projection, max_arrival, player, overhead=2)
        ...
        needed = capture_floor(target, projection, max_arrival, player, overhead=2)
        if total_budget < needed or needed < config.min_ships_to_launch * 2:
            continue

        remaining_need = int(needed)
        group_id = f"opening_combo:{target.id}:{current_step}"
        group_candidates = []
        for index, (source, target_x, target_y, budget, eta) in enumerate(selected):
            slots_left = len(selected) - index
            min_for_rest = config.min_ships_to_launch * max(0, slots_left - 1)
            send = min(budget, max(config.min_ships_to_launch, remaining_need - min_for_rest))
            remaining_need -= send
            if send <= 0:
                continue
            angle = math.atan2(target_y - source.y, target_x - source.x)
            group_candidates.append(
                Candidate("opening_combo", source.id, target.id, angle, int(send), eta, 0.0, group_id)
            )

        if len(group_candidates) < 2 or remaining_need > 0:
            continue

        total_sent = sum(cand.ships for cand in group_candidates)
        synthetic = Candidate("attack", group_candidates[0].source_id, target.id, 0.0, total_sent, max_arrival, 0.0)
        score = score_candidate(synthetic, target, projection, planets, fleets, player, current_step, is_2p, powers)
        score += target.production * (42.0 if is_2p else 28.0)
        score -= (len(group_candidates) - 1) * 9.0
        if current_step <= 45:
...
def greedy_select(candidates, budgets, config):
    ...
    for cand in candidates:
        if cand.group_id:
            grouped.setdefault(cand.group_id, []).append(cand)
        else:
            singles.append(cand)

    selection_units = singles + [
    ...
    def unit_score(unit):
        if isinstance(unit, list):
            return max(cand.score for cand in unit)
        return unit.score

    for unit in sorted(selection_units, key=unit_score, reverse=True):
        if len(selected) >= config.max_actions:
            break
        unit_candidates = unit if isinstance(unit, list) else [unit]
        if unit_score(unit_candidates) <= config.roi_threshold:
            break
        if len(selected) + len(unit_candidates) > config.max_actions:
            continue

        target_ids = {cand.target_id for cand in unit_candidates if cand.kind != "regroup"}
        source_ids = {cand.source_id for cand in unit_candidates}
        if any(cand.ships > budgets.get(cand.source_id, 0) for cand in unit_candidates):
            continue
        if any(target_id in target_taken for target_id in target_ids):
            continue
        if any(source_id in defended_targets for source_id in source_ids):
            continue
        if any(cand.kind == "defense" and cand.target_id in used_sources for cand in unit_candidates):
            continue
        if len(source_ids) != len(unit_candidates):
            continue
...
def agent(obs):
    ...
    candidates.extend(
        build_opening_combo_candidates(
            sources,
            shortlisted_targets,
            planets,
            fleets,
