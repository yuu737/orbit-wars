...
class ProducerLiteConfig:
    ...
    # --- sample107: one-ply candidate reservation before normal greedy -------
    enable_one_ply_candidate_4p: bool = False
    one_ply_start: int = 55
    one_ply_turn_limit: int = 170
    one_ply_top_k: int = 18
    one_ply_window: int = 10
    one_ply_response_frac: float = 0.62
    one_ply_response_min_ships: float = 8.0
    one_ply_response_eta_cap: float = 13.0
    one_ply_hold_margin: float = 7.0
    one_ply_hold_bonus: float = 0.95
    one_ply_poison_penalty: float = 2.4
    one_ply_prod_bonus: float = 0.16
    one_ply_source_strip_penalty: float = 0.55
    one_ply_roi_threshold: float = 1.05
...
def _plan_one_ply_candidate_reserve(
    *,
    score: Tensor,
    cand_src: Tensor,
    cand_send: Tensor,
    ...
    device = obs.device
    dtype = obs.ships.dtype
    if int(player_count) < 4 or not bool(config.enable_one_ply_candidate_4p):
        return _empty_entries(device, dtype), source_budget, target_exists
    step = int(float(obs.step.reshape(-1)[0].item()))
    if step < int(config.one_ply_start) or step > int(config.one_ply_turn_limit):
        return _empty_entries(device, dtype), source_budget, target_exists
    if getattr(garrison_status, "arrivals_by_owner", None) is None:
        return _empty_entries(device, dtype), source_budget, target_exists
    if score.numel() == 0:
    ...
    P = int(obs.P)
    H = int(garrison_status.ships_by_step.shape[1]) - 1
    if P <= 0 or H <= 0:
        return _empty_entries(device, dtype), source_budget, target_exists
    pid = int(obs.player_id)
    L = int(cand_src.shape[1])

    safe_short = cand_tgt_short.clamp(0, max(target_exists.shape[0] - 1, 0))
    budget_at = source_budget[cand_src.clamp(0, max(P - 1, 0))]
    can_fund = ((cand_send <= budget_at) | ~cand_active).all(dim=-1)
    valid = torch.isfinite(score) & cand_active.any(dim=-1) & can_fund & target_exists[safe_short]
    if not bool(valid.any()):
        return _empty_entries(device, dtype), source_budget, target_exists

    masked = torch.where(valid, score, torch.full_like(score, float("-inf")))
    top_k = max(1, min(int(config.one_ply_top_k), int(score.numel())))
    top_vals, top_idx = torch.topk(masked, k=top_k)
    keep = torch.isfinite(top_vals)
    if not bool(keep.any()):
        return _empty_entries(device, dtype), source_budget, target_exists
    ...
    active = cand_active[top_idx]
    tgt = cand_tgt_slot[top_idx].clamp(0, max(P - 1, 0))
    send_total = torch.where(active, cand_send[top_idx], torch.zeros_like(cand_send[top_idx])).sum(dim=1)
    eta = torch.where(active, cand_eta[top_idx], torch.zeros_like(cand_eta[top_idx])).max(dim=1).values
    k_arr = eta.ceil().to(torch.long).clamp(min=1, max=H)

    owner_at = garrison_status.owner_by_step[tgt, k_arr].to(torch.long)
    ships_at = garrison_status.ships_by_step[tgt, k_arr].to(dtype)
    enemy_or_neutral = owner_at != pid
    arrival_margin = torch.where(enemy_or_neutral, send_total - ships_at - 1.0, send_total + ships_at)

    window = int(config.one_ply_window)
    end_k = (k_arr + window).clamp(max=H)
    steps_h = torch.arange(1, H + 1, device=device).view(1, H)
    in_window = (steps_h > k_arr.view(-1, 1)) & (steps_h <= end_k.view(-1, 1))
    arr = garrison_status.arrivals_by_owner[tgt, 1:, :].to(dtype)
    ...
    d0 = cache.cross_dist[0].to(dtype)
    response_frac = float(config.one_ply_response_frac)
    response_min = float(config.one_ply_response_min_ships)
    response_eta_cap = float(config.one_ply_response_eta_cap)
    per_player_response = torch.zeros(top_idx.shape[0], dtype=dtype, device=device)
    owner_abs = obs.owner_abs.to(torch.long)
    if response_frac > 0.0:
        for enemy_id in range(int(player_count)):
            if enemy_id == pid:
                continue
            e_mask = obs.alive & (owner_abs == int(enemy_id)) & (obs.ships >= response_min)
            if not bool(e_mask.any()):
                continue
            e_idx = torch.nonzero(e_mask, as_tuple=False).flatten()
            e_ships = obs.ships[e_idx].to(dtype)
            e_speed = fleet_speed(e_ships.clamp(min=1.0))
            e_dist = d0[
                e_idx.view(1, -1).expand(tgt.shape[0], -1),
                tgt.view(-1, 1).expand(-1, e_idx.shape[0]),
            ]
            e_eta = e_dist / e_speed.view(1, -1).clamp(min=1e-6)
            can_respond = e_eta <= response_eta_cap
            masked_eta = torch.where(can_respond, e_eta, torch.full_like(e_eta, float("inf")))
            best_src = torch.argmin(masked_eta, dim=1)
            best_eta = masked_eta.gather(1, best_src.unsqueeze(1)).squeeze(1)
            best_ships = e_ships[best_src] * response_frac
            valid_resp = torch.isfinite(best_eta) & (best_ships >= response_min)
            per_player_response = per_player_response + torch.where(
                valid_resp,
                best_ships,
                torch.zeros_like(best_ships),
            )

    prod_t = prod[tgt].to(dtype)
    future_hold = arrival_margin + my_window + prod_t * float(window) - enemy_window - per_player_response
    hold_good = future_hold > float(config.one_ply_hold_margin)
    hold_bad = future_hold < -float(config.one_ply_hold_margin)
    neutral_or_enemy = owner_abs[tgt] != pid

    source_after_min = torch.full((top_idx.shape[0],), float("inf"), dtype=dtype, device=device)
    reserve = float(config.coord_source_reserve_base)
    for l in range(L):
        src_l = cand_src[top_idx, l].clamp(0, max(P - 1, 0))
        active_l = active[:, l]
        after_l = source_budget[src_l].to(dtype) - cand_send[top_idx, l].to(dtype)
        source_after_min = torch.where(active_l, torch.minimum(source_after_min, after_l), source_after_min)
    source_stripped = source_after_min < reserve

    local = score[top_idx].clone()
    local = local + torch.where(
        hold_good & neutral_or_enemy,
        torch.full_like(local, float(config.one_ply_hold_bonus)),
        torch.zeros_like(local),
    )
    local = local + prod_t * float(config.one_ply_prod_bonus)
    local = local - torch.where(
        hold_bad & neutral_or_enemy,
        torch.full_like(local, float(config.one_ply_poison_penalty)),
        torch.zeros_like(local),
    )
    local = local - torch.where(
        source_stripped,
        torch.full_like(local, float(config.one_ply_source_strip_penalty)),
        torch.zeros_like(local),
    )

    one_ply_score = torch.full_like(score, float("-inf"))
    commit_ok = (
        neutral_or_enemy
        & hold_good
        & (~source_stripped)
        & (prod_t >= 2.0)
        & (future_hold >= (float(config.one_ply_hold_margin) + 4.0))
    )
    one_ply_score[top_idx] = torch.where(commit_ok, local, torch.full_like(local, float("-inf")))
    entries, next_budget = _greedy_select(
        P=P, W=1, device=device, dtype=dtype, score=one_ply_score,
        cand_src=cand_src, cand_send=cand_send, cand_angle=cand_angle, cand_eta=cand_eta,
        cand_active=cand_active, cand_tgt_slot=cand_tgt_slot, cand_tgt_short=cand_tgt_short,
        cand_is_def=cand_is_def, source_budget=source_budget,
        target_exists=target_exists, roi_threshold=float(config.one_ply_roi_threshold),
    )
    if not bool(entries.valid.any()):
        return entries, source_budget, target_exists

    taken = entries.target_slots[entries.valid].clamp(0, max(P - 1, 0))
    next_target_exists = target_exists
    if taken.numel() > 0:
        taken_mask = torch.zeros(P, dtype=torch.bool, device=device)
        taken_mask.scatter_(0, taken, True)
        short_to_slot = cand_tgt_slot.new_zeros(target_exists.shape[0]).clamp(0, max(P - 1, 0))
        if cand_tgt_short.numel() > 0:
            short_to_slot.scatter_(0, cand_tgt_short.clamp(0, max(target_exists.shape[0] - 1, 0)), cand_tgt_slot.clamp(0, max(P - 1, 0)))
        next_target_exists = target_exists & (~taken_mask[short_to_slot.clamp(0, max(P - 1, 0))])
    return entries, next_budget, next_target_exists
...
def plan_lite_waves(
    *,
    ...
    one_ply_entries = _empty_entries(device, dtype)
    ...
    one_ply_entries, source_budget, target_exists_for_normal = _plan_one_ply_candidate_reserve(
        score=score,
        cand_src=cand_src,
        cand_send=cand_send,
    ...
    if not bool(config.enable_regroup):
        return concat_launch_entries([reserve_entries, one_ply_entries, wave_entries, coord_entries])
    ...
    return concat_launch_entries([reserve_entries, one_ply_entries, wave_entries, coord_entries, regroup_entries])
...
CONFIG_4P = dataclasses.replace(
    ...
    enable_one_ply_candidate_4p=True,
)
...
CONFIG_4P_TOP_DIRECTOR = dataclasses.replace(
    ...
    enable_coord_followup_4p=False,
)
