...
class ProducerLiteConfig:
    ...
    # --- sample153: 2P durable-capture ownership filter --------------------
    enable_durable_capture_2p: bool = False
    durable_hold_window: int = 7
    durable_margin: float = 2.0
    durable_enemy_frac: float = 0.70
    durable_latent_top_k: int = 2
    durable_start_turn: int = 0
    durable_turn_limit: int = 420
...
def _apply_durable_capture_filter_2p(
    *,
    score: Tensor,
    cand_send: Tensor,
    cand_eta: Tensor,
    ...
    """Drop 2P captures that are likely to churn back immediately.

    The older hard-bad one-ply experiments looked mainly at ship net. This filter
    instead asks whether a captured planet can remain ours for a short hold window.
    It is intentionally a hard delete only: durable captures keep their original
    score, defense/regroup candidates are untouched, and 4P never enters here.
    """
    if int(player_count) >= 4 or not bool(config.enable_durable_capture_2p):
        return score
    step = _obs_step(obs_tensors)
    if step < int(config.durable_start_turn) or step > int(config.durable_turn_limit):
        return score
    active = cand_active.any(dim=-1) & torch.isfinite(score)
    if not bool(active.any()):
        return score
    ...
    P = int(obs.P)
    K = int(floor.shape[-1]) if floor.ndim >= 2 else 0
    if P <= 0 or K <= 0:
        return score
    ...
    dtype = score.dtype
    pid = int(obs.player_id)
    hold = max(1, int(config.durable_hold_window))

    tgt = cand_tgt_slot.clamp(0, max(P - 1, 0)).to(torch.long)
    short = cand_tgt_short.clamp(0, max(int(floor.shape[0]) - 1, 0)).to(torch.long)
    send = cand_send[:, 0].to(dtype)
    eta = cand_eta[:, 0].to(dtype).clamp(min=1.0)
    k_arr = (torch.ceil(eta).to(torch.long) - 1).clamp(min=0, max=K - 1)
    floor_at_eta = floor[short, k_arr].to(dtype)

    # Only enemy-planet captures are filtered. This late/soft variant leaves
    # neutral growth intact and avoids changing symmetric opening races.
    target_not_mine = (~cand_is_def.to(torch.bool)) & obs.alive[tgt]
    capture = active & target_not_mine & obs.is_enemy[tgt] & (send >= floor_at_eta)
    if not bool(capture.any()):
        return score

    post_cap = (send - floor_at_eta).clamp(min=0.0)
    defense_capacity = post_cap + prod[tgt].to(dtype) * float(hold)

    arrivals = getattr(garrison_status, "arrivals_by_owner", None)
    H_arr = int(arrivals.shape[-2]) - 1 if arrivals is not None else 0
    enemy_window = torch.zeros_like(score)
    my_window = torch.zeros_like(score)
    if arrivals is not None and H_arr > 0:
        end_k = (torch.ceil(eta).to(torch.long) + hold).clamp(min=1, max=H_arr)
        start_k = torch.ceil(eta).to(torch.long).clamp(min=1, max=H_arr)
        steps_h = torch.arange(1, H_arr + 1, device=device).view(1, H_arr)
        window = (steps_h > start_k.view(-1, 1)) & (steps_h <= end_k.view(-1, 1))
        arr = arrivals[tgt, 1:, :].to(dtype)
        owner_ids = torch.arange(int(arr.shape[-1]), device=device).view(1, 1, -1)
        enemy_arr = torch.where(owner_ids != pid, arr, torch.zeros_like(arr)).sum(dim=2)
        enemy_window = torch.where(window, enemy_arr, torch.zeros_like(enemy_arr)).sum(dim=1)
        if 0 <= pid < int(arr.shape[-1]):
            my_arr = arr[:, :, pid]
            my_window = torch.where(window, my_arr, torch.zeros_like(my_arr)).sum(dim=1)

    d0 = cache.cross_dist[0].to(dtype)
    owner = obs.owner_abs.to(torch.long)
    enemy_planets = obs.alive & (owner >= 0) & (owner != pid)
    enemy_idx = torch.nonzero(enemy_planets, as_tuple=False).flatten()
    latent_enemy = torch.zeros_like(score)
    if int(enemy_idx.numel()) > 0:
        enemy_ships = obs.ships.to(dtype)[enemy_idx] * float(config.durable_enemy_frac)
        enemy_speed = fleet_speed(obs.ships.to(dtype)[enemy_idx].clamp(min=1.0)).clamp(min=1e-6)
        enemy_eta = d0[enemy_idx][:, tgt].to(dtype) / enemy_speed.view(-1, 1)
        response_horizon = eta.view(1, -1) + float(hold)
        reachable = (enemy_eta <= response_horizon) & (enemy_idx.view(-1, 1) != tgt.view(1, -1))
        # Closer reachable garrisons are more credible recapture threats. Sum only
        # the top-k latent threats so this remains a filter, not a turtle switch.
        slack = (response_horizon - enemy_eta).clamp(min=0.0)
        decay = (0.35 + 0.65 * (slack / float(hold + 1))).clamp(min=0.35, max=1.0)
        latent_matrix = torch.where(
            reachable,
            enemy_ships.view(-1, 1) * decay,
            torch.zeros_like(enemy_eta),
        )
        top_k = max(1, min(int(config.durable_latent_top_k), int(enemy_idx.numel())))
        latent_enemy = torch.topk(latent_matrix, k=top_k, dim=0).values.sum(dim=0)

    future_hold = defense_capacity + my_window - enemy_window - latent_enemy
    bad_capture = capture & (future_hold < float(config.durable_margin))
    if not bool(bad_capture.any()):
        return score
    return torch.where(bad_capture, torch.full_like(score, float("-inf")), score)
...
def plan_lite_waves(
    *,
    ...
    score = _apply_durable_capture_filter_2p(
        score=score,
        cand_send=cand_send,
        cand_eta=cand_eta,
        cand_tgt_slot=cand_tgt_slot,
...
# sample153: bake the 2P durable-capture filter into code. The local/Kaggle
# raw-agent path may execute main.py without __file__, which makes params.json
# unreliable; code defaults are the only dependable activation point.
_CONFIG_2P_BASE = dataclasses.replace(
    ProducerLiteConfig(),
    enable_durable_capture_2p=True,
    durable_hold_window=6,
    durable_margin=1.0,
    durable_enemy_frac=0.55,
    durable_latent_top_k=1,
    durable_start_turn=35,
