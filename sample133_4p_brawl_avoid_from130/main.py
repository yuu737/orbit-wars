...
class ProducerLiteConfig:
    ...
    # --- sample133: 4P brawl avoidance ---------------------------------------
    # Diagnosis of early-death losses: they are 4P mutual-destruction brawls where
    # three of four players are wiped to score 0 and one survivor wins small. Do not
    # feed our ships into an enemy planet that a THIRD-party enemy is already
    # attacking -- let the rivals grind each other down and conserve our force for
    # survival. Hard constraint (forbid such captures), narrowly targeted (only enemy
    # targets under third-party attack), so it does not cause broad passivity.
    enable_brawl_avoid_4p: bool = False
    brawl_avoid_window: int = 6
    brawl_min_third_party: float = 8.0
    brawl_avoid_start: int = 0
    brawl_avoid_limit: int = 160
...
def _apply_brawl_avoidance(
    *,
    ...
    """Forbid offensive captures of an enemy planet that a THIRD-party enemy is
    already attacking, so rival players grind each other down instead of us feeding
    a 4P mutual-destruction brawl. Hard ``-inf`` mask on those candidates only;
    neutral targets and enemy targets nobody else is attacking are untouched."""
    if int(player_count) < 4 or not bool(config.enable_brawl_avoid_4p):
        return score
    step = _obs_step(obs_tensors)
    if step < int(config.brawl_avoid_start) or step > int(config.brawl_avoid_limit):
        return score
    arrivals = getattr(garrison_status, "arrivals_by_owner", None)
    if arrivals is None:
        return score
    ...
    if H <= 0:
        return score
    W = max(1, min(int(config.brawl_avoid_window), H))

    tgt = cand_tgt_slot.clamp(0, max(P - 1, 0))           # [C]
    ...
    end_k = (k_arr + W).clamp(max=H)
    win = steps <= end_k.view(-1, 1)                      # [C, H] now .. arrival+window

    arr_t = arrivals[tgt, 1:, :].to(score.dtype)          # [C, H, A]
    A = int(arr_t.shape[-1])
    per_owner = (arr_t * win.unsqueeze(-1)).sum(dim=1)    # [C, A] ships each owner lands on the target
    owner_ids = torch.arange(A, device=device).view(1, A)
    tgt_owner = obs.owner_abs.to(torch.long)[tgt].view(-1, 1)  # [C, 1]
    third_party = (owner_ids != pid) & (owner_ids != tgt_owner)
    third_force = torch.where(third_party, per_owner, torch.zeros_like(per_owner)).sum(dim=1)  # [C]

    contested = enemy_tgt & (third_force >= float(config.brawl_min_third_party))
    return torch.where(contested, torch.full_like(score, float("-inf")), score)
...
CONFIG_4P = dataclasses.replace(
    ...
    # sample133: 4P brawl avoidance (do not feed enemies a third party is attacking).
    enable_brawl_avoid_4p=True,
    brawl_avoid_window=6,
    brawl_min_third_party=8.0,
    brawl_avoid_start=0,
    brawl_avoid_limit=160,
)
