...
class ProducerLiteConfig:
    ...
    # Extra source reserve used only by contested-rich 4P terrain mode.
    source_reserve_ships: float = 0.0
    source_reserve_prod_mult: float = 0.0
...
def plan_lite_waves(
    *,
    ...
    if float(config.source_reserve_ships) > 0.0 or float(config.source_reserve_prod_mult) > 0.0:
        source_prod = prod[source_idx.clamp(0, P - 1)].to(dtype)
        reserve = (
            float(config.source_reserve_ships)
            + float(config.source_reserve_prod_mult) * source_prod
        )
        drain = (drain - reserve).clamp(min=0.0)
...
CONFIG_4P_CONTESTED = dataclasses.replace(
    CONFIG_4P,
    horizon=14,
    ...
    reinforce_size_beta=2.8,
    reinforce_eta_free=2.0,
    reinforce_eta_scale=10.0,
    enable_regroup=True,
    max_regroup_time=5.0,
    regroup_pressure_delta_min=0.3,
    max_regroup_sources_per_lane=5,
    max_regroup_targets_per_source=6,
    terminal_phase_turns=0,
    size_multipliers=(0.55, 0.8, 1.0),
    source_reserve_ships=5.0,
    source_reserve_prod_mult=4.0,
)
