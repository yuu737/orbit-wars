...
class PlanetGarrisonStatus:
    ...
    before* combat resolution at each future step  Eafter that step's production
...
class PlanetMovement:
    ...
    def stash_pending_own_launches(
        ...
        *,
        ...
        seed the ``tracked_fleet_ids`` ledger yet  Ethe engine assigns IDs in
        ...
        # Clear any prior pending entries for this owner  Ea repeat stash for the
        ...
        # slots (ascending)  Epreserving emission order.
    ...
    def _reconcile_pending_own_launches(self, obs_tensors: dict) -> None:
        ...
        entries are treated as vanished mid-flight  Ethe engine can destroy
        ...
        source and the predicted target)  Eand we undo the bucket-arrival
    ...
    def _fill_garrison_trajectory(
        ...
        *,
        ...
        - **Half B** (sequential, branchless): walk ``k = 1..H`` advancing
          ``(state_owner, state_ships)``. Every operation is a fused ``where``  E          there is no host sync (no ``bool(has_arrivals.any())``), no boolean
        ...
        # match fit this profile, so this is the big algorithmic win  Ethese
        ...
        # Map each step k ∁E[1, H] to itself if there's an event there, else 0.
        ...
        if loop_iters > 0:
            ...
            # pair has a real cross-step dependency  Ean attacker capturing the planet
            # at step k flips who produces in subsequent steps  Eso we must walk
    ...
    def _ledger_bulk_insert(
        ...
        # Place the rows into the first ``n`` empty ledger slots, ascending  E        # which preserves input order (each row keeps its emission rank).
    ...
    def _reconcile_obs_fleets(self, obs_tensors: dict) -> None:
        ...
        knows to evict it  Eingest only adds, decrement only fires at eta=0.
...
def _estimate_new_fleet_arrivals(
    *,
    ...
    # turn ρE< t. ``cummax`` along the time axis gives the inclusive OR;
