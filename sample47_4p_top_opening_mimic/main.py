...
class ProducerLiteConfig:
    ...
    domain_factory_claim_turn: int = 64
    domain_factory_claim_bonus: float = 0.46
    domain_factory_claim_prod_bonus: float = 0.035
    ...
    domain_factory_regroup_core_bonus: float = 0.0
    domain_factory_regroup_anchor_bonus: float = 0.0
    domain_factory_regroup_expansion_bonus: float = 0.0
...
def _make_domain_factory_plan(
    *,
    ...
    best: tuple[float, int, list[int], list[int], list[int], list[int], float] | None = None
    for anchor in anchor_scan:
        ...
        for j in neutral_idx:
            ...
            if lane_delta <= float(config.domain_factory_angle_width) + 0.34:
                # Top openings first claim a connected local lane. These are not
                # always the final anchors, but they prevent the bot from jumping
                # straight to a far prize and leaving no factory behind.
                if d_base <= 76.0 and (d_base <= dist_from_base + 20.0 or d_anchor <= 72.0):
                    if jp >= 1.0 or js <= 38.0:
                        claim.append(j)
        ...
        claim_value = sum(
            float(prod[j].item()) * 1.35
            + max(0.0, 34.0 - float(obs.ships[j].item())) * 0.035
            + max(0.0, 60.0 - _dist_xy(bx, by, float(x[j].item()), float(y[j].item()))) * 0.018
            for j in claim
        )
        ...
        if best is None or score > best[0]:
            best = (score, anchor, claim, connectors, supports, expansions, lane_angle)

    if best is None or best[0] < (24.0 if preferred_anchor in neutral_idx else 32.0):
        # If no polished high-value anchor is visible, still commit to a
        # concrete lane. Top 4P openings do not wait for a perfect anchor:
        # they first connect cheap/mid planets into an outer factory, then
        # thicken the route. Returning None here made the domain mode stall.
        fallback: list[int] = []
        for j in neutral_idx:
            jx = float(x[j].item())
            jy = float(y[j].item())
            ...
            if d_base > 88.0:
                continue
            if _angle_diff(base_angle, ja) > 2.15:
                continue
            if jr < 18.0:
                continue
            if jp < 1.0 and js > 42.0:
                continue
            fallback.append(j)
        if not fallback:
            return None
        fallback_sorted = sorted(
            ...
            key=lambda j: (
                ...
                -float(prod[j].item()),
                float(obs.ships[j].item()),
            ),
        )[:11]
        anchor = max(
            fallback_sorted,
            key=lambda j: (
                float(prod[j].item()) * 11.0
                + float(radius[j].item()) * 0.09
                + max(0.0, 44.0 - float(obs.ships[j].item())) * 0.035
                - _dist_xy(bx, by, float(x[j].item()), float(y[j].item())) * 0.08
            ),
        )
        lane_angle = float(angle[anchor].item())
        supports_sorted = [
            int(j)
            for j in fallback_sorted
            if int(j) != int(anchor)
            and _dist_xy(float(x[j].item()), float(y[j].item()), float(x[anchor].item()), float(y[anchor].item())) <= 62.0
        ][:6]
        expansions_sorted = sorted(
            [
                int(j)
                for j in neutral_idx
                if int(j) not in set(fallback_sorted)
                and _angle_diff(lane_angle, float(angle[j].item())) <= 2.10
                and float(radius[j].item()) >= float(radius[anchor].item()) - 8.0
                and _dist_xy(float(x[j].item()), float(y[j].item()), float(x[anchor].item()), float(y[anchor].item())) <= 94.0
                and (float(prod[j].item()) >= 3.0 or float(obs.ships[j].item()) >= 45.0)
            ],
            key=lambda j: float(prod[j].item()) * 10.0 + float(obs.ships[j].item()) * 0.07 + float(radius[j].item()) * 0.05,
            reverse=True,
        )[:5]
        core = list(dict.fromkeys([int(j) for j in fallback_sorted] + [int(anchor)] + supports_sorted))
        return {
            "mode": "domain_factory",
            "anchor": int(anchor),
            "core": core,
            "claim": [int(j) for j in fallback_sorted],
            "bridge": core,
            "support": supports_sorted,
            "connector": [int(j) for j in fallback_sorted[:5]],
            "expansion": expansions_sorted,
            "targets": core + [int(j) for j in expansions_sorted if int(j) not in set(core)],
            "angle": float(lane_angle),
            "created_step": _obs_step(obs_tensors),
            "score": 28.0,
            "fallback": True,
    ...
    score, anchor, claim, connectors, supports, expansions, lane_angle = best
    claim_sorted = sorted(
        claim,
        key=lambda j: (
            _dist_xy(bx, by, float(x[j].item()), float(y[j].item())),
    ...
    bridge_sorted = sorted(
        list(dict.fromkeys([int(t) for t in claim_sorted] + [int(t) for t in connectors_sorted] + [int(t) for t in supports_sorted])),
        key=lambda j: (
            _dist_xy(bx, by, float(x[j].item()), float(y[j].item())),
            -float(prod[j].item()),
        ),
    )
    core = [int(t) for t in bridge_sorted]
    core.append(int(anchor))
    ...
    return {
        ...
        "anchor": int(anchor),
        "core": unique_core,
        "claim": [int(t) for t in claim_sorted],
        "bridge": [int(t) for t in bridge_sorted],
...
def _append_domain_factory_targets(
    *,
    ...
    claim_targets = [int(t) for t in plan.get("claim", [])]
    core_targets = [int(t) for t in plan.get("core", [])]
    bridge_targets = [int(t) for t in plan.get("bridge", [])]
    anchor_target = [int(plan.get("anchor", -1))]
    expansion_targets = [int(t) for t in plan.get("expansion", [])]
    if step < int(config.domain_factory_claim_turn):
        plan_targets = claim_targets + bridge_targets[:6] + anchor_target
...
def _apply_domain_factory_adjustment(
    *,
    ...
    bridge_mask = torch.zeros(P, dtype=torch.bool, device=score.device)
    claim_mask = torch.zeros(P, dtype=torch.bool, device=score.device)
    for t in domain_plan.get("claim", []):
        if 0 <= int(t) < P:
    ...
    for t in domain_plan.get("bridge", []):
        if 0 <= int(t) < P:
            bridge_mask[int(t)] = True
    ...
    in_claim_phase = step < int(config.domain_factory_claim_turn)
    selected_claim = claim_mask[tgt] & active & in_claim_phase
    selected_core = core_mask[tgt] & active
    selected_support = support_mask[tgt] & active
    selected_bridge = bridge_mask[tgt] & active & in_claim_phase
    ...
    target_bonus = target_bonus + torch.where(
        selected_claim,
        torch.full_like(score, float(config.domain_factory_claim_bonus)),
        torch.zeros_like(score),
    )
    target_bonus = target_bonus + torch.where(
        selected_core,
    ...
    target_bonus = target_bonus + torch.where(
        selected_bridge,
        torch.full_like(score, float(config.domain_factory_core_bonus) * 0.42),
        torch.zeros_like(score),
    )
    target_bonus = target_bonus + torch.where(
        selected_expansion,
    ...
    selected_any = selected_claim | selected_core | selected_support | selected_bridge | selected_expansion
    target_bonus = target_bonus + torch.where(
        selected_any,
        (
            prod[tgt].to(score.dtype)
            * torch.where(
                selected_claim,
                torch.full_like(score, float(config.domain_factory_claim_prod_bonus)),
                torch.full_like(score, float(config.domain_factory_prod_bonus)),
            )
        ).clamp(max=0.42)
    ...
    if in_claim_phase:
        drain_penalty = torch.zeros_like(score)
...
def _domain_factory_regroup_pressure(
    *,
    pressure: Tensor,
    obs,
    obs_tensors: dict,
    ...
    boosted = pressure.clone()
    P = int(prod.shape[0])
    anchor = int(domain_plan.get("anchor", -1))
    project_ok = _domain_factory_mature(obs=obs, prod=prod, config=config, plan=domain_plan, step=step) or step >= int(config.domain_factory_project_turn)

    def add_boost(t: int, amount: float) -> None:
        if 0 <= int(t) < P and bool(obs.alive[int(t)].item()) and bool(obs.owned[int(t)].item()):
            boosted[int(t)] = boosted[int(t)] + float(amount) + float(prod[int(t)].item()) * 2.4

    for t in domain_plan.get("core", []):
        add_boost(int(t), float(config.domain_factory_regroup_core_bonus))
    for t in domain_plan.get("support", []):
        add_boost(int(t), float(config.domain_factory_regroup_core_bonus) * 0.65)
    if 0 <= anchor < P:
        add_boost(anchor, float(config.domain_factory_regroup_anchor_bonus))
    if project_ok:
        for t in domain_plan.get("expansion", []):
            add_boost(int(t), float(config.domain_factory_regroup_expansion_bonus))
    return boosted


def _domain_factory_opening_entries(
    *,
    ...
    """Explicit top-style opening: claim connected lane planets before holding.

    This intentionally bypasses the ROI scorer for the first phase. The strategic
    goal is to create the same visible pattern as top replays: one connected
    outer lane first, factory thickness later.
    """
    device = obs.device
    dtype = obs.ships.dtype
    if int(player_count) < 4 or not bool(config.enable_domain_factory_4p) or not domain_plan:
    ...
    P = int(obs.P)
    owned = torch.nonzero(obs.owned & obs.alive, as_tuple=False).flatten()
    if owned.numel() <= 0:
        return _empty_entries(device, dtype)

    target_order = (
        [int(t) for t in domain_plan.get("claim", [])]
        + [int(t) for t in domain_plan.get("bridge", [])]
        + [int(domain_plan.get("anchor", -1))]
    )
    seen: set[int] = set()
    targets: list[int] = []
    for t in target_order:
        if 0 <= int(t) < P and int(t) not in seen and bool(obs.alive[int(t)].item()):
            if not bool(obs.owned[int(t)].item()):
                targets.append(int(t))
                seen.add(int(t))
    if not targets:
        return _empty_entries(device, dtype)

    src_scores = obs.ships[owned].to(dtype) + prod[owned].to(dtype) * 8.0
    src_order = owned[torch.argsort(src_scores, descending=True, stable=True)].tolist()

    launch_src: list[int] = []
    launch_tgt: list[int] = []
    launch_ships: list[float] = []
    launch_angle: list[float] = []
    launch_eta: list[float] = []

    reserved_sources: set[int] = set()
    reserved_targets: set[int] = set()
    max_orders = 2 if step >= 18 else 1
    for src in src_order:
        src = int(src)
        if src in reserved_sources:
            continue
        src_ships = float(obs.ships[src].item())
        src_prod = float(prod[src].item())
        reserve = max(8.0, src_prod * 2.0 + 5.0)
        spendable = math.floor(src_ships - reserve)
        if spendable < float(config.min_ships_to_launch):
            continue
        for tgt in targets:
            if tgt in reserved_targets or tgt == src:
                continue
            src_t = torch.tensor([src], dtype=torch.long, device=device)
            tgt_t = torch.tensor([tgt], dtype=torch.long, device=device)
            probe_send = max(float(config.min_ships_to_launch), min(float(spendable), float(obs.ships[tgt].item()) + 2.0))
            size_t = torch.tensor([probe_send], dtype=dtype, device=device)
            aim = intercept_angle(
                movement,
                src_t,
                tgt_t,
            ...
            if not bool(aim["viable"][0].item()):
                continue
            eta = float(aim["eta"][0].item())
            # Capture at arrival, not at launch. The previous version sent
            # only current garrison + 2, which produced pretty arrows that
            # dissolved into growing neutral planets.
            need = math.ceil(float(obs.ships[tgt].item()) + float(prod[tgt].item()) * max(0.0, eta) + 3.0)
            if step < 36 and need > spendable:
                continue
            send = min(float(spendable), max(float(config.min_ships_to_launch), float(need)))
            if send < need and step < 54:
                continue
            if abs(send - probe_send) > 1e-6:
                size_t = torch.tensor([send], dtype=dtype, device=device)
                aim = intercept_angle(
                    movement,
                    src_t,
                    tgt_t,
                ...
                if not bool(aim["viable"][0].item()):
                    continue
                eta = float(aim["eta"][0].item())
            launch_src.append(src)
            launch_tgt.append(tgt)
            launch_ships.append(send)
            launch_angle.append(float(aim["angle"][0].item()))
            launch_eta.append(eta)
            reserved_sources.add(src)
            reserved_targets.add(tgt)
            break
        if len(launch_src) >= max_orders:
            break

    if not launch_src:
        return _empty_entries(device, dtype)
    return LaunchEntries(
        source_slots=torch.tensor(launch_src, dtype=torch.long, device=device),
        target_slots=torch.tensor(launch_tgt, dtype=torch.long, device=device),
        ships=torch.tensor(launch_ships, dtype=dtype, device=device),
        angle=torch.tensor(launch_angle, dtype=dtype, device=device),
        eta=torch.tensor(launch_eta, dtype=dtype, device=device),
        valid=torch.ones(len(launch_src), dtype=torch.bool, device=device),
    )
...
def plan_lite_waves(
    *,
    ...
    opening_entries = _domain_factory_opening_entries(
        movement=movement,
        obs=obs,
        obs_tensors=obs_tensors,
        prod=prod,
    ...
    if bool(opening_entries.valid.any()):
    ...
    enemy_mass = _domain_factory_regroup_pressure(
        pressure=enemy_mass,
        obs=obs,
        obs_tensors=obs_tensors,
        prod=prod,
...
CONFIG_4P_DOMAIN_FACTORY = dataclasses.replace(
    ...
    size_multipliers=(1.0,),
    enable_domain_factory_4p=True,
    domain_factory_turn_limit=245,
    domain_factory_project_turn=92,
    domain_factory_max_extra_targets=18,
    domain_factory_angle_width=2.05,
    domain_factory_claim_turn=66,
    domain_factory_claim_bonus=0.64,
    domain_factory_claim_prod_bonus=0.052,
    domain_factory_core_bonus=0.42,
    domain_factory_support_bonus=0.24,
    domain_factory_expansion_bonus=0.48,
    domain_factory_prod_bonus=0.045,
    domain_factory_outer_bonus=0.14,
    domain_factory_offdomain_cheap_penalty=0.0,
    domain_factory_hold_base=18.0,
    domain_factory_hold_prod=4.2,
    domain_factory_support_hold_base=8.0,
    domain_factory_support_hold_prod=2.2,
    domain_factory_drain_penalty=0.08,
    domain_factory_regroup_core_bonus=8.0,
    domain_factory_regroup_anchor_bonus=15.0,
    domain_factory_regroup_expansion_bonus=7.0,
)
...
def _choose_4p_mode(obs_tensors: dict) -> str:
    ...
    # sample46 is intentionally not a conservative selector.
    # In 4P we want to complete a first-place outer factory strategy:
    # claim one lane, connect support planets, thicken the domain, then project.
    if (
        enemy_dist >= 38.0
        and (
            outer_anchor >= 34.0
