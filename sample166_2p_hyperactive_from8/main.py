...
# sample166: HYPER-ACTIVE 2P. Competition shadow (replays_my_2p_163) showed winners launch
# ~2x more and grab 2-3x more high-prod neutrals than even sample161. Push every activity
# knob: lower ROI gate, more waves, more sources, wider target shortlist, finer size tiers
# (more, smaller launches like the winners). Baked in CODE (params.json ignored at eval).
_CONFIG_2P_BASE = dataclasses.replace(
    ProducerLiteConfig(),
    roi_threshold=0.5,
    max_waves_per_turn=18,
    max_sources_per_lane=18,
    max_offensive_targets=18,
    size_multipliers=(0.34, 0.5, 0.75, 1.0),
)
