...
def _agent_h5() -> ModuleType:
    global _MOD_H5
    if _MOD_H5 is None:
        bots_folder = os.path.join(os.path.dirname(_HERE), "bots")
        main_path = os.path.join(bots_folder, "hairate5.py")
        old_path = list(sys.path)
        try:
            _clear_orbit_lite_modules()
            sys.path.insert(0, bots_folder)
            spec = importlib.util.spec_from_file_location("_sample73_h5_external", main_path)
            if spec is None or spec.loader is None:
                raise ImportError(f"Could not load {main_path}")
            module = importlib.util.module_from_spec(spec)
            sys.modules["_sample73_h5_external"] = module
            spec.loader.exec_module(module)
            _MOD_H5 = module
        finally:
            sys.path[:] = old_path
