...
class ProducerLiteConfig:
    ...
    # sample137: supply analysis. The in-flight reserve above only sees enemy fleets
    # already launched. Add a LATENT reachable-enemy term: the strongest single enemy
    # planet that could send a fleet reaching an owned planet within the horizon
    # (computed from current pairwise distance and enemy fleet speed). Combined with
    # the in-flight term via MAX (combat-aware: one enemy, not the sum), so a planet
    # is not stripped when a strong enemy is adjacent even if no fleet is yet in
    # flight -- the demonstrated source-stripping death cause. 0 = off.
    supply_latent_frac: float = 0.0
    supply_latent_horizon: float = 16.0
...
def _threat_reserve_source_budget(
    *,
    ...
    # sample137: latent reachable-enemy threat. The strongest single enemy planet that
    # could launch now and reach planet p within supply_latent_horizon, sending a
    # fraction of its garrison. Combined with the in-flight term via MAX so we reserve
    # against whichever is larger, without summing (which would over-reserve).
    latent_frac = float(config.supply_latent_frac)
    if latent_frac > 0.0 and cache is not None:
        d0 = cache.cross_dist[0].to(dtype)                         # [P(from), P(to)]
        enemy_mask = obs.alive & (obs.owner_abs >= 0) & (obs.owner_abs != pid)  # [P]
        enemy_send = (obs.ships.to(dtype) * latent_frac).clamp(min=0.0)         # [P] sendable
        enemy_speed = fleet_speed(enemy_send.clamp(min=1.0))                    # [P]
        reach_time = d0 / enemy_speed.view(P, 1).clamp(min=1e-6)                # [e, p]
        can_reach = enemy_mask.view(P, 1) & (reach_time <= float(config.supply_latent_horizon))
        threat_mat = torch.where(can_reach, enemy_send.view(P, 1).expand(P, P), torch.zeros_like(d0))
        latent_threat = threat_mat.amax(dim=0)                    # [p] strongest single reachable enemy
        threat = torch.maximum(enemy_arr, latent_threat)
    else:
        threat = enemy_arr
    ...
    reserve_need = threat + margin - my_arr - prod_buffer
    owned = obs.owned & obs.alive
    apply_mask = owned & (threat > 0.0) & (reserve_need > 0.0) & (reserve_need <= ships)
...
CONFIG_4P = dataclasses.replace(
    ...
    # sample137: supply analysis -- reserve also against the strongest latent reachable enemy.
    supply_latent_frac=0.5,
    supply_latent_horizon=16.0,
)
