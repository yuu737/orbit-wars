...
class ProducerLiteConfig:
    ...
    response_search_enabled: bool = False
    response_search_turns: int = 220
    response_search_top_my: int = 40
    response_search_enemy_sources: int = 5
    response_search_enemy_targets: int = 7
    response_search_enemy_top: int = 12
    response_search_enemy_threshold: float = 1.1
    response_search_penalty_weight: float = 0.50
    response_search_max_penalty: float = 0.60
...
def _empty_response(device, dtype) -> tuple[Tensor, Tensor, Tensor, Tensor, Tensor]:
...
def _build_enemy_response_candidates(
    *,
    ...
    P = int(obs.P)
    if P <= 0:
        return _empty_response(device, dtype)
...
def _apply_response_search(
    *,
    ...
    e_src, e_tgt, e_send, _e_angle, e_eta = _build_enemy_response_candidates(
        movement=movement,
        obs=obs,
    ...
    # Only punish replies that interact with this candidate. Independent enemy
    # good moves happen regardless, so treating them as candidate-specific risk
    # makes the bot abandon strong sample8 attacks.
    my_target = cand_tgt_slot[top_my].view(M, 1)
    my_source = cand_src[top_my, 0].view(M, 1)
    enemy_target = e_tgt.view(1, E)
    related_reply = (enemy_target == my_target) | (enemy_target == my_source)
    masked_reply = torch.where(
        related_reply,
        reply_score,
        torch.full_like(reply_score, float("inf")),
    )
    has_related = related_reply.any(dim=1)
    worst_reply = masked_reply.min(dim=1).values
    original = score[top_my]
    downward = torch.minimum(torch.zeros_like(original), worst_reply - original)
    downward = torch.where(has_related, downward, torch.zeros_like(downward))
    downward = downward.clamp(min=-float(config.response_search_max_penalty))
...
def _ow_apply_config_overrides(config: ProducerLiteConfig, values: dict | None) -> ProducerLiteConfig:
    ...
    for key, value in values.items():
        if key not in allowed:
            continue
        if key in ("size_multipliers", "response_search_size_multipliers"):
