...
def _append_enemy_domain_block_targets(
    *,
    ...
    min_leader_threat = 10.0 if int(config.enemy_block_start_turn) <= 20 else 18.0
    if float(threat_by_owner[leader].item()) < float(min_leader_threat):
        return target_idx, target_exists, None
    ...
    useful = (prod >= (1.5 if int(config.enemy_block_start_turn) <= 20 else 2.0)) | (obs.ships >= 24.0)
    reachable = eta_from_me <= float(config.enemy_block_eta_cap)
    leader_can_contest = eta_from_leader <= (eta_from_me + 8.0)
    target_mask = (
        obs.is_neutral
        & obs.alive
        & (radius >= (24.0 if int(config.enemy_block_start_turn) <= 20 else 26.0))
        & near_leader_outer
        & useful
        & reachable
        & leader_can_contest
...
CONFIG_4P_EARLY_ENEMY_DOMAIN_BLOCK = dataclasses.replace(
    CONFIG_4P_ENEMY_DOMAIN_BLOCK,
    enemy_block_start_turn=18,
    enemy_block_turn_limit=105,
    enemy_block_eta_cap=26.0,
    enemy_block_max_extra_targets=8,
    enemy_block_cluster_radius=48.0,
    enemy_block_bonus=0.78,
    enemy_block_prod_bonus=0.085,
    enemy_block_outer_bonus=0.28,
    enemy_block_direct_enemy_penalty=0.62,
    roi_threshold=1.38,
...
def _choose_4p_mode(obs_tensors: dict) -> str:
    ...
    if (
        ...
        return "early_enemy_domain_block"
    return "s7_stable"
...
def _config_for(player_count: int, mode: str | None = None) -> ProducerLiteConfig:
    ...
    if mode == "early_enemy_domain_block":
        return CONFIG_4P_EARLY_ENEMY_DOMAIN_BLOCK
