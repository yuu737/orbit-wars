...
class ProducerLiteConfig:
    ...
    # --- 4P opening connector ------------------------------------------
    enable_opening_connector_4p: bool = False
    connector_step_end: int = 75
    connector_max_extra_targets: int = 5
    connector_eta_cap: float = 16.0
    connector_center_x: float = 50.0
    connector_center_y: float = 50.0
    connector_min_progress: float = 4.0
    connector_bonus: float = 0.22
    connector_progress_bonus: float = 0.012
    connector_max_bonus: float = 0.58
...
def _append_4p_opening_connector_targets(
    *,
    target_idx: Tensor,
    ...
    """Expose early connector planets that move our outer anchor toward the centre.

    This is a route-generation layer, not a late-game score tweak. In 4P, first
    place often comes from building an outer anchor and then owning the connector
    planets that let it project force inward before another player snowballs.
    """
    if int(player_count) < 4 or not bool(config.enable_opening_connector_4p):
        return target_idx, target_exists, None
    if _obs_step(obs_tensors) > int(config.connector_step_end):
        return target_idx, target_exists, None

    P = int(obs.P)
    if P <= 0 or int(config.connector_max_extra_targets) <= 0:
        return target_idx, target_exists, None
    ...
    x = planets[:, 2]
    y = planets[:, 3]
    cx = torch.full((P,), float(config.connector_center_x), dtype=dtype, device=device)
    cy = torch.full((P,), float(config.connector_center_y), dtype=dtype, device=device)
    center_dist = torch.sqrt((x - cx).square() + (y - cy).square())
    ...
    reachable = eta_from_me <= float(config.connector_eta_cap)

    source_center = torch.where(source_mask, center_dist, torch.full_like(center_dist, float("inf")))
    nearest_source_center = torch.where(source_mask.view(P, 1), source_center.view(P, 1), torch.full((P, P), float("inf"), dtype=dtype, device=device)).amin(dim=0)
    progress = nearest_source_center - center_dist

    attackable = obs.alive & obs.is_neutral
    useful = (prod >= 2.0) | (obs.ships >= 20.0)
    connector_mask = attackable & reachable & useful & (progress >= float(config.connector_min_progress))
    if not bool(connector_mask.any()):
        return target_idx, target_exists, None

    all_connector_targets = torch.nonzero(connector_mask, as_tuple=False).flatten()
    ...
    connector_mask = connector_mask & ~existing
    if not bool(connector_mask.any()):
        return target_idx, target_exists, all_connector_targets

    score = (
        prod.to(dtype) * 5.0
        + obs.ships.to(dtype) * 0.035
        + progress.clamp(min=0.0) * 0.55
        - eta_from_me * 0.32
        - center_dist * 0.035
    )
    score = torch.where(connector_mask, score, torch.full_like(score, float("-inf")))
    extra_idx, extra_exists = _candidate_indices(
        score,
        torch.isfinite(score),
        max(1, min(int(config.connector_max_extra_targets), P)),
    )
    ...
    return torch.cat([target_idx, extras], dim=0), torch.cat([target_exists, extra_exists_full], dim=0), all_connector_targets


def _apply_4p_opening_connector_bonus(
    *,
    score: Tensor,
    cand_valid: Tensor,
    ...
    if int(player_count) < 4 or connector_targets is None or connector_targets.numel() == 0:
        return score
    if float(config.connector_bonus) <= 0.0:
        return score
    ...
    center_dist = torch.sqrt(
        (x - float(config.connector_center_x)).square()
        + (y - float(config.connector_center_y)).square()
    )

    target_mask = torch.zeros(P, dtype=torch.bool, device=score.device)
    target_mask[connector_targets.clamp(0, P - 1)] = True
    tgt = cand_tgt_slot.clamp(0, P - 1)
    selected = target_mask[tgt] & cand_valid
    central_bonus = ((50.0 - center_dist[tgt].to(dtype)).clamp(min=0.0) * float(config.connector_progress_bonus)).clamp(max=float(config.connector_max_bonus))
    bonus = torch.full_like(score, float(config.connector_bonus)) + central_bonus
...
def plan_lite_waves(
    *,
    ...
    target_idx, target_exists, connector_targets = _append_4p_opening_connector_targets(
    ...
    score = _apply_4p_opening_connector_bonus(
        score=score,
...
CONFIG_4P = replace(
    ...
    enable_opening_connector_4p=True,
    connector_step_end=75,
    connector_max_extra_targets=5,
    connector_eta_cap=16.0,
    connector_min_progress=4.0,
    connector_bonus=0.48,
    connector_progress_bonus=0.018,
    connector_max_bonus=0.95,
)
