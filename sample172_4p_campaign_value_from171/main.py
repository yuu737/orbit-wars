...
class ProducerLiteConfig:
    ...
    expansion_drive_hold_window: float = 8.0
    expansion_drive_hold_margin: float = 6.0
    expansion_drive_retake_penalty: float = 0.090
    expansion_campaign_radius: float = 18.0
    expansion_campaign_prod_bonus: float = 0.080
    expansion_campaign_cheap_bonus: float = 0.050
    expansion_campaign_risk_penalty: float = 0.010
    expansion_campaign_min_value: float = 1.5
    expansion_campaign_isolated_penalty: float = 0.35
...
def _apply_expansion_drive_4p(
    *,
    ...
    not_self = planet_idx != tgt.view(-1, 1)
    neighbor_neutral = (
    ...
    neighbor_cheap = torch.where(
        neighbor_neutral & (obs.ships.view(1, P).to(dtype) <= 26.0),
        neighbor_decay,
        torch.zeros((score.shape[0], P), dtype=dtype, device=device),
    ).sum(dim=1)
    ...
    campaign_value = neighbor_prod + neighbor_cheap * 0.5
    ...
    total_send = cand_send.to(dtype).sum(dim=-1)
    post_capture = (total_send - target_ships).clamp(min=0.0)
    hold_power = post_capture + target_prod * float(config.expansion_drive_hold_window)
    retake_excess = (
        pressure + float(config.expansion_drive_hold_margin) - hold_power
    ).clamp(min=0.0)
    ...
    neutral_bonus = (
        ...
        + neighbor_prod * float(config.expansion_campaign_prod_bonus) * (0.45 + need)
    ...
    adjust = adjust - torch.where(
        neutral_target & (campaign_value < float(config.expansion_campaign_min_value)) & (target_ships > 30.0),
        torch.full_like(score, float(config.expansion_campaign_isolated_penalty)),
        torch.zeros_like(score),
    )
    adjust = adjust - torch.where(
    ...
    adjust = adjust - torch.where(
        attack_target,
        retake_excess * float(config.expansion_drive_retake_penalty),
        torch.zeros_like(score),
    )
    return score + adjust
...
CONFIG_4P = dataclasses.replace(
    ...
    expansion_drive_limit=170,
    expansion_drive_low_planets=7,
    expansion_drive_base_bonus=0.55,
    expansion_drive_lag_bonus=1.15,
    expansion_drive_prod_bonus=0.20,
    expansion_drive_outer_bonus=0.42,
    expansion_drive_cheap_bonus=0.26,
    expansion_drive_enemy_penalty=0.40,
    expansion_drive_center_penalty=0.42,
    expansion_drive_pressure_penalty=0.015,
    expansion_drive_strip_penalty=0.85,
    expansion_drive_min_prod=1.0,
    expansion_drive_hold_window=8.0,
    expansion_drive_hold_margin=6.0,
    expansion_drive_retake_penalty=0.090,
    expansion_campaign_radius=18.0,
    expansion_campaign_prod_bonus=0.080,
    expansion_campaign_cheap_bonus=0.050,
    expansion_campaign_risk_penalty=0.010,
    expansion_campaign_min_value=1.5,
    expansion_campaign_isolated_penalty=0.35,
    coord_start_turn=18,
