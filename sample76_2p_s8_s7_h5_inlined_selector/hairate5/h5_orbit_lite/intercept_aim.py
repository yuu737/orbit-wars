"""Fixed-fleet intercept aim  Esub-turn-accurate angle for an orbiting target.

Solves the **continuous** intercept time ``t*`` (root of
``v·t = dist(target_pos(t), source) ∁Egap`` with the target on its analytic
orbit), aims at ``target_pos(t*)``, and verifies that angle with a
fully-vectorized analytic first-contact check.

* **Root**  Ea continuous fixed-point iteration (no grid scan / argmax /
  bisection), free of grid-resolution artifacts.
* **Verify**  E:func:`_analytic_first_contact` reproduces the engine's
...
def intercept_angle(
    ...
    *,
    ...
    speedup  E``None`` screens everything.
    """
    dev = movement.device
    dt = movement.dtype
...
def _analytic_first_contact(
    *,
    ...
    Returns ``(contact_slot, eta)``  E``contact_slot == -1`` and ``eta == H`` when
    the fleet contacts no planet (or dies first).

    Two-phase to keep the exact swept-pair off the common clear-shot path:

    * **Broad phase**  Ean AABB cull (the fleet's full-horizon segment box vs each
      ...
      Conservative ↁEthe kept set always contains every hittable planet, so the
      result is **byte-identical** to checking all ``P``.
    * **Narrow phase**  Ethe exact swept-pair only on the shortlisted planets,
    ...
    # --- Broad phase: AABB cull (no time axis ↁEcheap). The conservative segment box
    # runs launch ↁElaunch + u·seg_len, where seg_len bounds the fleet's relevant
    ...
    for s in range(0, N, chunk):
        ...
        # advanced-index the K shortlisted planets directly ↁE[n, H+1, K] (no [n,H+1,P])
        sl_e = sl.view(-1, 1, K)
