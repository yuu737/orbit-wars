...
``planets`` / ``initial_planets``  ``[P, 7]`` float32::
    ...
    0  Eplanet_id   (alive sentinel: id >= 0; padding value: -1)
    1  Eowner       (absolute player index; -1 = neutral)
    2  Ex           (board coordinates, 0 E00)
    3  Ey
    4  Eradius
    5  Eships       (current count)
    6  Eproduction  (ships added per turn when owned)

``fleets``  ``[F, 7]`` float32::

    0  Efleet_id    (alive sentinel: id >= 0)
    1  Eowner
    2  Ex
    3  Ey
    4  Eangle       (radians)
...
class ParsedObs:
    ...
    # --- raw planet fields --------------------------------------------------
    alive: Tensor       # bool   Eplanet_id >= 0 (not a padding slot)
    x: Tensor           # float  Ecurrent x position (0 E00)
    y: Tensor           # float  Ecurrent y position (0 E00)
    r: Tensor           # float  Eradius
    ships: Tensor       # float  Ecurrent ship count
    prod: Tensor        # float  Eproduction per turn
    owner_abs: Tensor   # float  Eabsolute owner id (-1 = neutral)

    # --- relative ownership masks (computed from owner_abs + player_id) -----
    owned: Tensor       # bool  Ealive & owner_abs == player_id
    is_enemy: Tensor    # bool  Ealive & owner_abs >= 0 & owner_abs != player_id
    is_neutral: Tensor  # bool  Ealive & owner_abs < 0

    # --- orbital parameters (reconstructed from initial_planets) ------------
    orb_r: Tensor       # float  Eorbital radius; 0.0 for static planets
    orb_a0: Tensor      # float  Einitial angle from CENTER (radians)
    is_orbiting: Tensor # bool   ETrue for rotating planets

    # --- game scalars -------------------------------------------------------
    angvel: Tensor      # [B] float  Eboard angular velocity (rad/turn)
    step: Tensor        # [B] float  Ecurrent game step
    ...
    f_owner: Tensor     # [F] float  Eabsolute owner
    f_x: Tensor         # [F] float
    f_y: Tensor         # [F] float
    f_angle: Tensor     # [F] float  Eradians
