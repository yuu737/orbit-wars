...
def _leader_owner_by_power(obs, prod: Tensor, *, player_count: int) -> int:
    owners = obs.owner_abs.to(torch.long)
    planet_owner_count = int(owners[owners >= 0].max().item()) + 1 if bool((owners >= 0).any()) else 1
    fleet_owners = obs.f_owner.to(torch.long)
    fleet_owner_count = int(fleet_owners[obs.f_alive].max().item()) + 1 if bool(obs.f_alive.any()) else 1
    owner_count = max(int(player_count), planet_owner_count, fleet_owner_count)
    ...
    for owner in range(owner_count):
        ...
        fleet_mask = obs.f_alive & (fleet_owners == int(owner))
        if bool(mask.any()) or bool(fleet_mask.any()):
            score = torch.tensor(0.0, dtype=prod.dtype, device=prod.device)
            if bool(mask.any()):
                score = score + obs.ships[mask].to(prod.dtype).sum() + prod[mask].to(prod.dtype).sum() * 13.0
                if step >= 80:
                    score = score + mask.to(prod.dtype).sum() * 5.0
            if bool(fleet_mask.any()):
                score = score + obs.f_ships[fleet_mask].to(prod.dtype).sum()
            power[owner] = score
