...
class ProducerLiteConfig:
    ...
    # --- sample139: 2P threat-aware source garrison reservation -------------
    # 4P で勝率を 13W->16W 改善した守備予約(_threat_reserve_source_budget)を 2P 専用
    # フラグで独立に有効化。2P の負け筋は「早期全滅」であり、攻め込まれる自惑星に
    # 生存に必要な守備を残す HARD 制約は「確実に勝ち切る」方針に直結する。4P 経路は不変。
    enable_threat_reserve_2p: bool = False
    threat_reserve_2p_window: int = 6
    threat_reserve_2p_margin: float = 3.0
    threat_reserve_2p_prod_frac: float = 0.5
    threat_reserve_2p_start: int = 0
    threat_reserve_2p_limit: int = 500
    # --- sample140: 2P threat_reserve 発火の精密化 -------------------------
    # 無差別な守備予約は random20 で攻撃機会を奪い勝ち数 -2 になった。価値ゲート
    # (低生産の前線惑星は失っても安い→予約しない)と終盤除外(攻め切る局面では予約
    # しない)で、早期全滅だけを防ぎ攻めの主導権を残す。0.0/0 は無効(=全惑星/全期間)。
    threat_reserve_2p_min_prod: float = 0.0
...
def _threat_reserve_source_budget(
    *,
    ...
    """
    is_2p = int(player_count) == 2
    if is_2p:
        if not bool(config.enable_threat_reserve_2p):
            return source_budget
    elif int(player_count) < 4 or not bool(config.enable_threat_reserve_4p):
        return source_budget
    step = _obs_step(obs_tensors)
    reserve_start = int(config.threat_reserve_2p_start if is_2p else config.threat_reserve_start)
    reserve_limit = int(config.threat_reserve_2p_limit if is_2p else config.threat_reserve_limit)
    if step < reserve_start or step > reserve_limit:
        return source_budget
    ...
    if H <= 0:
        return source_budget
    W = max(1, min(int(config.threat_reserve_2p_window if is_2p else config.threat_reserve_window), H))
    ...
    ships = obs.ships.to(dtype)
    prod_buffer = prod.to(dtype) * float(W) * float(config.threat_reserve_2p_prod_frac if is_2p else config.threat_reserve_prod_frac)
    margin = float(config.threat_reserve_2p_margin if is_2p else config.threat_reserve_margin)

    # Ships that must stay home for the planet to survive inbound enemy fleets.
    reserve_need = enemy_arr + margin - my_arr - prod_buffer
    owned = obs.owned & obs.alive
    # sample140: 価値ゲート — 2P では高生産惑星だけ守る(低生産の前線は失っても安い)。
    # min_prod<=0 は従来どおり全惑星対象。4P 経路は min_prod=0 で不変。
    min_prod = float(config.threat_reserve_2p_min_prod) if is_2p else 0.0
    if min_prod > 0.0:
        value_mask = prod.to(dtype) >= min_prod
    else:
        value_mask = torch.ones_like(owned)
    apply_mask = owned & value_mask & (enemy_arr > 0.0) & (reserve_need > 0.0) & (reserve_need <= ships)
...
# sample139/140: 2P で守備予約(threat_reserve)を精密化して有効化。4P 経路は不変。
_CONFIG_2P_BASE = dataclasses.replace(
    ProducerLiteConfig(),
    enable_threat_reserve_2p=True,
    threat_reserve_2p_min_prod=3.0,  # 高生産惑星だけ守る(価値ゲート)
    threat_reserve_2p_limit=300,     # 終盤(300+)は攻め切る/総力戦を優先
)
