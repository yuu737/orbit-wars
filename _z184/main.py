...
class ProducerLiteConfig:
    ...
    # --- sample184: 4P snowball focus-fire (aggressive, NOT passive) ---
    # Competition (replays_my_4p_176): one rival snowballs to 16-26 planets while we and the
    # others fail to stop it. When an opponent is the CLEAR runaway production leader, boost
    # attacks against THAT player's planets (deny their growth) -- the 4P gang-up-the-leader
    # meta. Offensive (no defensive suppression), gated so it only fires vs a real runaway.
    enable_snowball_focus_4p: bool = False
    snowball_focus_margin: float = 1.3        # leader_prod >= max(2nd, mine) * this => runaway
    snowball_focus_bonus: float = 0.5
    snowball_focus_prod_scale: float = 0.10
    snowball_focus_start: int = 45
...
def _apply_snowball_focus_4p(
    *,
    ...
    """4P snowball focus-fire: when one opponent is the clear runaway production leader,
    boost attacks against THAT player's planets to deny its growth (gang-up-the-leader).
    Offensive only -- no defensive suppression, so it does not cause passivity."""
    if int(player_count) < 4 or not bool(config.enable_snowball_focus_4p):
        return score
    step = int(float(obs.step.reshape(-1)[0].item()))
    if step < int(config.snowball_focus_start):
        return score
    P = int(obs.P)
    if P <= 0 or score.numel() == 0:
        return score
    ...
    for owner in range(int(player_count)):
        if owner == pid:
            continue
        prod_by_owner[owner] = prod[alive & (owners == int(owner))].to(dtype).sum()
    if not prod_by_owner:
        return score
    leader = max(prod_by_owner, key=lambda o: float(prod_by_owner[o]))
    leader_prod = prod_by_owner[leader]
    others = [float(v) for o, v in prod_by_owner.items() if o != leader]
    second = max(others) if others else 0.0
    ref = max(float(second), float(my_prod.item()))
    runaway = bool(float(leader_prod.item()) > 0.0 and float(leader_prod.item()) >= ref * float(config.snowball_focus_margin))
    if not runaway:
        return score

    active = cand_active.any(dim=-1) & torch.isfinite(score)
    tgt = cand_tgt_slot.clamp(0, max(P - 1, 0))
    target_is_leader = active & (owners[tgt] == int(leader)) & alive[tgt]
    target_prod = prod[tgt].to(dtype)
    bonus = torch.where(
        target_is_leader,
        torch.full_like(score, float(config.snowball_focus_bonus)) + target_prod * float(config.snowball_focus_prod_scale),
        torch.zeros_like(score),
    )
    return score + bonus
...
def plan_lite_waves(
    *,
    ...
    score = _apply_snowball_focus_4p(
        score=score,
        cand_tgt_slot=cand_tgt_slot,
        cand_active=cand_active,
        obs=obs,
...
CONFIG_4P = dataclasses.replace(
    ...
    enable_snowball_focus_4p=True,
    snowball_focus_margin=1.3,
    snowball_focus_bonus=0.5,
    snowball_focus_prod_scale=0.10,
    snowball_focus_start=45,
    coord_start_turn=18,
