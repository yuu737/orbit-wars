...
class ProducerLiteConfig:
    ...
    # --- 2P stateful opening policy --------------------------------------
    enable_opening_policy_2p: bool = True
    opening_policy_turns: int = 105
    opening_policy_max_targets: int = 5
    opening_policy_bonus: float = 0.70
...
def _make_opening_policy_2p(
    *,
    ...
    """Choose a 2P opening mode once, then keep following its target route.

    Modes:
    - race: we can reach the contested neutral at about the same time or earlier.
    - delayed_retake: enemy should arrive first, so launch a follow-up capture.
    - alternate_route: the contest is bad, so build a safer nearby route.
    """
    P = int(obs.P)
    if P <= 0:
        return None
    ...
    enemy_src = obs.alive & obs.is_enemy & (obs.ships >= float(config.min_ships_to_launch))
    if not bool(neutral.any()) or not bool(source_mask.any()) or not bool(enemy_src.any()):
        return None
    ...
    enemy_eta = torch.where(enemy_src.view(P, 1), eta_all, inf_all).amin(dim=0)

    useful = neutral & ((prod >= 2.0) | (obs.ships <= 30.0))
    base_value = prod.to(dtype) * 7.0 - obs.ships.to(dtype) * 0.06 - my_eta * 0.18

    race_mask = useful & (my_eta <= float(K_eta + 5)) & (enemy_eta <= float(K_eta + 7)) & (my_eta <= enemy_eta + 1.5)
    race_score = torch.where(
        race_mask,
        base_value + (enemy_eta - my_eta).clamp(min=-2.0, max=5.0) * 0.8,
        torch.full((P,), float("-inf"), dtype=dtype, device=device),
    )

    retake_mask = useful & (enemy_eta + 0.5 < my_eta) & (my_eta <= enemy_eta + 7.0) & (my_eta <= float(K_eta + 8)) & (prod >= 2.0)
    retake_score = torch.where(
        retake_mask,
        base_value + prod.to(dtype) * 2.0 + (7.0 - (my_eta - enemy_eta)).clamp(min=0.0) * 0.7,
        torch.full((P,), float("-inf"), dtype=dtype, device=device),
    )

    alt_mask = useful & (my_eta <= float(K_eta + 4)) & ((enemy_eta - my_eta >= 3.0) | (enemy_eta > float(K_eta + 10)))
    alt_score = torch.where(
        alt_mask,
        base_value + (enemy_eta - my_eta).clamp(min=0.0, max=8.0) * 0.35,
        torch.full((P,), float("-inf"), dtype=dtype, device=device),
    )

    mode_scores = {
        "race": float(race_score.max().item()),
        "delayed_retake": float(retake_score.max().item()),
        "alternate_route": float(alt_score.max().item()),
    }
    mode = max(mode_scores, key=mode_scores.get)
    ranked = {"race": race_score, "delayed_retake": retake_score, "alternate_route": alt_score}[mode]
    if not torch.isfinite(ranked).any():
        return None

    primary_cap = max(1, min(3, int(config.opening_policy_max_targets), P))
    primary_idx, primary_exists = _candidate_indices(ranked, torch.isfinite(ranked), primary_cap)
    primary_idx = primary_idx[primary_exists]
    if primary_idx.numel() == 0:
        return None

    # Add a small second-hop cluster around the selected route.  This is what
    # makes the policy a route rather than one more single-target bonus.
    chain_rank = torch.full((P,), float("-inf"), dtype=dtype, device=device)
    if int(config.opening_policy_max_targets) > int(primary_idx.numel()):
        chain_d = d0[primary_idx.clamp(0, P - 1), :].amin(dim=0)
        already = torch.zeros(P, dtype=torch.bool, device=device)
        already.scatter_(0, primary_idx.clamp(0, P - 1), True)
        chain_mask = neutral & ~already & (chain_d <= 42.0) & ((prod >= 2.0) | (obs.ships <= 25.0))
        chain_rank = torch.where(
            chain_mask,
            prod.to(dtype) * 5.0 - obs.ships.to(dtype) * 0.04 - chain_d * 0.04,
            chain_rank,
        )
    chain_cap = max(0, int(config.opening_policy_max_targets) - int(primary_idx.numel()))
    if chain_cap > 0 and bool(torch.isfinite(chain_rank).any()):
        chain_idx, chain_exists = _candidate_indices(chain_rank, torch.isfinite(chain_rank), min(chain_cap, P))
        plan_idx = torch.cat([primary_idx, chain_idx[chain_exists]], dim=0)
    else:
        plan_idx = primary_idx

    return {
        "mode": mode,
        "targets": [int(x) for x in plan_idx.detach().cpu().tolist()],
        "created_step": _obs_step(obs_tensors),
    }


def _opening_policy_targets(
    *,
    memory,
    obs,
    obs_tensors: dict[str, Any],
    ...
    if int(player_count) != 2 or not bool(config.enable_opening_policy_2p):
        return None, None
    step = _obs_step(obs_tensors)
    if step > int(config.opening_policy_turns):
        return None, None
    if getattr(memory, "opening_policy", None) is None or step <= 1:
        memory.opening_policy = _make_opening_policy_2p(
            obs=obs,
    ...
    plan = getattr(memory, "opening_policy", None)
    if not plan or not plan.get("targets"):
        return None, None
    P = int(obs.P)
    targets = [t for t in plan["targets"] if 0 <= int(t) < P and bool(obs.alive[int(t)])]
    if not targets:
        return None, str(plan.get("mode", ""))
    return torch.tensor(targets, dtype=torch.long, device=obs.device), str(plan.get("mode", ""))


def _append_opening_policy_targets(
    target_idx: Tensor,
    target_exists: Tensor,
    opening_idx: Tensor | None,
) -> tuple[Tensor, Tensor]:
    if opening_idx is None or opening_idx.numel() == 0:
        return target_idx, target_exists
    P_existing = int(target_idx.numel())
    existing = set(int(x) for x in target_idx[target_exists].detach().cpu().tolist())
    extras = [int(x) for x in opening_idx.detach().cpu().tolist() if int(x) not in existing]
    if not extras:
        return target_idx, target_exists
...
def _apply_opening_policy_bonus(
    *,
    ...
    if int(player_count) != 2 or opening_idx is None or opening_idx.numel() == 0:
        return score
    P = max(int(cand_tgt_slot.max().item()) + 1 if cand_tgt_slot.numel() else 0, int(opening_idx.max().item()) + 1)
    if P <= 0:
        return score
    target_mask = torch.zeros(P, dtype=torch.bool, device=score.device)
    target_mask[opening_idx.clamp(0, P - 1)] = True
    selected = target_mask[cand_tgt_slot.clamp(0, P - 1)] & cand_valid
    mode_mult = {
        "race": 1.00,
        "delayed_retake": 1.18,
        "alternate_route": 0.82,
    }.get(str(opening_mode), 1.0)
    bonus = float(config.opening_policy_bonus) * float(mode_mult)
    return score + torch.where(selected, torch.full_like(score, bonus), torch.zeros_like(score))
...
def plan_lite_waves(
    *,
    ...
    opening_idx, opening_mode = _opening_policy_targets(
        memory=memory,
        obs=obs,
        obs_tensors=obs_tensors,
        cache=cache,
    ...
    target_idx, target_exists = _append_opening_policy_targets(target_idx, target_exists, opening_idx)
    ...
    score = _apply_opening_policy_bonus(
        score=score,
...
class ProducerLiteMemory:
    ...
    def __init__(self) -> None:
        ...
        self.opening_policy: Optional[dict[str, Any]] = None
