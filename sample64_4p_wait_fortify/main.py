...
class ProducerLiteConfig:
    ...
    # --- sample64 wait/fortify: avoid early counters until the table weakens
    enable_wait_fortify_4p: bool = False
    wait_fortify_turn_limit: int = 125
    wait_fortify_enemy_attack_penalty: float = 0.62
    wait_fortify_contested_penalty: float = 0.70
    wait_fortify_center_penalty: float = 0.36
    wait_fortify_safe_neutral_bonus: float = 0.26
    wait_fortify_defense_bonus: float = 0.34
    wait_fortify_drain_penalty: float = 0.34
    wait_fortify_hold_base: float = 14.0
    wait_fortify_hold_prod: float = 3.6
...
def _apply_wait_fortify_policy(
    *,
    score: Tensor,
    cand_src: Tensor,
    cand_send: Tensor,
    ...
    if int(player_count) < 4 or not bool(config.enable_wait_fortify_4p):
        return score
    step = _obs_step(obs_tensors)
    if step > int(config.wait_fortify_turn_limit):
        return score
    P = int(obs.P)
    if P <= 0:
        return score
    ...
    alive_owners = {
        int(round(float(obs.owner_abs[i].item())))
        for i in range(P)
        if bool(obs.alive[i].item()) and int(round(float(obs.owner_abs[i].item()))) >= 0
    }
    if len(alive_owners) < 4:
        return score
    ...
    contested_neutral = (
        ...
        & obs.is_neutral[tgt]
        & (enemy_reach_count >= 2)
        & (my_support + send < enemy_pressure * 1.12 + 4.0)
    )
    center_target = active & (radius[tgt] <= 30.0) & (obs.is_neutral[tgt] | obs.is_enemy[tgt])
    safe_neutral = (
        active
        & obs.is_neutral[tgt]
        & (enemy_reach_count <= 1)
        & (my_support + send >= enemy_pressure * 0.72)
        & ((obs.ships[tgt].to(score.dtype) <= 35.0) | (prod[tgt].to(score.dtype) >= 3.0))
        & (radius[tgt] >= 27.0)
    )
    threatened_own = (
        active
        & obs.owned[tgt]
        & (enemy_pressure > my_support + obs.ships[tgt].to(score.dtype) * 0.55)
    )

    src_after = obs.ships[src].to(score.dtype) - send
    source_hold = (
        torch.full_like(score, float(config.wait_fortify_hold_base))
        + prod[src].to(score.dtype) * float(config.wait_fortify_hold_prod)
    )
    drain_risk = active & obs.owned[src] & (src_after < source_hold)

    bonus = torch.zeros_like(score)
    bonus = bonus + torch.where(
        safe_neutral,
        torch.full_like(score, float(config.wait_fortify_safe_neutral_bonus))
        + (prod[tgt].to(score.dtype) * 0.050).clamp(max=0.22),
        torch.zeros_like(score),
    )
    bonus = bonus + torch.where(
        threatened_own,
        torch.full_like(score, float(config.wait_fortify_defense_bonus)),
        torch.zeros_like(score),
    )
    ...
    penalty = penalty + torch.where(
        enemy_target,
        torch.full_like(score, float(config.wait_fortify_enemy_attack_penalty))
        + (send / 120.0).clamp(max=0.70),
        torch.zeros_like(score),
    )
    penalty = penalty + torch.where(
        contested_neutral,
        torch.full_like(score, float(config.wait_fortify_contested_penalty)),
        torch.zeros_like(score),
    )
    penalty = penalty + torch.where(
        center_target,
        torch.full_like(score, float(config.wait_fortify_center_penalty)),
        torch.zeros_like(score),
    )
    penalty = penalty + torch.where(
        drain_risk,
        ((source_hold - src_after).clamp(min=0.0) / 34.0).clamp(max=1.8)
        * float(config.wait_fortify_drain_penalty),
        torch.zeros_like(score),
    )
    return score + bonus - penalty
...
def plan_lite_waves(
    *,
    ...
    score = _apply_wait_fortify_policy(
        score=score,
        cand_src=cand_src,
        cand_send=cand_send,
        cand_eta=cand_eta,
...
CONFIG_4P_WAIT_FORTIFY = dataclasses.replace(
    CONFIG_4P_S7_STABLE,
    enable_wait_fortify_4p=True,
)
...
def _choose_4p_mode(obs_tensors: dict) -> str:
    ...
    # wait_fortify is kept as a forced probe. Automatic selection was too
    # passive on sparse 4P boards: survival improved, but score diff worsened.
    if (
        enemy_dist <= 55.0
...
def _config_for(player_count: int, mode: str | None = None) -> ProducerLiteConfig:
    ...
    if mode == "wait_fortify":
        return CONFIG_4P_WAIT_FORTIFY
    if mode == "s8_burst":
