...
class ProducerLiteConfig:
    ...
    phase_opening_turns: int = 90
    phase_midgame_turns: int = 260
    phase_endgame_turn: int = 420
    phase_prod_margin: float = 4.0
    phase_ship_ratio: float = 1.25
...
def _apply_2p_phase_controller(config: ProducerLiteConfig, obs_tensors: dict, player_count: int) -> ProducerLiteConfig:
    if not bool(config.phase_controller_enabled) or int(player_count) != 2:
        return config

    obs = parse_obs(obs_tensors)
    step = int(obs.step.reshape(-1)[0].item())
    my_prod = float(obs.prod[obs.owned].sum().item())
    enemy_prod = float(obs.prod[obs.is_enemy].sum().item())
    my_ships = float(obs.ships[obs.owned].sum().item())
    enemy_ships = float(obs.ships[obs.is_enemy].sum().item())
    if int(obs.F) > 0:
        my_fleet = (obs.f_alive & (obs.f_owner == float(obs.player_id)))
        enemy_fleet = (obs.f_alive & (obs.f_owner >= 0.0) & (obs.f_owner != float(obs.player_id)))
        my_ships += float(obs.f_ships[my_fleet].sum().item())
        enemy_ships += float(obs.f_ships[enemy_fleet].sum().item())

    prod_margin = float(config.phase_prod_margin)
    ship_ratio = float(config.phase_ship_ratio)

    if step >= int(config.phase_endgame_turn):
        if my_prod >= enemy_prod and my_ships >= enemy_ships:
            return dataclasses.replace(
                config,
                roi_threshold=max(float(config.roi_threshold), 1.9),
                max_waves_per_turn=min(int(config.max_waves_per_turn), 4),
                enable_regroup=False,
            )
        return dataclasses.replace(
            config,
            roi_threshold=min(float(config.roi_threshold), 1.05),
            max_waves_per_turn=max(int(config.max_waves_per_turn), 8),
            enable_regroup=False,
            reinforce_size_beta=min(float(config.reinforce_size_beta), 1.3),
        )

    if step < int(config.phase_opening_turns):
        return dataclasses.replace(
            config,
            roi_threshold=min(float(config.roi_threshold), 1.35),
            max_waves_per_turn=max(int(config.max_waves_per_turn), 7),
            max_offensive_targets=max(int(config.max_offensive_targets), 14),
            max_sources_per_lane=max(int(config.max_sources_per_lane), 14),
            max_regroup_time=min(float(config.max_regroup_time), 4.0),
        )

    ahead = (my_prod >= enemy_prod + prod_margin) or (my_ships >= enemy_ships * ship_ratio and my_prod >= enemy_prod)
    behind = (enemy_prod >= my_prod + prod_margin) or (enemy_ships >= my_ships * ship_ratio and enemy_prod >= my_prod)

    if ahead:
        return dataclasses.replace(
            config,
            roi_threshold=min(float(config.roi_threshold), 1.25),
            max_waves_per_turn=max(int(config.max_waves_per_turn), 8),
            max_offensive_targets=max(int(config.max_offensive_targets), 15),
            max_sources_per_lane=max(int(config.max_sources_per_lane), 14),
            max_regroup_time=min(float(config.max_regroup_time), 4.0),
        )

    if behind and step < int(config.phase_midgame_turns):
        return dataclasses.replace(
            config,
            roi_threshold=min(float(config.roi_threshold), 1.20),
            max_waves_per_turn=max(int(config.max_waves_per_turn), 8),
            max_offensive_targets=max(int(config.max_offensive_targets), 15),
            max_sources_per_lane=max(int(config.max_sources_per_lane), 14),
            reinforce_size_beta=min(float(config.reinforce_size_beta), 1.5),
            enable_regroup=False,
        )

    return dataclasses.replace(
        config,
        roi_threshold=min(float(config.roi_threshold), 1.45),
        max_waves_per_turn=max(int(config.max_waves_per_turn), 7),
        max_offensive_targets=max(int(config.max_offensive_targets), 13),
    )
...
class ProducerLiteRuntime:
    ...
    def tensor_action(self, obs_tensors: dict):
        ...
        config = _apply_2p_phase_controller(base, obs_tensors, int(mem.cached_player_count))
