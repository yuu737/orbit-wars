...
class ProducerLiteConfig:
    ...
    # --- sample143: 2P true-one-ply HARD-BAD exclusion (§3.2 lever 2B) ---
    # In 2P the mid-game leak (trace seed 24790546) is planet churn: we capture a
    # planet then get it immediately retaken while ship counts stay even, so equal
    # ships convert to fewer planets -> production-complex loss. This reads the enemy's
    # single best response to each top candidate and DROPS (sets score to -inf) only the
    # catastrophic ones -- captures that get recaptured / leave us badly behind after the
    # response. It NEVER adds a bonus, mixes base_weight, or rescales surviving candidates
    # (score-addition / action-steering is established as harmful, §1.1). Equal weighting
    # (my net minus the single enemy's net, no leader bias). Turn-gated from t34 to leave
    # the symmetric opening untouched. 2P ONLY: guarded by player_count >= 4 AND this flag.
...
def _apply_true_one_ply_hard_bad_2p(
    *,
    ...
    """2P-only true-one-ply HARD-BAD exclusion (§3.2 lever 2B).

    Reads the enemy's single best response to each of our top candidate launches and
    DROPS (sets score to -inf) only the catastrophic ones -- captures that are
    immediately retaken / leave us badly behind after the response. Unlike the 4P
    rescore it does NOT add bonuses, mix ``base_weight``, or rescale surviving
    candidates (score-addition / action-steering is established as harmful, §1.1): the
    score of every non-hard-bad candidate is returned unchanged. 2P uses equal
    weighting (``one_ply_net = my net - enemy net``, no leader bias). 4P is untouched.
    """
    ...
    # 2P equal weighting: my net minus the single enemy's net (no leader bias).
    me = net[:, pid] if 0 <= pid < int(net.shape[1]) else torch.zeros(N, dtype=dtype, device=device)
    ...
    # Same hard-bad shape as 4P: a move is catastrophic if the post-response net is
    # deeply negative, or it attacks an enemy planet that gets recaptured (the enemy's
    # response mass dwarfs ours and the net is negative) -- i.e. the churn we traced.
    hard_bad = (
...
def plan_lite_waves(
    *,
    ...
    # 2P-only hard-bad exclusion (sample143 / §3.2 lever 2B). The 4P rescore above
    # returns score unchanged for 2P; this drops only catastrophic (churn) candidates
    # and never touches 4P (guarded by player_count >= 4).
...
# sample143: enable the 2P true-one-ply hard-bad filter in CODE, not via params.json.
# The eval harness (evaluate.py / kaggle_environments env.run([path])) execs the agent
# WITHOUT setting __file__, so _HERE falls back to cwd (project root) and params.json is
# never found -> any config_2p override there is silently ignored. Baking the flags into
# the CONFIG_2P base guarantees the feature is active under both evaluate.py and the trace
# tools. Only the §3.2-2B fields differ from the dataclass default, so the baseline-
# relevant config is unchanged and the 2B effect is isolated. params.json (if it ever
# loads, e.g. a real submission where __file__ is set) still overrides on top.
_CONFIG_2P_BASE = dataclasses.replace(
    ProducerLiteConfig(),
    enable_true_one_ply_2p=True,
    true_one_ply_2p_start=55,
