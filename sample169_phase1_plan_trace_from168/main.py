...
def _tier_candidates(
    *,
    ...
    _plan_dbg(
        ...
        K=int(K),
        cells=int(S * T),
        reachable=_count_bool(active),
        aim_viable=_count_bool(viable),
        clears_floor=_count_bool(clears_floor),
        floor_blocked=_count_bool(viable & (~clears_floor)),
        src_eq_tgt=_count_bool(viable & clears_floor & (~src_neq_tgt)),
        source_or_target_missing=_count_bool(
            viable & clears_floor & (~(source_exists.view(S, 1) & target_exists.view(1, T)))
        ),
        valid=_count_bool(valid),
        defense_targets=_count_bool(target_is_mine),
    )
