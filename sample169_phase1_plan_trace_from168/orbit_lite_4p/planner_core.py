...
def _plan_dbg(event: str, **payload) -> None:
    """Append lightweight planner diagnostics when OW_PLAN_DBG is set."""
    path = os.environ.get("OW_PLAN_DBG")
    if not path:
        return
    try:
        rec = {"event": event, "module": __name__}
        rec.update(payload)
...
def _count_bool(x: Tensor) -> int:
    try:
        return int(x.to(torch.bool).sum().item())
    except Exception:
        return 0
...
def largest_initial_player_count(obs_tensors: dict) -> int:
    ...
    """
    player_tensor = obs_tensors.get("player")
    current_player = 0
    if player_tensor is not None:
        current_player = (
            int(player_tensor.flatten()[0].item())
            if isinstance(player_tensor, Tensor)
            else int(player_tensor)
        )
    ...
    if metadata_count is not None:
        ...
        if count in (2, 4):
            return 4 if max(count, current_player + 1) > 2 else 2
    initial = obs_tensors["initial_planets"]      # [P, 7]
    pid = initial[:, 0]
    owner = initial[:, 1]
    mask = (pid >= 0) & (owner >= 0)
    owners = owner[mask]
    n_max = max(2, current_player + 1)
    if owners.numel() > 0:
        n_max = max(n_max, int(torch.unique(owners.long()).numel()))
    return 4 if n_max > 2 else 2
    ...
    *, P, W, device, dtype, score, cand_src, cand_send, cand_angle, cand_eta,
    ...
    for w in range(W):
        ...
        if not fired:
            ...
            valid_mask = finite_mask & ~taken_cand & can_fund & ~tgt_used_as_src & ~contrib_defended
            reason = "roi" if bool(torch.isfinite(best_score)) else "no_candidate"
            _plan_dbg(
                "greedy_stop",
                W=int(W),
                C=int(C),
                L=int(L),
                wave=int(w),
                fired=int(fired_count),
                reason=reason,
                best_score=float(best_score.item()) if bool(torch.isfinite(best_score)) else None,
                roi_threshold=float(roi_threshold),
                finite=_count_bool(finite_mask),
                blocked_taken=_count_bool(finite_mask & taken_cand),
                blocked_budget=_count_bool(finite_mask & ~can_fund),
                blocked_tgt_used_as_src=_count_bool(finite_mask & tgt_used_as_src),
                blocked_contrib_defended=_count_bool(finite_mask & contrib_defended),
                valid_after_mask=_count_bool(valid_mask),
            )
            break
        fired_count += 1
        ...
        _plan_dbg("greedy_stop", W=int(W), C=int(C), L=int(L), wave=int(W), fired=int(fired_count), reason="max_waves")
