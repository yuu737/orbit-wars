...
# sample163 SUBMISSION MERGE: sample157 4P (third-party guard, in CONFIG_4P above) +
# sample161 2P (more-active: roi_threshold 1.5->0.9, max_waves_per_turn 7->12). Both baked
# in CODE because params.json is ignored under the eval harness (__file__ unset). 2P
# activity is the asymmetric edge vs the low-activity sample8 baseline (60% on random40).
_CONFIG_2P_BASE = dataclasses.replace(ProducerLiteConfig(), roi_threshold=0.9, max_waves_per_turn=12)
