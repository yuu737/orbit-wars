...
class ProducerLiteConfig:
    ...
    # sample135: opening neutral focus. Trace shows we are even at t32 then over-commit
    # launches (incl. enemy attacks) and lose planets while rivals take neutrals and
    # expand. Hard-forbid offensive ENEMY-target captures before opening_no_enemy_turn
    # in 4P so the opening builds a holdable neutral cluster instead of trading.
    enable_opening_neutral_focus_4p: bool = False
    opening_no_enemy_turn: int = 60
...
def _apply_opening_neutral_focus(
    *,
    score: Tensor,
    cand_tgt_slot: Tensor,
    cand_is_def: Tensor,
    ...
    """Hard-forbid offensive captures of enemy-owned planets during the 4P opening,
    so the early game builds a holdable neutral cluster instead of trading ships in
    attrition. Neutral and own (reinforcement) targets are untouched."""
    if int(player_count) < 4 or not bool(config.enable_opening_neutral_focus_4p):
        return score
    if _obs_step(obs_tensors) >= int(config.opening_no_enemy_turn):
        return score
    P = int(obs.P)
    if P <= 0:
        return score
    tgt = cand_tgt_slot.clamp(0, max(P - 1, 0))
    enemy_tgt = cand_active.any(dim=-1) & torch.isfinite(score) & (~cand_is_def) & obs.is_enemy[tgt]
    return torch.where(enemy_tgt, torch.full_like(score, float("-inf")), score)
...
def plan_lite_waves(
    *,
    ...
    score = _apply_opening_neutral_focus(
        score=score,
        cand_tgt_slot=cand_tgt_slot,
...
CONFIG_4P = dataclasses.replace(
    ...
    # sample135: opening neutral focus (no enemy attacks before turn 60 in 4P).
    enable_opening_neutral_focus_4p=True,
    opening_no_enemy_turn=60,
)
