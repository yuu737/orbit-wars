...
def _apply_2p_midgame_hold_bias(
    *,
    ...
    dtype = score.dtype
    center_radius = torch.sqrt((obs.x.to(dtype) - 50.0) ** 2 + (obs.y.to(dtype) - 50.0) ** 2)
    center_prod = torch.where(
        obs.alive & (center_radius <= 28.0),
        prod.to(dtype),
        torch.zeros_like(prod.to(dtype)),
    ).sum()
...
def _choose_2p_mode(obs_tensors: dict) -> str:
    ...
    sa = _angle_from_center_xy(sx, sy)
    ...
    for mid in neutrals:
        ...
        if mp < 2.0 and not (15.0 <= msh <= 40.0):
            continue
        ma = _angle_from_center_xy(mx, my)
        ...
        for outer in neutrals:
            ...
            if not (18.0 <= od <= 80.0):
                continue
            if _angle_diff(ma, _angle_from_center_xy(ox, oy)) > 0.85:
                continue
    ...
    for anchor in neutrals:
        ...
        support = [
            p for p in neutrals
            if p is not anchor
            and _angle_diff(aa, _angle_from_center_xy(p[2], p[3])) <= 0.95
...
class ProducerLiteRuntime:
    ...
    def tensor_action(self, obs_tensors: dict):
        ...
        if int(mem.cached_player_count) < 4:
            mem.strategy_mode = None
        mode = mem.strategy_mode if int(mem.cached_player_count) >= 4 else mem.mode_2p
        base = _config_for(mem.cached_player_count, mode)
        step = int(obs_tensors["step"].reshape(-1)[0].item())
        if int(mem.cached_player_count) >= 4:
