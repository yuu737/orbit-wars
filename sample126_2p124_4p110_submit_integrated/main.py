...
def _purge_orbit_lite_for_4p() -> None:
...
def _agent_4p():
    global _MOD_4P
    if _MOD_4P is not None:
        return _MOD_4P

    root = os.path.join(_HERE, "sample110")
    main_path = os.path.join(root, "main.py")
    if not os.path.exists(main_path):
        raise FileNotFoundError(main_path)

    _purge_orbit_lite_for_4p()
    if root in sys.path:
        sys.path.remove(root)
    sys.path.insert(0, root)

    spec = importlib.util.spec_from_file_location("_sample126_sample110", main_path)
    if spec is None or spec.loader is None:
        raise ImportError(main_path)
    mod = importlib.util.module_from_spec(spec)
    sys.modules["_sample126_sample110"] = mod
...
def agent(obs):
    ...
    if raw_step is not None and int(raw_step or 0) == 0:
        _USE_4P = _player_count_from_obs(obs) >= 4
    if _USE_4P is None:
        _USE_4P = _player_count_from_obs(obs) >= 4
