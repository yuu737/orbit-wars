...
class ProducerLiteConfig:
    ...
    # --- sample180: 4P core-objective production-compound weight (Phase 3 deepen) ---
    # Scales the production component of the competitive objective above combat (1.0 = the
    # unchanged net_ship_delta). Targets the myopic objective at the root: production over
    # the short flow horizon under-credits full-game compounding, so high-prod durable
    # captures are under-valued -> the 5-8 planet plateau. Applied in orbit_lite_4p only.
    production_weight_4p: float = 1.0
...
def _tier_candidates(
    *,
    ...
    # sample180: 4P-only production-compound weight. Passed ONLY for 4P (>=4) so the 2P
    # planner (orbit_lite_2p.score_candidates, which has no such kwarg) is never touched.
    _sc_kw = {}
    if int(player_count) >= 4 and float(config.production_weight_4p) != 1.0:
        _sc_kw["production_weight"] = float(config.production_weight_4p)
    score = score_candidates(
        garrison_status, prod=prod, alive_by_step=alive_by_step,
        player_count=int(player_count), launches=launches, player_id=pid, **_sc_kw,
    )
