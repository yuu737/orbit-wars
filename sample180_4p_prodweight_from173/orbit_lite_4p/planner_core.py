...
def competitive_score(diff: GarrisonFlowDiff, *, player_id: int, production_weight: float = 1.0) -> Tensor:
    ...
    sample180 (Phase-3 deepen, orbit_lite_4p ONLY): ``production_weight`` (default 1.0
    = byte-identical to net_ship_delta) scales the PRODUCTION component above the combat
    component. A planet's production over the short flow horizon under-credits its
    full-game compounding value, so weighting produced_delta > 1 pushes the planner to
    value durable high-prod captures (the structural plateau at 5-8 planets).
    """
    pw = float(production_weight)
    if pw == 1.0:
        net = diff.net_ship_delta                   # [*prefix, A]
    else:
        net = diff.ships_produced_delta * pw - diff.ships_lost_combat_delta
...
def score_candidates(
    ...
    *,
    ...
    Uses the sparse exact flow projector. ``production_weight`` (default 1.0) is the
    Phase-3 production-compound knob; see ``competitive_score``.
    """
    ...
    return competitive_score(diff, player_id=int(player_id), production_weight=production_weight)
