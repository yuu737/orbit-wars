...
def _single_launch_entries(
    *,
    ...
    return LaunchEntries(
        source_slots=torch.tensor([int(source)], dtype=torch.long, device=device),
        target_slots=torch.tensor([int(target)], dtype=torch.long, device=device),
        ships=torch.tensor([float(ships)], dtype=dtype, device=device),
        angle=angle.reshape(1).to(dtype),
        eta=eta.reshape(1).to(dtype).clamp(min=1.0),
        valid=torch.ones(1, dtype=torch.bool, device=device),
    )


def _mark_taken_target(target_exists_for_normal: Tensor, target_idx: Tensor, *, target: int, P: int) -> Tensor:
    taken = torch.zeros(P, dtype=torch.bool, device=target_exists_for_normal.device)
    taken[int(target)] = True
    return target_exists_for_normal & (~taken[target_idx.clamp(0, P - 1)])


def _direct_safe_neutral_slot(
    *,
    ...
    dtype = obs.ships.dtype
    P = int(obs.P)
    if not _portfolio_enabled(config, player_count=player_count, current_step=current_step) or P <= 0:
        return _empty_entries(device, dtype), source_budget, target_exists_for_normal
    mine = obs.owned & obs.alive & (source_budget >= float(config.min_ships_to_launch))
    neutral = obs.is_neutral & obs.alive
    if not bool(mine.any()) or not bool(neutral.any()):
        return _empty_entries(device, dtype), source_budget, target_exists_for_normal

    d0 = cache.cross_dist[0].to(dtype)
    enemy = obs.is_enemy & obs.alive
    enemy_speed = fleet_speed(obs.ships.to(dtype).clamp(min=1e-6))
    enemy_eta_all = torch.where(
        enemy.view(P, 1),
        d0 / enemy_speed.view(P, 1).clamp(min=1e-6),
        torch.full((P, P), float("inf"), dtype=dtype, device=device),
    )
    enemy_min_eta = enemy_eta_all.min(dim=0).values
    enemy_reach_count = ((enemy_eta_all <= 14.0) & enemy.view(P, 1)).sum(dim=0)

    best = None
    source_ids = torch.nonzero(mine, as_tuple=False).flatten()
    target_ids = torch.nonzero(neutral, as_tuple=False).flatten()
    for src_t in source_ids.tolist():
        src = int(src_t)
        for tgt_t in target_ids.tolist():
            tgt = int(tgt_t)
            if bool((target_idx[target_exists_for_normal].clamp(0, max(P - 1, 0)) == tgt).any()) is False:
                continue
            dist = float(d0[src, tgt].item())
            speed = float(fleet_speed(source_budget[src].reshape(())).item())
            eta_est = max(1.0, dist / max(speed, 1e-6))
            if eta_est > float(min(int(config.horizon), max(1, H))):
                continue
            if eta_est > 18.0:
                continue
            need = float(obs.ships[tgt].item()) + 1.0
            send = max(float(config.min_ships_to_launch), need + 1.0)
            budget = float(source_budget[src].item())
            if send > budget or send > max(8.0, budget * 0.72):
                continue
            enemy_eta = float(enemy_min_eta[tgt].item())
            if enemy_eta + 2.5 < eta_est:
                continue
            if int(enemy_reach_count[tgt].item()) >= 2 and float(prod[tgt].item()) <= 3.0:
                continue
            radius = math.hypot(float(obs.x[tgt].item()) - 50.0, float(obs.y[tgt].item()) - 50.0)
            score = (
                float(prod[tgt].item()) * 44.0
                - send * 2.6
                - eta_est * 2.2
                + (6.0 if radius >= 30.0 else -8.0)
                + max(0.0, enemy_eta - eta_est) * 0.8
            )
            if best is None or score > best[0]:
                best = (score, src, tgt, send)
    ...
    aim = intercept_angle(
        ...
        torch.tensor([send], dtype=dtype, device=device),
        active=torch.ones(1, dtype=torch.bool, device=device),
    )
    if not bool(aim["viable"].reshape(-1)[0].item()):
        return _empty_entries(device, dtype), source_budget, target_exists_for_normal
    entries = _single_launch_entries(
        source=src, target=tgt, ships=send,
        angle=aim["angle"].reshape(-1)[0], eta=aim["eta"].reshape(-1)[0],
        device=device, dtype=dtype,
    )
    source_budget = source_budget.clone()
    source_budget[src] = (source_budget[src] - float(send)).clamp(min=0.0)
    target_exists_for_normal = _mark_taken_target(target_exists_for_normal, target_idx, target=tgt, P=P)
    return entries, source_budget, target_exists_for_normal


def _direct_abandon_regroup_slot(
    *,
    movement: PlanetMovement,
    obs,
    prod: Tensor,
    ...
    doomed = (
        mine
        & torch.isfinite(loss_turn)
        & (loss_turn <= float(config.portfolio_abandon_high_prod_loss_turn))
        & (prod.to(dtype) <= float(config.portfolio_abandon_max_prod))
        & (source_budget >= float(config.min_ships_to_launch))
    )
    anchors = mine & ((~torch.isfinite(loss_turn)) | (loss_turn > float(config.portfolio_home_high_prod_loss_turn)))
    anchors = anchors & (prod.to(dtype) >= float(config.portfolio_regroup_anchor_min_prod))
    if not bool(doomed.any()) or not bool(anchors.any()):
        return _empty_entries(device, dtype), source_budget, target_exists_for_normal
    d0 = cache.cross_dist[0].to(dtype)
    best = None
    for src_t in torch.nonzero(doomed, as_tuple=False).flatten().tolist():
        src = int(src_t)
        max_send = float(source_budget[src].item()) * float(config.portfolio_abandon_send_frac)
        send = math.floor(max(float(config.min_ships_to_launch), max_send))
        if send < float(config.min_ships_to_launch):
            continue
        for tgt_t in torch.nonzero(anchors, as_tuple=False).flatten().tolist():
            tgt = int(tgt_t)
            if src == tgt:
                continue
            dist = float(d0[src, tgt].item())
            speed = float(fleet_speed(torch.tensor(float(send), dtype=dtype, device=device)).item())
            eta_est = max(1.0, dist / max(speed, 1e-6))
            if eta_est > 18.0 or eta_est > float(max(1, H)):
                continue
            score = (
                float(source_budget[src].item()) * 0.38
                + float(prod[tgt].item()) * 9.0
                - float(prod[src].item()) * 10.0
                - eta_est * 1.5
                - float(loss_turn[src].item()) * 1.2
            )
            if best is None or score > best[0]:
                best = (score, src, tgt, send)
...
def plan_lite_waves(
    *,
    ...
    portfolio_direct_safe_entries = _empty_entries(device, dtype)
    portfolio_features = None
    if _portfolio_enabled(config, player_count=player_count, current_step=current_step):
        direct_portfolio_enabled = getattr(memory, "strategy_mode", None) == "low_conflict_expansion"
        ...
        if direct_portfolio_enabled:
            portfolio_abandon_entries, source_budget, target_exists_for_normal = _direct_abandon_regroup_slot(
                movement=movement,
                obs=obs,
                prod=prod,
                cache=cache,
            ...
            portfolio_direct_safe_entries, source_budget, target_exists_for_normal = _direct_safe_neutral_slot(
                movement=movement,
                obs=obs,
                prod=prod,
                cache=cache,
    ...
    if bool(portfolio_direct_safe_entries.valid.any()) or (portfolio_features is not None and portfolio_features.cheap_safe_neutral_count <= 0):
    ...
    if not bool(config.enable_regroup):
        return concat_launch_entries([portfolio_home_entries, portfolio_abandon_entries, portfolio_direct_safe_entries, reserve_entries, lane_entries, wave_entries])
    ...
    return concat_launch_entries([portfolio_home_entries, portfolio_abandon_entries, portfolio_direct_safe_entries, reserve_entries, lane_entries, wave_entries, regroup_entries])
...
CONFIG_4P_LOW_CONFLICT = dataclasses.replace(
    CONFIG_4P_S7_STABLE,
    enable_portfolio_executor_4p=True,
