...
class ProducerLiteConfig:
    ...
    opening_route_enabled: bool = False
    opening_route_turns: int = 90
    opening_route_extra_targets: int = 6
    opening_route_near_dist: float = 360.0
    opening_route_max_dist: float = 760.0
    opening_route_link_dist: float = 340.0
    opening_route_min_score: float = 6.0
    opening_route_mid_prod: float = 1.0
    opening_route_high_prod: float = 2.0
    opening_route_high_ships: float = 24.0
...
def _add_opening_route_targets(
    *,
    obs,
    cache,
    prod: Tensor,
    ...
    if (
        not bool(config.opening_route_enabled)
        or int(player_count) != 2
        or int(step) >= int(config.opening_route_turns)
        or int(obs.P) <= 0
        or not bool(source_mask.any())
    ):
    ...
    src_dist = torch.where(source_mask.view(P, 1), d0, torch.full_like(d0, float("inf"))).amin(dim=0)
    ...
    near_mask = (
        neutral
        & (src_dist <= float(config.opening_route_near_dist))
        & ((prod.to(dtype) >= float(config.opening_route_mid_prod)) | (obs.ships.to(dtype) >= 10.0))
    )
    far_mask = (
        neutral
        & (src_dist <= float(config.opening_route_max_dist))
        & (src_dist >= float(config.opening_route_near_dist) * 0.35)
        & ((prod.to(dtype) >= float(config.opening_route_high_prod)) | (obs.ships.to(dtype) >= float(config.opening_route_high_ships)))
    )
    if not bool(near_mask.any()) or not bool(far_mask.any()):
        return target_idx, target_exists

    near_idx = torch.nonzero(near_mask, as_tuple=False).view(-1)
    near_to_all = d0[near_idx, :] if int(near_idx.numel()) > 0 else torch.empty(0, P, dtype=dtype, device=device)
    near_closer = src_dist[near_idx].view(-1, 1) + 1.0 < src_dist.view(1, P)
    linked = (near_to_all <= float(config.opening_route_link_dist)) & near_closer
    near_value = (prod[near_idx].to(dtype) * 4.0 + obs.ships[near_idx].to(dtype) * 0.04).view(-1, 1)
    chain_bonus = torch.where(linked, near_value, torch.zeros_like(near_value)).amax(dim=0)
    ...
    if bool(enemy_mask.any()):
        ...
        enemy_adv = (src_dist - enemy_dist).clamp(min=0.0)
    else:
    ...
    route_score = (
        ...
        + chain_bonus
        - src_dist.clamp(max=float(config.opening_route_max_dist)) * 0.015
        - enemy_adv * 0.035
    )
    route_score = torch.where(far_mask, route_score, torch.full_like(route_score, float("-inf")))
    viable_route = far_mask & (route_score >= float(config.opening_route_min_score))
    if not bool(viable_route.any()):
        return target_idx, target_exists

    cap = max(1, min(int(config.opening_route_extra_targets), P))
    route_idx, route_exists = _candidate_indices(route_score, viable_route, cap)
    route_idx = route_idx[route_exists]
    if int(route_idx.numel()) == 0:
        return target_idx, target_exists

    base = target_idx[target_exists]
    seen = set()
    merged = []
    for value in base.detach().cpu().tolist() + route_idx.detach().cpu().tolist():
        ivalue = int(value)
        if 0 <= ivalue < P and ivalue not in seen:
            seen.add(ivalue)
            merged.append(ivalue)
    if not merged:
        return target_idx, target_exists
    out_idx = torch.tensor(merged, dtype=torch.long, device=device)
    out_exists = torch.ones(int(out_idx.numel()), dtype=torch.bool, device=device)
    return out_idx, out_exists
...
def plan_lite_waves(
    *,
    ...
    target_idx, target_exists = _add_opening_route_targets(
        obs=obs,
        cache=cache,
        prod=prod,
        config=config,
