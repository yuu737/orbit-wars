...
class ProducerLiteConfig:
    ...
    # --- sample66: use orbit_lite recurrence to reject attacks we cannot hold.
    enable_hold_predictor_4p: bool = False
    hold_predictor_turn_limit: int = 170
    hold_predictor_window: int = 7
    hold_predictor_enemy_penalty: float = 1.10
    hold_predictor_neutral_penalty: float = 0.42
    hold_predictor_response_frac: float = 0.55
    hold_predictor_response_margin: float = 6.0
    hold_predictor_source_reserve: float = 18.0
...
def _candidate_target_hold_ok(
    *,
    ...
    if P <= 0 or H <= 0 or C <= 0 or garrison_status.arrivals_by_owner is None:
        valid = cand_active.any(dim=-1)
        return valid, torch.zeros(C, dtype=dtype, device=device)
    ...
    A = int(player_count)
    arrivals_base = garrison_status.arrivals_by_owner[tgt, 1:, :].to(dtype)  # [C,H,A]
    arrivals_delta = torch.zeros(C, H, A, dtype=dtype, device=device)
    rows = torch.arange(C, device=device)
    valid_launch = active & (send > 0.0)
    rows_v = rows[valid_launch]
    if rows_v.numel() > 0:
        arrivals_delta[rows_v, h_idx[valid_launch], int(player_id)] = send[valid_launch]
    arrivals = arrivals_base + arrivals_delta
    ...
    alive_t = alive_by_step[:, tgt].transpose(0, 1).to(torch.bool)  # [C,H+1]
    owner_t, ships_t, _pre_owner, _pre_ships = _run_exact_recurrence(
    ...
    owner_t = owner_t.squeeze(1)  # [C,H+1]
    ships_t = ships_t.squeeze(1)

    owner_at_arrival = owner_t.gather(1, k_arr.view(C, 1)).squeeze(1)
    mine_at_arrival = owner_at_arrival == int(player_id)
    steps = torch.arange(H + 1, device=device).view(1, H + 1)
    end_k = (k_arr + int(hold_window)).clamp(max=H)
    hold_window_mask = (steps >= k_arr.view(C, 1)) & (steps <= end_k.view(C, 1))
    lost_during_window = ((owner_t != int(player_id)) & hold_window_mask).any(dim=1)
    min_ships = torch.where(
        hold_window_mask & (owner_t == int(player_id)),
        ships_t,
        torch.full_like(ships_t, float("inf")),
    ).min(dim=1).values
    min_ships = torch.where(torch.isfinite(min_ships), min_ships, torch.zeros_like(min_ships))
    hold_ok = active & mine_at_arrival & (~lost_during_window)
    return hold_ok, min_ships


def _apply_orbit_lite_hold_predictor(
    *,
    score: Tensor,
    cand_send: Tensor,
    cand_eta: Tensor,
    ...
    if int(player_count) < 4 or not bool(config.enable_hold_predictor_4p):
        return score
    step = _obs_step(obs_tensors)
    if step > int(config.hold_predictor_turn_limit):
        return score
    P = int(obs.P)
    if P <= 0:
        return score
    ...
    if not bool((enemy_target | neutral_target).any()):
        return score

    hold_ok, min_hold_ships = _candidate_target_hold_ok(
        cand_send=cand_send,
        cand_eta=cand_eta,
        ...
        player_count=int(player_count),
        hold_window=int(config.hold_predictor_window),
    )
    dtype = score.dtype
    d0 = cache.cross_dist[0].to(dtype)
    enemy_source = obs.alive & (obs.owner_abs >= 0) & (obs.owner_abs != int(obs.player_id))
    source_ships = obs.ships.to(dtype)
    source_speed = fleet_speed(source_ships.clamp(min=1.0))
    reserve = (
        torch.full_like(source_ships, float(config.hold_predictor_source_reserve))
        + prod.to(dtype) * 2.5
    )
    response_available = ((source_ships - reserve).clamp(min=0.0) * float(config.hold_predictor_response_frac))
    response_available = torch.where(enemy_source, response_available, torch.zeros_like(response_available))
    dist_to_tgt = d0[:, tgt].transpose(0, 1)  # [C, P]
    turns_to_tgt = dist_to_tgt / source_speed.clamp(min=1e-6).view(1, P)
    k_arr = torch.ceil(cand_eta[:, 0].to(dtype)).clamp(min=1.0)
    response_deadline = k_arr + float(config.hold_predictor_window)
    can_respond = turns_to_tgt <= response_deadline.view(-1, 1)
    enemy_response = torch.where(
        can_respond,
        response_available.view(1, P).expand_as(turns_to_tgt),
        torch.zeros_like(turns_to_tgt),
    ).sum(dim=1)
    response_fail = hold_ok & ((enemy_response - min_hold_ships) > float(config.hold_predictor_response_margin))

    weak_hold = hold_ok & (min_hold_ships < 2.0)
    enemy_fail = enemy_target & (~hold_ok)
    neutral_fail = neutral_target & (~hold_ok)
    penalty = torch.zeros_like(score)
    penalty = penalty + torch.where(
        enemy_fail,
        torch.full_like(score, float(config.hold_predictor_enemy_penalty)),
        torch.zeros_like(score),
    )
    penalty = penalty + torch.where(
        neutral_fail,
        torch.full_like(score, float(config.hold_predictor_neutral_penalty)),
        torch.zeros_like(score),
    )
    penalty = penalty + torch.where(
        weak_hold & enemy_target,
        torch.full_like(score, 0.35),
        torch.zeros_like(score),
    )
    penalty = penalty + torch.where(
        response_fail & enemy_target,
        ((enemy_response - min_hold_ships).clamp(min=0.0) / 45.0).clamp(max=1.4),
        torch.zeros_like(score),
    )
    penalty = penalty + torch.where(
        response_fail & neutral_target,
        ((enemy_response - min_hold_ships).clamp(min=0.0) / 65.0).clamp(max=0.7),
        torch.zeros_like(score),
    )
    hard_block = (enemy_fail | (response_fail & enemy_target)) & (cand_eta[:, 0].to(score.dtype) >= 4.0)
    return torch.where(hard_block, torch.full_like(score, float("-inf")), score - penalty)
...
def plan_lite_waves(
    *,
    ...
    score = _apply_orbit_lite_hold_predictor(
        score=score,
        cand_send=cand_send,
        cand_eta=cand_eta,
        cand_tgt_slot=cand_tgt_slot,
