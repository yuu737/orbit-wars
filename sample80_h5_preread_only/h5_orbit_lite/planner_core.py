...
def competitive_score(diff: GarrisonFlowDiff, *, player_id: int) -> Tensor:
    """Competitive score: ``Δnet_me ∁Emax_opp Δnet_opp``.

    ``diff.net_ship_delta`` is ``[*prefix, A]`` (per-player change in net ships
    gained = produced ∁Elost-to-combat); returns ``[*prefix]``.
    """
...
# counts / proximity that tie constantly  Eso device-stable selection is what
...
def _candidate_indices(values: Tensor, mask: Tensor, cap: int) -> tuple[Tensor, Tensor]:
    ...
    Device-stable (ascending-index tie-break)  Esee note above.
    """
...
def reinforcement_timing_factor(
    ...
    *,
    ...
    """Reaction-likelihood ramp ``ρEeta) ∁E[0, 1]`` for reinforcement risk.

    ``ρE= clamp((eta ∁Eeta_free) / eta_scale, 0, 1)``. Below ``eta_free`` turns of
    flight the enemy has no time to react (ρE0); over the next ``eta_scale`` turns
    reaction likelihood ramps linearly to 1. Pure arithmetic ↁECPU/CUDA agree.
    """
...
def capture_floor(
    ...
    *,
    ...
    - If I **own** the target at ``k`` (reinforcement), the floor is 1  Earriving
    ...
    - ``reinforcement`` (optional, ``[T, K' ≥ K]``) is added to the defender count
      before the ceil on capture cells (not on ``mine_at_k`` reinforcement cells)  E      the ETA-aware reactive-reinforcement margin. ``None`` ⇁Etoday's behaviour.
...
def friendly_flip_targets(
    ...
    Returns ``(mask [P] bool, urgency [P] float)``. ``urgency`` ≁Eprojected
    ships lost if unaddressed = ``prod·(H ∁Eflip_turn) + garrison_now``  Esame ship
...
def reachable_mask(
    ...
    *,
    ...
    Hence ``viable => reachable`` (the ``eps`` absorbs fp32 boundary noise)  Ethe gate
    ...
    # Point-to-segment distance from (sx,sy) to segment [(ax,ay),(bx,by)] ↁE[S,K,T].
    abx = bx - ax; aby = by - ay
    apx = sx - ax; apy = sy - ay
...
def safe_drain(
    garrison_status: PlanetGarrisonStatus,
    *,
    source_idx: Tensor,            # [S] long  Eplanet slots to evaluate
    source_ships: Tensor,          # [S] float  Ecurrent garrison at those slots
    H_eff: Tensor,                 # scalar float  Ehorizon to protect the source over
    player_id: int = 0,
) -> Tensor:
    ...
    ``min_t(ships_traj[t])``  Eleaving the planet at 0 ships on the worst held turn
