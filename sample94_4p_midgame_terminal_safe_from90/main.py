...
class ProducerLiteConfig:
    ...
    terminal_4p_roi_threshold: float = 1.3
    terminal_4p_enable_regroup: bool = True
    ...
    top_director_reserve_count: int = 1
    top_director_reserve_multi_roi: float = -0.20
...
def _apply_phase_config(config: ProducerLiteConfig, step: int, *, player_count: int = 2) -> ProducerLiteConfig:
    if int(step) >= TOTAL_STEPS - int(config.terminal_phase_turns):
        if int(player_count) >= 4:
            return dataclasses.replace(
                config,
                roi_threshold=float(config.terminal_4p_roi_threshold),
                max_waves_per_turn=int(config.terminal_max_waves_per_turn),
                enable_regroup=bool(config.terminal_4p_enable_regroup),
                orbit_response_turn_limit=TOTAL_STEPS,
            )
        return dataclasses.replace(
            config,
...
def _append_winner_path_targets(
    *,
    ...
    if int(player_count) < 4 or not bool(config.enable_winner_path_4p):
        return target_idx, target_exists, None
    step = _obs_step(obs_tensors)
    if step > int(config.winner_path_turn_limit):
        return target_idx, target_exists, getattr(memory, "winner_path", None)
    plan = getattr(memory, "winner_path", None)
    if plan is not None and step >= 70:
        planned = [int(t) for t in plan.get("targets", []) if 0 <= int(t) < int(obs.P)]
        if planned:
            owners = obs.owner_abs.to(torch.long)
            enemy_owned = 0
            alive_count = 0
            for t in planned:
                if bool(obs.alive[t].item()):
                    alive_count += 1
                    if int(owners[t].item()) >= 0 and int(owners[t].item()) != int(obs.player_id):
                        enemy_owned += 1
            anchor = int(plan.get("anchor", -1))
            anchor_lost = 0 <= anchor < int(obs.P) and bool(obs.alive[anchor].item()) and int(owners[anchor].item()) >= 0 and int(owners[anchor].item()) != int(obs.player_id)
            if anchor_lost or (alive_count > 0 and enemy_owned * 2 >= alive_count):
                memory.winner_path = None
                plan = None
    if plan is None or step <= 1:
...
def plan_lite_waves(
    *,
    ...
    if bool(reserve_mask.any()):
        reserve_count = max(1, int(config.top_director_reserve_count))
        reserve_roi = float(config.top_director_reserve_roi)
        if reserve_count > 1:
            reserve_roi = max(reserve_roi, float(config.top_director_reserve_multi_roi))
        reserve_entries, source_budget = _greedy_select(
            P=P, W=reserve_count, device=device, dtype=dtype, score=reserve_score,
            cand_src=cand_src, cand_send=cand_send, cand_angle=cand_angle, cand_eta=cand_eta,
            cand_active=cand_active, cand_tgt_slot=cand_tgt_slot, cand_tgt_short=cand_tgt_short,
            cand_is_def=cand_is_def, source_budget=source_budget,
            target_exists=target_exists_for_normal, roi_threshold=reserve_roi,
        )
...
class ProducerLiteRuntime:
    ...
    def tensor_action(self, obs_tensors: dict):
        ...
        config = _apply_phase_config(base, step, player_count=int(mem.cached_player_count))
        row = run_turn(
