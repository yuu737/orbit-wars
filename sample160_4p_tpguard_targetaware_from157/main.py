...
def _reach_by_owner_4p(
    obs, cache, garrison_status, *, horizon, window, player_count, pid, dtype, device,
):
    """Per OPPONENT e and owned planet P, the reachable force of e onto P:
    latent garrison within ``horizon`` (distance/speed-decayed) + in-flight arrivals
    within ``window``. Returns ``[player_count, P]`` (row ``pid`` is 0).

    sample160 (v2): keep this per-opponent (don't collapse to a max here) so the caller
    can take ``max over opponents EXCLUDING the one a wave is attacking`` -- the opponent
    we commit force against has its garrison engaged and cannot retake our source, so
    counting it over-estimates the threat and over-drops launches (the passivity that
    capped sample157/158). The real killer is the *third party* we are NOT attacking."""
    P = int(obs.P)
    if P == 0:
        return torch.zeros(int(player_count), 0, dtype=dtype, device=device)
    d0 = cache.cross_dist[0].to(dtype)
    ships = obs.ships.to(dtype)
    owners = obs.owner_abs.to(torch.long)
    alive = obs.alive
    ...
    contrib = torch.where(base_valid, ships.view(P, 1) * decay, torch.zeros_like(decay))
    ...
    reach = torch.zeros(int(player_count), P, dtype=dtype, device=device)
    for e in range(int(player_count)):
        if e == pid:
            continue
        mask_e = (owners == e) & alive
        latent_e = (contrib * mask_e.view(P, 1)).sum(dim=0)
...
def _global_select_4p(
    *, P, W, device, dtype, score, cand_src, cand_send, cand_angle, cand_eta,
    ...
    target_exists, roi_threshold, reach_by_owner, owner_abs, player_count, pid,
    ships, owned, vuln_weight, margin, drop_sweeps, drop_deadband,
):
    """4P-only global-select (§2 Idea A) -- sample160 v2: TARGET-AWARE.

    Phase 1 reproduces ``_greedy_select`` exactly while recording each wave's score.
    Phase 2 drops a wave when it bares one of *its own* source planets (the planets it
    launches FROM) against the strongest single opponent's reachable force -- but the
    threat for a wave EXCLUDES the opponent that wave is attacking (their garrison is
    engaged and cannot retake our source; counting them over-estimates the threat and
    over-drops, the passivity that capped v1/sample158). The real killer is the third
    party we are NOT attacking. Per-wave own-source attribution (not a global per-planet
    penalty) blames the wave that actually drains a planet. Removes launches only;
    savable-gated. No holdable bared source -> no-op == ``_greedy_select``."""
    L = int(cand_src.shape[1])
    ...
    # ---- Phase 2: TARGET-AWARE per-wave source-vulnerability drop sweep (v2) ----
    lam = float(vuln_weight)
    deadband = float(drop_deadband)
    opp = [e for e in range(int(player_count)) if e != int(pid)]
    fired_idx = [w for w in range(W) if bool(w_fired[w])]

    # per fired-wave: threat EXCLUDING the attacked opponent, savable-gate, own-source mask
    need_w = {}
    holdable_w = {}
    srcmask_w = {}
    for w in fired_idx:
        tgt_planet = int(w_tgt[w, 0])
        a_w = int(owner_abs[tgt_planet]) if 0 <= tgt_planet < P else -1   # attacked opponent (<0 = neutral)
        rows = [e for e in opp if e != a_w]
        mx = reach_by_owner[rows].max(dim=0).values if rows else torch.zeros(P, dtype=dtype, device=device)
        nd = mx + float(margin)
        need_w[w] = nd
        holdable_w[w] = owned & (ships >= nd)
        sm = torch.zeros(P, dtype=torch.bool, device=device)
        active_src = w_src[w][w_active[w]]
        if active_src.numel() > 0:
            sm[active_src.clamp(0, P - 1)] = True
        srcmask_w[w] = sm

    def _committed(kept_mask):
        send = torch.where((kept_mask & w_fired).view(W, 1) & w_active, w_send, torch.zeros_like(w_send))
        c = torch.zeros(P, dtype=dtype, device=device)
        c.scatter_add_(0, w_src.reshape(W * L), send.reshape(W * L))
        return c

    def _total_penalty(kept_mask):
        garrison_after = ships - _committed(kept_mask)
        tot = torch.zeros((), dtype=dtype, device=device)
        for w in fired_idx:
            if not bool(kept_mask[w]):
                continue
            short = (need_w[w] - garrison_after).clamp(min=0.0)
            tot = tot + torch.where(holdable_w[w] & srcmask_w[w], short, torch.zeros_like(short)).sum()
        return tot

    kept = w_fired.clone()
    if lam > 0.0 and bool(w_fired.any()):
        for _ in range(max(1, int(drop_sweeps))):
            ...
            for wi in order.tolist():
                if not bool(kept[wi]):
                    continue
                pen_before = _total_penalty(kept)
                kept2 = kept.clone()
                kept2[wi] = False
                pen_after = _total_penalty(kept2)
    ...
    final_committed = torch.zeros(P, dtype=dtype, device=device)
    final_committed.scatter_add_(0, w_src.reshape(W * L), w_send.reshape(W * L))
    leftover = (source_budget - final_committed).clamp(min=0.0)
...
def plan_lite_waves(
    *,
    ...
    if (int(player_count) >= 4 and bool(config.enable_thirdparty_guard_4p)
        ...
        reach_by_owner = _reach_by_owner_4p(
        ...
        wave_entries, leftover = _global_select_4p(
            ...
            reach_by_owner=reach_by_owner, owner_abs=obs.owner_abs.to(torch.long),
            player_count=int(player_count), pid=pid,
            ships=obs.ships.to(dtype), owned=(obs.owned & obs.alive),
