...
class ProducerLiteConfig:
    ...
    # --- sample175: light pre-score overextension lookahead ---
    # Unlike sample171, this is not a target holdability filter. It only nudges down
    # launches that leave their source exposed to third-party pressure before the
    # final third-party guard does its hard drop.
    enable_light_overextend_4p: bool = False
    light_overextend_start: int = 15
    light_overextend_limit: int = 170
    light_overextend_window: float = 8.0
    light_overextend_margin: float = 4.0
    light_overextend_source_weight: float = 0.055
    light_overextend_target_weight: float = 0.010
    light_overextend_max_penalty: float = 1.10
...
def _apply_light_overextend_4p(
    *,
    score: Tensor,
    cand_src: Tensor,
    cand_send: Tensor,
    ...
    if int(player_count) < 4 or not bool(config.enable_light_overextend_4p):
        return score
    step = int(float(obs.step.reshape(-1)[0].item()))
    if step < int(config.light_overextend_start) or step > int(config.light_overextend_limit):
        return score
    P = int(obs.P)
    if P <= 0 or score.numel() == 0:
        return score
    ...
    pressure = cheap_enemy_pressure(
        obs, cache, horizon=max(float(config.light_overextend_window), float(config.horizon)), player_id=pid
    ).to(dtype)
    src = cand_src[:, 0].clamp(0, max(P - 1, 0))
    tgt = cand_tgt_slot.clamp(0, max(P - 1, 0))
    attack_target = active & (obs.is_neutral[tgt] | obs.is_enemy[tgt])

    primary_send = cand_send[:, 0].to(dtype)
    source_after = obs.ships[src].to(dtype) - primary_send
    source_hold = source_after + prod[src].to(dtype) * float(config.light_overextend_window)
    source_excess = (
        pressure[src] + float(config.light_overextend_margin) - source_hold
    ).clamp(min=0.0)
    source_penalty = source_excess * float(config.light_overextend_source_weight)

    target_send = cand_send.to(dtype).sum(dim=-1)
    target_after = (target_send - obs.ships[tgt].to(dtype)).clamp(min=0.0)
    target_hold = target_after + prod[tgt].to(dtype) * (float(config.light_overextend_window) * 0.65)
    target_excess = (pressure[tgt] - target_hold).clamp(min=0.0)
    target_penalty = target_excess * float(config.light_overextend_target_weight)

    penalty = (source_penalty + target_penalty).clamp(max=float(config.light_overextend_max_penalty))
    return score - torch.where(attack_target & obs.owned[src], penalty, torch.zeros_like(score))
...
def plan_lite_waves(
    *,
    ...
    score = _apply_light_overextend_4p(
        score=score,
        cand_src=cand_src,
        cand_send=cand_send,
        cand_tgt_slot=cand_tgt_slot,
...
CONFIG_4P = dataclasses.replace(
    ...
    enable_light_overextend_4p=True,
    light_overextend_start=15,
    light_overextend_limit=170,
    light_overextend_window=8.0,
    light_overextend_margin=4.0,
    light_overextend_source_weight=0.055,
    light_overextend_target_weight=0.010,
    light_overextend_max_penalty=1.10,
    coord_start_turn=18,
