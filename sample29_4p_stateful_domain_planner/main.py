...
class ProducerLiteConfig:
    ...
    # --- 4P stateful domain planner -------------------------------------
    enable_domain_planner_4p: bool = False
    domain_plan_turns: int = 190
    domain_confidence_min: float = 18.0
    domain_route_targets: int = 7
    domain_support_radius: float = 55.0
    domain_expansion_radius: float = 85.0
    domain_connector_radius: float = 78.0
    domain_anchor_bonus: float = 0.34
    domain_support_bonus: float = 0.24
    domain_connector_bonus: float = 0.16
    domain_expansion_bonus: float = 0.26
    domain_anchor_hold_base: float = 45.0
    domain_anchor_hold_prod: float = 10.0
    domain_hold_penalty: float = 0.62
...
def _make_domain_plan_4p(
    *,
    ...
    """Choose one 4P domain plan from multiple route/anchor candidates."""
    P = int(obs.P)
    ...
    anchor_mask = neutral & (my_eta <= float(K_eta + 8)) & ((prod >= 2.0) | (obs.ships >= 20.0))
    ...
    expansion = neutral.view(1, P) & (d0 <= float(config.domain_expansion_radius)) & (
        (prod.view(1, P) >= 3.0) | (obs.ships.view(1, P) >= 55.0)
    )
    expansion_prod = torch.where(expansion, prod.view(1, P).to(dtype), torch.zeros(P, P, dtype=dtype, device=device)).sum(dim=1)
    expansion_big_count = expansion.sum(dim=1).to(dtype)

    contest_penalty = (my_eta - enemy_eta + 1.5).clamp(min=0.0, max=8.0) * 1.5
    distance_penalty = my_eta * 0.55
    score = (
        prod.to(dtype) * 8.0
        ...
        + support_count * 1.4
        + expansion_prod * 1.2
        + expansion_big_count * 2.0
        - contest_penalty
        - distance_penalty
    )
    score = torch.where(anchor_mask, score, torch.full_like(score, float("-inf")))
    anchor = int(torch.argmax(score).item())
    confidence = float(score[anchor].item()) if torch.isfinite(score[anchor]) else float("-inf")
    if confidence < float(config.domain_confidence_min):
        return None

    support_score = torch.where(
        local[anchor],
        prod.to(dtype) * 6.0 + obs.ships.to(dtype) * 0.035 - d0[anchor] * 0.055,
        torch.full((P,), float("-inf"), dtype=dtype, device=device),
    )
    route_cap = max(1, min(int(config.domain_route_targets), P))
    ...
    connector_mask = (
        neutral
        & (d0[anchor] <= float(config.domain_connector_radius))
        & (center_dist < center_dist[anchor] - 4.0)
        & ((prod >= 2.0) | (obs.ships >= 20.0))
    )
    connector_score = torch.where(
        connector_mask,
        prod.to(dtype) * 4.5 + obs.ships.to(dtype) * 0.03 - d0[anchor] * 0.05,
        torch.full((P,), float("-inf"), dtype=dtype, device=device),
    )
    connector: list[int] = []
    if bool(torch.isfinite(connector_score).any()):
        connector_idx, connector_exists = _candidate_indices(connector_score, torch.isfinite(connector_score), min(3, P))
        connector = [int(x) for x in connector_idx[connector_exists].detach().cpu().tolist()]
    ...
    expansion_mask = neutral & ~support_mask & (d0[anchor] <= float(config.domain_expansion_radius)) & (
        (prod >= 3.0) | (obs.ships >= 55.0)
    )
    expansion_score = torch.where(
        expansion_mask,
        prod.to(dtype) * 7.0 + obs.ships.to(dtype) * 0.04 - d0[anchor] * 0.04,
        torch.full((P,), float("-inf"), dtype=dtype, device=device),
    )
    expansion_list: list[int] = []
    if bool(torch.isfinite(expansion_score).any()):
        expansion_idx, expansion_exists = _candidate_indices(expansion_score, torch.isfinite(expansion_score), min(4, P))
        expansion_list = [int(x) for x in expansion_idx[expansion_exists].detach().cpu().tolist()]

    return {
        "mode": "domain",
        "anchor": anchor,
        "support": list(dict.fromkeys(support)),
        "connector": list(dict.fromkeys(connector)),
...
def _domain_plan_targets(
    *,
    ...
    K_eta: int,
    player_count: int,
) -> tuple[Tensor | None, Tensor | None, Tensor | None, Tensor | None, int | None, str, float, float]:
    if int(player_count) < 4 or not bool(config.enable_domain_planner_4p):
        return None, None, None, None, None, "free", 0.0, 0.0

    step = _obs_step(obs_tensors)
    if step > int(config.domain_plan_turns):
        return None, None, None, None, None, "free", 0.0, 0.0
    ...
    phase = "claim" if step < 45 else ("consolidate" if step < 105 else ("project" if step < 190 else "free"))
    plan["phase"] = phase

    support = [int(t) for t in plan.get("support", []) if 0 <= int(t) < P and bool(obs.alive[int(t)])]
    connector = [int(t) for t in plan.get("connector", []) if 0 <= int(t) < P and bool(obs.alive[int(t)])]
    ...
    hold = float(config.domain_anchor_hold_base) + float(config.domain_anchor_hold_prod) * anchor_prod
    mature = 1.0 if anchor_owned and anchor_ships >= hold else 0.0

    targets = list(support)
    if phase == "project" and mature > 0.0:
        targets.extend(connector)
        targets.extend(expansion)
    targets = list(dict.fromkeys(targets))

    def tensor_or_none(values: list[int]) -> Tensor | None:
        return torch.tensor(values, dtype=torch.long, device=obs.device) if values else None

    return (
        tensor_or_none(targets),
        tensor_or_none(support),
        tensor_or_none(connector),
        tensor_or_none(expansion),
        anchor,
        phase,
        mature,
...
def _append_domain_targets(
    ...
    extras = [int(x) for x in domain_targets.detach().cpu().tolist() if int(x) not in existing]
    if not extras:
        return target_idx, target_exists
...
def _apply_domain_score_adjustment(
    *,
    score: Tensor,
    cand_valid: Tensor,
    cand_src: Tensor,
    ...
    connector_targets: Tensor | None,
    expansion_targets: Tensor | None,
    anchor_slot: int | None,
    phase: str,
    anchor_mature: float,
) -> Tensor:
    if int(player_count) < 4 or domain_targets is None or domain_targets.numel() == 0:
        return score
    ...
    def mask_from(values: Tensor | None) -> Tensor:
        mask = torch.zeros(P, dtype=torch.bool, device=score.device)
        if values is not None and values.numel() > 0:
            mask[values.clamp(0, P - 1)] = True
        return mask

    support_mask = mask_from(support_targets)
    connector_mask = mask_from(connector_targets)
    expansion_mask = mask_from(expansion_targets)

    adjustment = torch.zeros_like(score)
    if phase == "claim":
        adjustment += torch.where(support_mask[tgt] & cand_valid, torch.full_like(score, float(config.domain_anchor_bonus)), torch.zeros_like(score))
    elif phase == "consolidate":
        adjustment += torch.where(support_mask[tgt] & cand_valid, torch.full_like(score, float(config.domain_support_bonus)), torch.zeros_like(score))
    elif phase == "project" and anchor_mature > 0.0:
        adjustment += torch.where(connector_mask[tgt] & cand_valid, torch.full_like(score, float(config.domain_connector_bonus)), torch.zeros_like(score))
        adjustment += torch.where(expansion_mask[tgt] & cand_valid, torch.full_like(score, float(config.domain_expansion_bonus)), torch.zeros_like(score))

    if anchor_slot is not None and 0 <= int(anchor_slot) < P:
        anchor = int(anchor_slot)
        hold = float(config.domain_anchor_hold_base) + float(config.domain_anchor_hold_prod) * float(prod[anchor].item())
        anchor_under_hold = bool(obs.owned[anchor]) and float(obs.ships[anchor].item()) < hold
        if anchor_under_hold:
            from_anchor = (src == anchor) & cand_valid & (cand_send.squeeze(-1) > 0)
            adjustment -= torch.where(from_anchor, torch.full_like(score, float(config.domain_hold_penalty)), torch.zeros_like(score))
...
def plan_lite_waves(
    *,
    ...
    target_idx, target_exists = _append_domain_targets(target_idx, target_exists, domain_targets)
    ...
    score = _apply_domain_score_adjustment(
        score=score,
...
class ProducerLiteMemory:
    ...
    def __init__(self) -> None:
        ...
        self.domain_plan: Optional[dict[str, Any]] = None
