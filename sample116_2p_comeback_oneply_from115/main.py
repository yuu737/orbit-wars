...
class ProducerLiteConfig:
    ...
    enable_coord_followup_2p: bool = False
    coord_one_ply_gate_2p: bool = False
...
def _coord_followup_enabled(config: ProducerLiteConfig, player_count: int) -> bool:
    if int(player_count) >= 4:
        return bool(config.enable_coord_followup_4p)
    return bool(config.enable_coord_followup_2p)


def _coord_one_ply_gate_enabled(config: ProducerLiteConfig, player_count: int) -> bool:
    if int(player_count) >= 4:
        return bool(config.coord_one_ply_gate_4p)
    return bool(config.coord_one_ply_gate_2p)


def _true_one_ply_enabled(config: ProducerLiteConfig, player_count: int) -> bool:
    if int(player_count) >= 4:
        return bool(config.enable_true_one_ply_4p)
    return bool(config.enable_true_one_ply_2p)
...
def _plan_coord_followups(
    *,
    ...
    if not _coord_followup_enabled(config, player_count):
...
def _coord_one_ply_pass(
    *,
    ...
    if not _coord_one_ply_gate_enabled(config, player_count):
        return True
    P = int(obs.P)
...
def _apply_true_one_ply_rescore(
    *,
    ...
    if not _true_one_ply_enabled(config, player_count):
        return score
    step = int(float(obs.step.reshape(-1)[0].item()))
    ...
    if int(player_count) < 4:
        ...
        owners = obs.owner_abs.to(torch.long)
        pid = int(obs.player_id)
        my_mask = obs.alive & (owners == pid)
        enemy_mask = obs.alive & (owners >= 0) & (owners != pid)
        my_prod = prod[my_mask].to(score.dtype).sum() if bool(my_mask.any()) else torch.zeros((), dtype=score.dtype, device=score.device)
        enemy_prod = prod[enemy_mask].to(score.dtype).sum() if bool(enemy_mask.any()) else torch.zeros((), dtype=score.dtype, device=score.device)
        my_power = obs.ships[my_mask].to(score.dtype).sum() + my_prod * 13.0 if bool(my_mask.any()) else torch.zeros((), dtype=score.dtype, device=score.device)
        enemy_power = obs.ships[enemy_mask].to(score.dtype).sum() + enemy_prod * 13.0 if bool(enemy_mask.any()) else torch.zeros((), dtype=score.dtype, device=score.device)
        if int(getattr(obs, "F", 0)) > 0:
            my_fleet = obs.f_alive & (obs.f_owner.to(torch.long) == pid)
            enemy_fleet = obs.f_alive & (obs.f_owner.to(torch.long) >= 0) & (obs.f_owner.to(torch.long) != pid)
            if bool(my_fleet.any()):
                my_power = my_power + obs.f_ships[my_fleet].to(score.dtype).sum()
            if bool(enemy_fleet.any()):
                enemy_power = enemy_power + obs.f_ships[enemy_fleet].to(score.dtype).sum()
        if enemy_prod <= my_prod + 1.0 and enemy_power <= my_power * 1.12:
            return score
    active = cand_active.any(dim=-1) & torch.isfinite(score)
    if not bool(active.any()):
        return score
...
CONFIG_2P_BASE = _ow_apply_config_overrides(ProducerLiteConfig(), _OW_STRATEGY_PARAMS.get("config_2p"))
CONFIG_2P = dataclasses.replace(
    CONFIG_2P_BASE,
    enable_true_one_ply_2p=True,
    true_one_ply_start=20,
    true_one_ply_turn_limit=190,
    true_one_ply_top_k=18,
    true_one_ply_enemy_frac=0.66,
    true_one_ply_enemy_eta_cap=16.0,
    true_one_ply_enemy_reserve_base=6.0,
    true_one_ply_enemy_reserve_prod=1.8,
    true_one_ply_base_weight=0.88,
    true_one_ply_net_scale=0.025,
    true_one_ply_good_bonus=0.30,
    true_one_ply_bad_penalty=0.55,
    true_one_ply_bad_net=-18.0,
    true_one_ply_good_net=24.0,
    true_one_ply_hard_bad_net=-60.0,
    enable_coord_followup_2p=False,
    coord_start_turn=20,
    coord_turn_limit=185,
    coord_max_extra=2,
    coord_eta_delta=5.0,
    coord_min_leftover=14.0,
    coord_source_reserve_base=8.0,
    coord_source_reserve_prod=2.0,
    coord_send_frac=0.34,
    coord_send_cap=38.0,
    coord_min_target_prod=3.0,
    coord_one_ply_gate_2p=False,
    coord_one_ply_min_target_prod=3.0,
    coord_one_ply_top_sources=6,
    coord_one_ply_min_net=3.0,
    coord_one_ply_hard_min_net=-22.0,
    coord_one_ply_response_ratio_cap=1.35,
)
