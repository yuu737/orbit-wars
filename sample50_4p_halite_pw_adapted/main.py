...
class ProducerLiteConfig:
    ...
    # --- Halite II / Planet Wars-inspired 4P shape: avoid center fights -----
    enable_halite_4p_shape: bool = False
    halite_shape_turn_limit: int = 190
    halite_shape_center_radius: float = 27.0
    halite_shape_outer_radius: float = 34.0
    halite_shape_center_penalty: float = 0.16
    halite_shape_outer_bonus: float = 0.11
    halite_shape_prod_bonus: float = 0.018
    halite_shape_pressure_penalty: float = 0.06
...
def _apply_halite_4p_shape_adjustment(
    *,
    score: Tensor,
    cand_tgt_slot: Tensor,
    cand_active: Tensor,
    ...
    if int(player_count) < 4 or not bool(config.enable_halite_4p_shape):
        return score
    step = _obs_step(obs_tensors)
    if step < 50:
        return score
    if step > int(config.halite_shape_turn_limit):
        return score
    ...
    attack = active & (obs.is_neutral[tgt] | obs.is_enemy[tgt])
    center = attack & (radius[tgt] <= float(config.halite_shape_center_radius))
    outer = attack & (radius[tgt] >= float(config.halite_shape_outer_radius))
    high_prod = attack & (prod[tgt] >= 3.0)

    bonus = torch.zeros_like(score)
    bonus = bonus + torch.where(
        outer,
        torch.full_like(score, float(config.halite_shape_outer_bonus)),
        torch.zeros_like(score),
    )
    bonus = bonus + torch.where(
        high_prod,
        (prod[tgt].to(score.dtype) * float(config.halite_shape_prod_bonus)).clamp(max=0.22),
        torch.zeros_like(score),
    )
    penalty = torch.where(
        center,
        torch.full_like(score, float(config.halite_shape_center_penalty)),
        torch.zeros_like(score),
    )
    if enemy_mass is not None:
        pressure = enemy_mass[tgt].to(score.dtype)
        pressure_cost = (pressure / 60.0).clamp(max=1.4) * float(config.halite_shape_pressure_penalty)
        penalty = penalty + torch.where(attack, pressure_cost, torch.zeros_like(score))
...
def plan_lite_waves(
    *,
    ...
    score = _apply_halite_4p_shape_adjustment(
        score=score,
        cand_tgt_slot=cand_tgt_slot,
        cand_active=cand_active,
        obs=obs,
