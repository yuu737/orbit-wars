...
def _choose_4p_mode(obs_tensors: dict) -> str:
    ...
    # General lane rescue A:
    # low/high65, cheap-rich outer lane where s7_stable often scatters instead
    # of forming the outer factory quickly enough.
    if (
        outer_anchor >= 58.0
        and support_density >= 11.0
        and chain >= 42.0
        and enemy_dist >= 45.0
        and high65 <= 6.0
        and mid_cheap >= 4.0
    ...
    # General lane rescue B:
    # expensive but strong outer anchor/support cluster. These need a committed
    # lane plan even when the cheap-neutral picture is not attractive.
    if (
        outer_anchor >= 78.0
        and support_density >= 22.0
        and chain >= 46.0
        and enemy_dist >= 58.0
        and high65 >= 8.0
        and near_prod <= 9.0
