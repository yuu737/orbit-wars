...
def _make_domain_factory_plan(
    *,
    ...
    bridge_sorted = sorted(
        list(dict.fromkeys([int(t) for t in connectors_sorted] + [int(t) for t in supports_sorted])),
        key=lambda j: (
...
def _append_domain_factory_targets(
    *,
    ...
    if step < 45:
        return target_idx, target_exists, plan
    if bool(config.enable_winner_path_4p) and step < 62:
        return target_idx, target_exists, None
    ...
    if step < 55 and bridge_targets:
        plan_targets = bridge_targets[:6] + anchor_target
...
def _apply_domain_factory_adjustment(
    *,
    ...
    selected_bridge = bridge_mask[tgt] & active & (step < 62)
    ...
    selected_any = selected_core | selected_support | selected_bridge | selected_expansion
    target_bonus = target_bonus + torch.where(
        selected_any,
...
CONFIG_4P_DOMAIN_FACTORY = dataclasses.replace(
    ...
    max_offensive_targets=17,
    max_defensive_targets=2,
    max_waves_per_turn=8,
    roi_threshold=1.40,
    min_ships_to_launch=4.0,
    reinforce_size_beta=2.35,
    reinforce_eta_free=3.0,
    reinforce_eta_scale=12.0,
    enable_regroup=True,
    max_regroup_time=7.0,
    regroup_pressure_delta_min=0.14,
    ...
    domain_factory_project_turn=78,
    domain_factory_max_extra_targets=16,
    ...
    domain_factory_drain_penalty=0.08,
    domain_factory_regroup_core_bonus=7.0,
    domain_factory_regroup_anchor_bonus=14.0,
    domain_factory_regroup_expansion_bonus=6.0,
)
