...
class ProducerLiteConfig:
    ...
    # --- sample131: active defensive reinforcement of predicted-loss planets ---
    # The threat reserve (sample130) keeps ships home on planets a planet can still
    # hold by itself. This complementary, disjoint mechanism handles owned planets
    # the no-action projection says will FLIP even at full garrison: reinforce them
    # from a neighbour in a pre-offense pass, so a base about to be lost is saved
    # before offense drains its neighbours. Selects only existing defensive
    # candidates (target is an owned, flip-predicted, worth-saving planet); it does
    # not fabricate score and is gated to genuine projected losses, so it does not
    # cause broad passivity.
    enable_defense_reinforce_4p: bool = False
    defense_reinforce_window: int = 5
    defense_reinforce_max_waves: int = 3
    defense_reinforce_min_prod: float = 2.0
    defense_reinforce_roi: float = -0.5
    defense_reinforce_start: int = 0
    defense_reinforce_limit: int = 200
...
def _defensive_reinforce_score(
    *,
    score: Tensor,
    cand_tgt_slot: Tensor,
    cand_is_def: Tensor,
    ...
    """Expose only reinforcement waves to owned planets that the no-action
    projection says will flip to an enemy within ``defense_reinforce_window`` steps.

    Returns a copy of ``score`` masked to ``-inf`` everywhere except those defensive
    candidates, so a pre-offense greedy pass can reinforce a base about to be lost
    before offense drains its neighbours. Returns all ``-inf`` when disabled or when
    no owned planet is projected to flip. Complementary to the sample130 reserve,
    which covers planets that can still hold by themselves (disjoint by construction:
    a flip-predicted planet cannot be saved by holding its own garrison alone).
    """
    neg_inf = torch.full_like(score, float("-inf"))
    P = int(obs.P)
    if int(player_count) < 4 or not bool(config.enable_defense_reinforce_4p) or P <= 0:
        return neg_inf
    step = _obs_step(obs_tensors)
    if step < int(config.defense_reinforce_start) or step > int(config.defense_reinforce_limit):
        return neg_inf
    owner = getattr(garrison_status, "owner", None)
    if owner is None:
        return neg_inf
    pid = int(obs.player_id)
    H = int(owner.shape[-1]) - 1  # owner time axis is [P, H+1]; k=0 is current
    if H <= 0:
        return neg_inf
    W = max(1, min(int(config.defense_reinforce_window), H))

    owner_l = owner.to(torch.long)
    future_flips = (owner_l[:, 1 : W + 1] != pid).any(dim=1)  # [P]
    worth = prod.to(score.dtype) >= float(config.defense_reinforce_min_prod)
    flip_pred = obs.owned & obs.alive & future_flips & worth  # [P]
    if not bool(flip_pred.any()):
        return neg_inf

    tgt_planet = cand_tgt_slot.clamp(0, max(P - 1, 0))
    def_cand = cand_is_def & flip_pred[tgt_planet] & cand_active.any(dim=-1) & torch.isfinite(score)
    return torch.where(def_cand, score, neg_inf)
...
def plan_lite_waves(
    *,
    ...
    # sample131: pre-offense defensive reinforcement of predicted-loss planets.
    defense_entries = _empty_entries(device, dtype)
    defense_score = _defensive_reinforce_score(
        score=score,
        cand_tgt_slot=cand_tgt_slot,
    ...
    if bool(torch.isfinite(defense_score).any()):
        defense_entries, source_budget = _greedy_select(
            P=P, W=max(1, int(config.defense_reinforce_max_waves)), device=device, dtype=dtype,
            score=defense_score,
            cand_src=cand_src, cand_send=cand_send, cand_angle=cand_angle, cand_eta=cand_eta,
            cand_active=cand_active, cand_tgt_slot=cand_tgt_slot, cand_tgt_short=cand_tgt_short,
            cand_is_def=cand_is_def, source_budget=source_budget,
            target_exists=target_exists_for_normal, roi_threshold=float(config.defense_reinforce_roi),
        )
        if bool(defense_entries.valid.any()):
            dtaken = defense_entries.target_slots[defense_entries.valid].clamp(0, max(P - 1, 0))
            if dtaken.numel() > 0:
                dtaken_mask = torch.zeros(P, dtype=torch.bool, device=device)
                dtaken_mask.scatter_(0, dtaken, True)
                target_exists_for_normal = target_exists_for_normal & (~dtaken_mask[target_idx.clamp(0, P - 1)])
    ...
    if not bool(config.enable_regroup):
        return concat_launch_entries([reserve_entries, defense_entries, wave_entries, coord_entries])
    ...
    return concat_launch_entries([reserve_entries, defense_entries, wave_entries, coord_entries, regroup_entries])
...
CONFIG_4P = dataclasses.replace(
    ...
    # sample131: active defensive reinforcement of predicted-loss planets (all 4P modes).
    enable_defense_reinforce_4p=True,
    defense_reinforce_window=5,
    defense_reinforce_max_waves=3,
    defense_reinforce_min_prod=2.0,
    defense_reinforce_roi=-0.5,
    defense_reinforce_start=0,
    defense_reinforce_limit=200,
)
