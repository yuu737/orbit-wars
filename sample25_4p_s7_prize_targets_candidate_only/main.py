...
class ProducerLiteConfig:
    ...
    # --- 4P first-place prize targeting ----------------------------------
    enable_prize_targets_4p: bool = False
    prize_max_extra_targets: int = 6
    prize_bonus: float = 0.0
    prize_behind_bonus: float = 0.0
    prize_turn_limit: int = 260
...
def _append_4p_prize_targets(
    *,
    ...
    """Add high-upside 4P targets that can swing first place.

    This keeps sample7's exact scorer in charge; it only makes sure valuable
    reachable prizes are present in the candidate set.
    """
    if int(player_count) < 4 or not bool(config.enable_prize_targets_4p):
        return target_idx, target_exists, None
    if _obs_step(obs_tensors) > int(config.prize_turn_limit):
        return target_idx, target_exists, None

    P = int(obs.P)
    if P <= 0 or int(config.prize_max_extra_targets) <= 0:
        return target_idx, target_exists, None

    device = obs.device
    dtype = obs.ships.dtype
    attackable = obs.alive & ~obs.owned
    if not bool(attackable.any()) or not bool(source_mask.any()):
        return target_idx, target_exists, None
    ...
    reachable = eta_from_me <= float(K_eta)
    high_value = (prod >= 3.0) | (obs.ships >= 45.0)
    thin_enemy_prod = obs.is_enemy & (prod >= 3.0) & (obs.ships <= prod * 12.0 + 30.0)
    prize_mask = attackable & reachable & (high_value | thin_enemy_prod)

    if not bool(prize_mask.any()):
        return target_idx, target_exists, None
    ...
    prize_mask = prize_mask & ~existing
    if not bool(prize_mask.any()):
        return target_idx, target_exists, None

    enemy_owned_bonus = torch.where(obs.is_enemy, torch.full((P,), 3.0, dtype=dtype, device=device), torch.zeros(P, dtype=dtype, device=device))
    neutral_bonus = torch.where(obs.is_neutral, torch.full((P,), 1.2, dtype=dtype, device=device), torch.zeros(P, dtype=dtype, device=device))
    prize_score = (
        prod.to(dtype) * 7.5
        + obs.ships.to(dtype) * 0.06
        + enemy_owned_bonus
        + neutral_bonus
        - eta_from_me * 0.42
    )
    prize_score = torch.where(prize_mask, prize_score, torch.full_like(prize_score, float("-inf")))

    extra_idx, extra_exists = _candidate_indices(
        prize_score,
        torch.isfinite(prize_score),
        max(1, min(int(config.prize_max_extra_targets), P)),
    )
    ...
    merged_idx = torch.cat([target_idx, extras], dim=0)
    merged_exists = torch.cat([target_exists, extra_exists_full], dim=0)
    return merged_idx, merged_exists, extras


def _apply_4p_prize_bonus(
    *,
    score: Tensor,
    cand_valid: Tensor,
    ...
    config: ProducerLiteConfig,
    player_count: int,
    prize_targets: Tensor | None,
) -> Tensor:
    if int(player_count) < 4 or prize_targets is None or prize_targets.numel() == 0:
        return score
    if float(config.prize_bonus) <= 0.0 and float(config.prize_behind_bonus) <= 0.0:
        return score
    P = int(obs.P)
    target_mask = torch.zeros(P, dtype=torch.bool, device=score.device)
    target_mask[prize_targets.clamp(0, P - 1)] = True
    tgt = cand_tgt_slot.clamp(0, P - 1)
    selected = target_mask[tgt] & cand_valid

    my_prod = prod[obs.owned].sum() if bool(obs.owned.any()) else torch.tensor(0.0, dtype=score.dtype, device=score.device)
    enemy_prod = torch.zeros((), dtype=score.dtype, device=score.device)
    for owner_id in torch.unique(obs.owner_abs[obs.is_enemy].long()) if bool(obs.is_enemy.any()) else []:
        owner_mask = obs.owner_abs.long() == owner_id
        enemy_prod = torch.maximum(enemy_prod, prod[owner_mask].sum().to(score.dtype))
    behind = enemy_prod > my_prod + 3.0

    base_bonus = float(config.prize_bonus) + (float(config.prize_behind_bonus) if bool(behind) else 0.0)
    target_prod_bonus = (prod[tgt].to(score.dtype) * 0.035).clamp(max=0.25)
    bonus = torch.full_like(score, base_bonus) + target_prod_bonus
...
def plan_lite_waves(
    *,
    ...
    target_idx, target_exists, prize_targets = _append_4p_prize_targets(
