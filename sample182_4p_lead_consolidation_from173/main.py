...
class ProducerLiteConfig:
    ...
    # --- sample182: 4P lead-gated consolidation ---
    # Competition (replays_my_4p_176): with the expansion drive we now EXPAND (lead at t60-80,
    # 10 planets) but then bleed the won position via continuous source-stripping (91 events)
    # while one rival snowballs. Only when we are the CLEAR production leader, penalize
    # offensive launches that bare their own source below a safe reserve -> consolidate and
    # hold the lead. Lead-gated so it does not make us passive when behind (sample173 keeps
    # its aggression there); off by default.
    enable_lead_consolidation_4p: bool = False
    lead_consolidation_margin: float = 1.15      # my_prod >= max_opp_prod * this => leading
    lead_consolidation_reserve_base: float = 10.0
    lead_consolidation_reserve_prod: float = 2.0
    lead_consolidation_penalty: float = 0.6
    lead_consolidation_start: int = 40
...
def _apply_lead_consolidation_4p(
    *,
    score: Tensor,
    cand_src: Tensor,
    cand_send: Tensor,
    ...
    """4P lead-gated consolidation: when we are the clear production leader, penalize
    offensive launches that bare their own source below a safe reserve, so we hold a won
    position instead of bleeding it via source-stripping. No effect when behind (keeps
    sample173's aggression) -> avoids the blanket-defense passivity of sample131/135."""
    if int(player_count) < 4 or not bool(config.enable_lead_consolidation_4p):
        return score
    step = int(float(obs.step.reshape(-1)[0].item()))
    if step < int(config.lead_consolidation_start):
        return score
    P = int(obs.P)
    if P <= 0 or score.numel() == 0:
        return score
    ...
    owners = obs.owner_abs.to(torch.long)
    alive = obs.alive
    my_prod = prod[obs.owned & alive].to(dtype).sum()
    opp_prod_max = torch.zeros((), dtype=dtype, device=device)
    for owner in range(int(player_count)):
        if owner == pid:
            continue
        op = prod[alive & (owners == int(owner))].to(dtype).sum()
        opp_prod_max = torch.maximum(opp_prod_max, op)
    leading = bool((my_prod > 0.0) and (my_prod >= opp_prod_max * float(config.lead_consolidation_margin)))
    if not leading:
        return score

    active = cand_active.any(dim=-1) & torch.isfinite(score)
    tgt = cand_tgt_slot.clamp(0, max(P - 1, 0))
    offensive = active & (obs.is_enemy[tgt] | obs.is_neutral[tgt])   # not reinforcing our own
    src = cand_src[:, 0].clamp(0, max(P - 1, 0))
    send = cand_send[:, 0].to(dtype)
    source_after = obs.ships[src].to(dtype) - send
    reserve = float(config.lead_consolidation_reserve_base) + prod[src].to(dtype) * float(config.lead_consolidation_reserve_prod)
    shortfall = (1.0 - (source_after / reserve.clamp(min=1.0))).clamp(min=0.0, max=1.0)
    bare = active & obs.owned[src] & (source_after < reserve)
    penalty = torch.where(
        offensive & bare,
        torch.full_like(score, float(config.lead_consolidation_penalty)) * shortfall,
        torch.zeros_like(score),
    )
...
def plan_lite_waves(
    *,
    ...
    score = _apply_lead_consolidation_4p(
        score=score,
        cand_src=cand_src,
        cand_send=cand_send,
        cand_tgt_slot=cand_tgt_slot,
...
CONFIG_4P = dataclasses.replace(
    ...
    enable_lead_consolidation_4p=True,
    lead_consolidation_margin=1.15,
    lead_consolidation_reserve_base=10.0,
    lead_consolidation_reserve_prod=2.0,
    lead_consolidation_penalty=0.6,
    lead_consolidation_start=40,
    coord_start_turn=18,
