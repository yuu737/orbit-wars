...
def _make_outer_lane_sequence_plan(
    *,
    ...
    for anchor in neutral_idx:
        ...
        support_prod = sum(float(prod[j].item()) for j in support)
        ...
        if best is None or score > best[0]:
            best = (score, anchor, claim, support, connector, expansion, lane_angle)

    if best is None or best[0] < 22.0:
        return None

    score, anchor, claim, support, connector, expansion, lane_angle = best
    claim_sorted = sorted(
    ...
    for t in [anchor] + connector_sorted + support_sorted:
        if int(t) not in seen:
    ...
    return {
        ...
        "expansion": unique_expansion,
        "targets": unique_claim + unique_support + unique_expansion,
        "angle": float(lane_angle),
        "created_step": _obs_step(obs_tensors),
        "score": float(score),
    }
...
def _append_outer_lane_sequence_targets(
    *,
    ...
    if phase == "claim":
        ...
        plan_targets = [int(t) for t in plan.get("claim", []) + early_support]
    elif phase == "anchor":
        plan_targets = [int(t) for t in plan.get("claim", []) + plan.get("support", [])]
    elif phase == "project" and mature:
        plan_targets = [int(t) for t in plan.get("claim", []) + plan.get("support", []) + plan.get("expansion", [])]
    elif phase == "project":
        plan_targets = [int(t) for t in plan.get("claim", []) + plan.get("support", [])]
    else:
        return target_idx, target_exists, plan
...
def _apply_outer_lane_sequence_adjustment(
    *,
    ...
    selected_any = selected_claim | selected_support | selected_expansion
    ...
    lane_mask = claim_mask | support_mask | expansion_mask
    offlane_cheap = (
        active
        & obs.is_neutral[tgt]
        & (~lane_mask[tgt])
    ...
    src_core = (claim_mask[src] | support_mask[src]) & obs.owned[src] & active & (step >= int(config.outer_lane_claim_turn))
    src_prod = prod[src].to(score.dtype)
    src_ships = obs.ships[src].to(score.dtype)
...
def plan_lite_waves(
    *,
    ...
    lane_budget_enabled = mode in ("winner_outer_domain", "domain_factory", "lane_anchor")
    lane_waves = (
    ...
    if not bool(config.enable_regroup):
        return concat_launch_entries([lane_entries, wave_entries])
    ...
    return concat_launch_entries([lane_entries, wave_entries, regroup_entries])
