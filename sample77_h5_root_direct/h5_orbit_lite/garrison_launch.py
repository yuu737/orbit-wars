...
agent faces  E*"if I launch these ships, how does each player's net ship flow
(production minus combat losses) change?"*  Eby recomputing the production→combat
...
- ``C``  Ecandidates: the different launches / launch-sets being scored.
- ``L``  Elaunches within a candidate: a candidate *is* a set of launches; ``L``
...
def _run_exact_recurrence(
    *,
    ...
    saved kernels  Ethis is the reference path). Returns
...
class GarrisonFlowDiff:
    ...
    gained (``produced - lost_to_combat``)  Ei.e. how much better/worse off each
