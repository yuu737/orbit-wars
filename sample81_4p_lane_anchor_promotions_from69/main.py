...
def _choose_4p_mode(obs_tensors: dict) -> str:
    ...
    if (
        ...
        return "lane_anchor"
    # sample81: promote more s7_stable losses into lane_anchor when the board has
    # a real outer-factory shape but falls just short of the older hard gates.
    if (
        outer_anchor >= 64.0
        ...
        and mid_prod <= 25.0
        and mid_cheap <= 6.0
        and enemy_cluster_risk <= 118.0
    ):
        return "lane_anchor"
    if (
        planets <= 20.0
        and outer_anchor >= 48.0
