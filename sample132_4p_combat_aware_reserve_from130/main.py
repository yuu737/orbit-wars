...
def _threat_reserve_source_budget(
    *,
    ...
    A = int(arr.shape[-1])
    per_owner = arr.sum(dim=1)  # [P, A] total ships each owner lands over the window
    owner_ids = torch.arange(A, device=device).view(1, A)
    enemy_per_owner = torch.where(owner_ids != pid, per_owner, torch.zeros_like(per_owner))  # [P, A]
    # sample132: combat-aware threat. Multiple enemy owners arriving at the same
    # planet fight EACH OTHER first (engine rule: largest force vs second largest,
    # only the difference survives to fight the garrison). Summing all enemy
    # arrivals over-estimates the threat in 4P brawls and over-reserves -> passive.
    # Use the surviving net hostile force = top1_enemy - top2_enemy (>= 0). For a
    # single inbound enemy this equals its size (top2 = 0), so safety is unchanged;
    # it only relaxes the reserve when enemies cancel each other.
    k_top = min(2, A)
    top_vals, _ = torch.topk(enemy_per_owner, k=k_top, dim=1)  # [P, k_top] descending
    top1 = top_vals[:, 0]
    top2 = top_vals[:, 1] if k_top > 1 else torch.zeros_like(top1)
    enemy_arr = (top1 - top2).clamp(min=0.0)  # [P] net hostile force reaching the garrison
    if pid < A:
