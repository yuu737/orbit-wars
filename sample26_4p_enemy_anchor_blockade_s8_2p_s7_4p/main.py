...
def _append_4p_enemy_anchor_blockade_targets(
    *,
    ...
    soft_enemy_border = obs.is_enemy & (prod >= 3.0) & (obs.ships <= prod * 16.0 + 80.0)
    blockade_mask = attackable & reachable & near_anchor & (useful_body | soft_enemy_border)
    ...
    target_score = (
        ...
        + torch.where(soft_enemy_border, torch.full((P,), 3.0, dtype=dtype, device=device), torch.zeros(P, dtype=dtype, device=device))
        - eta_from_me * 0.35
        - anchor_dist * 0.012
    )
...
CONFIG_4P = replace(
    ...
    blockade_step_start=55,
    blockade_step_end=130,
    blockade_max_extra_targets=6,
    blockade_anchor_ships=95.0,
    blockade_anchor_prod=3.0,
    blockade_neighbor_dist=150.0,
    blockade_eta_cap=18.0,
    blockade_bonus=0.28,
    blockade_anchor_bonus=0.18,
