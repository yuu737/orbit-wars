    ...
    *,
    ...
    for anchor in neutral_idx:
        ...
        if score > 14.0:
            ...
            plans.append((rollout, score, anchor, claim, support, connector, expansion, lane_angle, raw_queue))
...
def _append_outer_lane_sequence_targets(
    *,
    ...
    if phase == "claim":
        ...
        plan_targets = queued[: int(config.outer_lane_queue_width)] + [int(t) for t in plan.get("claim", []) + plan.get("support", [])]
    elif phase == "project" and mature:
        plan_targets = queued[: int(config.outer_lane_queue_width) + 1] + [int(t) for t in plan.get("claim", []) + plan.get("support", []) + plan.get("expansion", [])]
    elif phase == "project":
        plan_targets = queued[: int(config.outer_lane_queue_width)] + [int(t) for t in plan.get("claim", []) + plan.get("support", [])]
    else:
        return target_idx, target_exists, plan
