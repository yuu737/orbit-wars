...
class ProducerLiteConfig:
    ...
    # --- 4P producer-plus: hairate5-style partial commitments, guarded ------
    enable_producer_plus_4p: bool = False
    producer_plus_turn_limit: int = 185
    producer_reserve_base: float = 8.0
    producer_reserve_prod: float = 5.5
    producer_reserve_pressure: float = 0.035
    producer_drain_penalty: float = 0.55
    producer_target_prod_bonus: float = 0.035
    producer_neutral_bonus: float = 0.08
...
def _apply_producer_plus_adjustment(
    *,
    score: Tensor,
    cand_src: Tensor,
    cand_send: Tensor,
    ...
    if int(player_count) < 4 or not bool(config.enable_producer_plus_4p):
        return score
    if int(step) > int(config.producer_plus_turn_limit):
        return score
    if enemy_mass is None:
        pressure = torch.zeros_like(obs.ships.to(score.dtype))
    else:
        pressure = enemy_mass.to(score.dtype)
    ...
    reserve = (
        float(config.producer_reserve_base)
        + src_prod * float(config.producer_reserve_prod)
        + pressure[src] * float(config.producer_reserve_pressure)
    )
    after = src_ships - send
    drain_short = (reserve - after).clamp(min=0.0)
    drain_penalty = (drain_short / 18.0).clamp(max=1.4) * float(config.producer_drain_penalty)

    target_prod = prod[tgt].to(score.dtype)
    neutral_bonus = torch.where(
        obs.is_neutral[tgt] & (target_prod >= 2.0),
        torch.full_like(score, float(config.producer_neutral_bonus)),
        torch.zeros_like(score),
    )
    prod_bonus = (target_prod * float(config.producer_target_prod_bonus)).clamp(max=0.28)
    adjustment = neutral_bonus + prod_bonus - drain_penalty
    return score + torch.where(active, adjustment, torch.zeros_like(score))
...
def plan_lite_waves(
    *,
    ...
    score = _apply_producer_plus_adjustment(
        score=score,
        cand_src=cand_src,
        cand_send=cand_send,
        cand_tgt_slot=cand_tgt_slot,
...
CONFIG_4P_PRODUCER_PLUS = dataclasses.replace(
    CONFIG_4P,
    horizon=13,
    max_sources_per_lane=7,
    max_offensive_targets=13,
    max_defensive_targets=2,
    max_waves_per_turn=7,
    roi_threshold=1.42,
    min_ships_to_launch=4.0,
    reinforce_size_beta=0.9,
    reinforce_eta_free=3.0,
    reinforce_eta_scale=12.0,
    enable_regroup=True,
    max_regroup_time=6.0,
    ...
    max_regroup_sources_per_lane=6,
    max_regroup_targets_per_source=8,
    terminal_phase_turns=25,
    terminal_roi_threshold=1.0,
    terminal_max_waves_per_turn=9,
    terminal_enable_regroup=False,
    size_multipliers=(0.5, 0.75, 1.0),
    enable_producer_plus_4p=True,
)
...
def _choose_4p_mode(obs_tensors: dict) -> str:
    ...
    if (
        ...
        and high65 <= 9.0
        and mid_cheap <= 5.0
        and not (chain >= 56.0 and mid_prod >= 23.0)
    ):
        return "producer_plus"
...
def _config_for(player_count: int, mode: str | None = None) -> ProducerLiteConfig:
    ...
    if mode == "producer_plus":
        return CONFIG_4P_PRODUCER_PLUS
    if mode == "s8_burst":
