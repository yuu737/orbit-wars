    ...
    *,
    ...
    if bool(reserve_mask.any()):
        reserve_entries, source_budget = _greedy_select(
            P=P, W=3, device=device, dtype=dtype, score=reserve_score,
        ...
        # Structural experiment: if the portfolio executor found safe neutral
        # expansion in the opening, spend the remaining greedy pass on territory
        # and defense instead of starting non-essential enemy wars. Very cheap
        # vulture captures are still allowed.
        step_now = _obs_step(obs_tensors)
        if int(player_count) >= 4 and step_now <= 72:
            tgt_all = cand_tgt_slot.clamp(0, max(P - 1, 0))
            enemy_tgt = obs.is_enemy[tgt_all]
            cheap_vulture = enemy_tgt & (obs.ships[tgt_all].to(dtype) <= 8.0) & (prod[tgt_all].to(dtype) >= 2.0)
            suppress = enemy_tgt & (~cheap_vulture) & (~cand_is_def)
            score = torch.where(suppress, torch.full_like(score, float("-inf")), score)
...
CONFIG_4P = dataclasses.replace(
    ...
    top_director_reserve_start=6,
    top_director_reserve_limit=135,
    top_director_reserve_roi=-0.30,
    coord_start_turn=18,
