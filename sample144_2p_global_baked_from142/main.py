...
# sample144: enable the 2P global source-selection pass in CODE, not via
# params.json. evaluate.py / kaggle_environments can exec raw agents without
# __file__, which makes params.json lookup fall back to the project root.
# Keep baseline-equivalent fields at ProducerLiteConfig defaults; only the 2A
# global-select knobs differ, so the experiment isolates the feature.
_CONFIG_2P_BASE = dataclasses.replace(
    ProducerLiteConfig(),
    enable_global_select_2p=True,
    global_select_vuln_weight=1.0,
    global_select_margin=2.0,
    global_select_drop_sweeps=3,
    global_select_drop_deadband=0.0,
)
