...
from h5_orbit_lite.geometry import fleet_speed
from h5_orbit_lite.intercept_aim import intercept_angle
from h5_orbit_lite.movement import MovementConfig, PlanetMovement
from h5_orbit_lite.movement_step import (
...
from h5_orbit_lite.obs import parse_obs
from h5_orbit_lite.distance_cache import build_distance_cache
from h5_orbit_lite.planner_core import (
    _candidate_indices,
    _empty_entries,
    _greedy_select,
    _plan_regroup,
...
from h5_orbit_lite.adapter import single_obs_to_tensor, sparse_action_row_to_moves
...
def _agent_s8() -> ModuleType:
    global _MOD_S8
    if _MOD_S8 is None:
        _MOD_S8 = _load_agent_module("sample8", "_sample79_s8")
    return _MOD_S8
...
def _agent_s7() -> ModuleType:
    global _MOD_S7
    if _MOD_S7 is None:
        _MOD_S7 = _load_agent_module("sample7", "_sample79_s7")
    return _MOD_S7
...
def _agent_4p() -> ModuleType:
    global _MOD_4P
    if _MOD_4P is None:
        _MOD_4P = _load_agent_module("sample69_4p_orbit_lite_response_gate", "_sample79_4p")
    return _MOD_4P
...
def _choose_2p_mode(obs) -> str:
    ...
    if (
        cheap8 >= 6 and highprod8 >= 5 and heavy5 >= 2 and very_heavy8 >= 1
        and len(prod5_near) == 0 and len(prod4_near) == 0
        and 20.0 <= avg_near5_ships <= 26.0
    ):
        return "h5"
    if (
        cheap8 == 4 and highprod8 == 2 and heavy5 == 1 and very_heavy8 >= 2
        and len(prod5_near) == 1 and min_highprod_dist >= 25.0
        and avg_near5_ships <= 24.0
    ):
        return "h5"
    if (
        cheap8 <= 2 and highprod8 == 2 and heavy5 >= 2 and very_heavy8 >= 2
        and len(prod4_near) >= 2 and len(prod5_near) == 0
        and min_highprod_dist <= 18.0 and avg_near5_ships <= 22.0
    ):
        return "h5"

    heavy_value_lane = heavy5 >= 3 and cheap8 <= 3 and highprod8 >= 3
    rich_prod5_lane = (
        len(prod5_near) >= 3 and cheap8 >= 4 and highprod8 >= 3
        and min_highprod_dist <= 20.0 and len(planets) >= 28
    )
    close_prod5_route = len(prod5_near) >= 2 and min_highprod_dist <= 18.0
    low_mass_highprod_trap = cheap8 >= 5 and highprod8 >= 3 and min_highprod_dist <= 16.0 and avg_near5_ships <= 18.5
    mixed_heavy_value_lane = highprod8 >= 3 and heavy5 >= 2 and avg_near5_ships >= 20.0 and len(prod4_near) >= 1
    heavy_anchor_lane = very_heavy8 >= 2 and cheap8 <= 3 and min_highprod_dist <= 25.0
    if heavy_value_lane or rich_prod5_lane or heavy_anchor_lane or close_prod5_route or low_mass_highprod_trap or mixed_heavy_value_lane:
        return "s7"
    return "s8"
