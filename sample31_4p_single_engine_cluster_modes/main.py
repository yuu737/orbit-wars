...
# 4P terrain modes.  These stay inside one engine so Kaggle/local execution
# does not depend on dynamic folder imports, which changed behavior in testing.
CONFIG_4P_BURST = CONFIG_4P
CONFIG_4P_STABLE = dataclasses.replace(
    CONFIG_4P,
    horizon=13,
    max_sources_per_lane=6,
...
CONFIG_4P_DOMAIN = dataclasses.replace(
    CONFIG_4P,
    horizon=15,
    ...
    max_defensive_targets=2,
    max_waves_per_turn=6,
    reinforce_size_beta=1.2,
    max_regroup_targets_per_source=8,
    terminal_phase_turns=25,
    terminal_max_waves_per_turn=7,
...
def _config_for(player_count: int, mode: str | None = None) -> ProducerLiteConfig:
    if int(player_count) < 4:
        return CONFIG_2P
    if mode == "burst":
        return CONFIG_4P_BURST
    if mode == "domain":
        return CONFIG_4P_DOMAIN
    return CONFIG_4P_STABLE
