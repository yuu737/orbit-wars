...
class ProducerLiteConfig:
    ...
    # --- sample130: threat-aware source garrison reservation (early-defense) ---
    # Before offensive wave selection, keep enough garrison on owned planets that
    # face incoming enemy fleets so offense cannot strip a base that is about to be
    # attacked. This is a HARD constraint on source_budget, not a score bonus. It is
    # threat-triggered (only fires when real enemy arrivals are inbound) and
    # savable-gated (no reserve on planets that cannot be held even at full garrison),
    # so it does not make play passive on quiet boards or waste ships on doomed ones.
    enable_threat_reserve_4p: bool = False
    threat_reserve_window: int = 6
    threat_reserve_margin: float = 3.0
    threat_reserve_prod_frac: float = 0.5
    threat_reserve_start: int = 0
    threat_reserve_limit: int = 200
    # --- sample142: 2P global-select post-processing drop (source-stripping guard) ---
    # After the greedy wave selection (the initial solution), drop offensive waves whose
    # cumulative source drain leaves a *holdable* owned planet short against reachable
    # enemy garrison (latent pressure), when the marginal source-vulnerability cost
    # outweighs the wave's competitive score. This is the §2.2-stage-1 local-improvement
    # approximation of a global allocation: it REMOVES launches only (never adds, forces,
    # or rescores) and is savable-gated (only defends planets holdable at full garrison),
    # so it cannot make play passive on quiet boards or waste ships on doomed planets.
    # 2P ONLY: guarded by player_count < 4 AND this flag; 4P is unaffected.
    enable_global_select_2p: bool = False
    global_select_vuln_weight: float = 1.0
    global_select_margin: float = 2.0
    global_select_drop_sweeps: int = 3
    global_select_drop_deadband: float = 0.0
    # --- sample149: combine soft global-select with 2P true-one-ply hard-bad ---
    # Drops only catastrophic 2P candidates after simulating the enemy's best response.
    # It does not add bonuses or rescale surviving candidates.
    enable_true_one_ply_2p: bool = False
    true_one_ply_2p_start: int = 34
    true_one_ply_2p_limit: int = 400
    true_one_ply_2p_hard_bad_net: float = -42.0
    true_one_ply_2p_recapture_ratio: float = 1.25
    true_one_ply_2p_recapture_net: float = -12.0
...
def _apply_true_one_ply_hard_bad_2p(
    *,
    score: Tensor,
    cand_src: Tensor,
    cand_send: Tensor,
    ...
    """2P-only true-one-ply hard-bad exclusion.

    Simulate our candidate plus each enemy's best response, then set only the
    catastrophic candidates to -inf. Surviving candidates keep their original score.
    """
    if int(player_count) >= 4 or not bool(config.enable_true_one_ply_2p):
        return score
    step = int(float(obs.step.reshape(-1)[0].item()))
    if step < int(config.true_one_ply_2p_start) or step > int(config.true_one_ply_2p_limit):
        return score
    active = cand_active.any(dim=-1) & torch.isfinite(score)
    if not bool(active.any()):
        return score
    ...
    me = net[:, pid] if 0 <= pid < int(net.shape[1]) else torch.zeros(N, dtype=dtype, device=device)
    opp = torch.zeros(N, dtype=dtype, device=device)
    for other in range(int(player_count)):
        if other == pid or other >= int(net.shape[1]):
            continue
        opp = opp + net[:, other]
    one_ply_net = me - opp
    ...
    target_enemy = target_owner >= 0
    response_mass = (
        resp_send.masked_fill(~resp_valid, 0.0).sum(dim=1)
        if resp_send.numel() else torch.zeros(N, dtype=dtype, device=device)
    )
    own_mass = own_send.masked_fill(~own_valid, 0.0).sum(dim=1)
    response_ratio = response_mass / own_mass.clamp(min=1.0)

    hard_bad = (
        (one_ply_net <= float(config.true_one_ply_2p_hard_bad_net))
        | (
            target_enemy
            & (response_ratio > float(config.true_one_ply_2p_recapture_ratio))
            & (one_ply_net < float(config.true_one_ply_2p_recapture_net))
        )
    )
    out = score.clone()
    out[idx] = torch.where(hard_bad, torch.full_like(score[idx], float("-inf")), score[idx])
    return out
...
def _threat_reserve_source_budget(
    *,
    ...
    """Reserve garrison on owned planets facing inbound enemy fleets.

    Returns a possibly-reduced copy of ``source_budget`` (per-planet ships the
    offensive planner may spend this turn). Owned planets that would be flipped by
    enemy fleets already in flight keep just enough ships to survive the next
    ``threat_reserve_window`` steps. Planets with no inbound enemy arrivals, and
    planets that cannot be held even at full garrison, are left untouched -- so the
    reservation never makes play passive on quiet boards and never wastes ships
    holding a doomed planet. This is a hard source-side constraint, not a score term.
    """
    if int(player_count) < 4 or not bool(config.enable_threat_reserve_4p):
        return source_budget
    step = _obs_step(obs_tensors)
    if step < int(config.threat_reserve_start) or step > int(config.threat_reserve_limit):
        return source_budget
    ...
    dtype = source_budget.dtype
    device = source_budget.device
    pid = int(obs.player_id)
    H = int(arrivals.shape[-2]) - 1  # arrivals time axis is [P, H+1, A]; k=0 is a zero frame
    if H <= 0:
        return source_budget
    W = max(1, min(int(config.threat_reserve_window), H))

    arr = arrivals[:, 1 : W + 1, :].to(dtype)  # [P, W, A] incoming ships per owner, k=1..W
    A = int(arr.shape[-1])
    owner_ids = torch.arange(A, device=device).view(1, 1, A)
    enemy_arr = torch.where(owner_ids != pid, arr, torch.zeros_like(arr)).sum(dim=(1, 2))  # [P]
    if pid < A:
        my_arr = arr[:, :, pid].sum(dim=1)  # [P]
    else:
        my_arr = torch.zeros(P, dtype=dtype, device=device)

    ships = obs.ships.to(dtype)
    prod_buffer = prod.to(dtype) * float(W) * float(config.threat_reserve_prod_frac)
    margin = float(config.threat_reserve_margin)

    # Ships that must stay home for the planet to survive inbound enemy fleets.
    reserve_need = enemy_arr + margin - my_arr - prod_buffer
    owned = obs.owned & obs.alive
    apply_mask = owned & (enemy_arr > 0.0) & (reserve_need > 0.0) & (reserve_need <= ships)
    reserve = torch.where(apply_mask, reserve_need, torch.zeros_like(ships)).clamp(min=0.0)
    reserve = torch.minimum(reserve, ships)
    return (source_budget - reserve).clamp(min=0.0)


def _global_select_2p(
    *, P, W, device, dtype, score, cand_src, cand_send, cand_angle, cand_eta,
    ...
    target_exists, roi_threshold, enemy_pressure, ships, owned,
    vuln_weight, margin, drop_sweeps, drop_deadband,
):
    """2P-only global-select (§2.2 stage 1): greedy initial solution -- byte-identical
    to ``_greedy_select`` -- followed by a local source-vulnerability drop sweep.
    ...
    planet short against reachable enemy garrison (``enemy_pressure``) and the marginal
    vulnerability cost ``vuln_weight * shortfall`` exceeds the wave's competitive score.
    It removes launches only (never adds/forces/rescore) and is savable-gated (a planet
    is only defended when ``ships >= enemy_pressure + margin`` at full garrison), so it
    cannot make play passive on quiet boards or hold a doomed planet. With no naked
    holdable source the sweep is a no-op and the output equals ``_greedy_select``.
    """
    L = int(cand_src.shape[1])
    ...
    # ---- Phase 2: source-vulnerability drop sweep ----
    need = enemy_pressure.to(dtype) + float(margin)
    ...
    def _penalty_sum(committed):
        # On holdable planets, baseline shortfall is 0, so this is the marginal
        # vulnerability our drain creates beyond the safe garrison (ships - need).
...
def plan_lite_waves(
    *,
    ...
    score = _apply_true_one_ply_hard_bad_2p(
        score=score,
        cand_src=cand_src,
        cand_send=cand_send,
        cand_eta=cand_eta,
    ...
    source_budget = _threat_reserve_source_budget(
    ...
    if int(player_count) < 4 and bool(config.enable_global_select_2p):
        # 2P-only: greedy + source-vulnerability drop sweep (sample142 / §2.2 stage 1).
        # 4P never reaches this branch, so 4P stays byte-identical to _greedy_select.
        gs_pressure = enemy_mass if enemy_mass is not None else cheap_enemy_pressure(
            obs, cache, horizon=float(K_eta), player_id=pid
        )
        wave_entries, leftover = _global_select_2p(
            P=P, W=W, device=device, dtype=dtype, score=score,
            cand_src=cand_src, cand_send=cand_send, cand_angle=cand_angle, cand_eta=cand_eta,
            cand_active=cand_active, cand_tgt_slot=cand_tgt_slot, cand_tgt_short=cand_tgt_short,
            cand_is_def=cand_is_def, source_budget=source_budget,
            target_exists=target_exists_for_normal, roi_threshold=float(config.roi_threshold),
            enemy_pressure=gs_pressure, ships=obs.ships.to(dtype), owned=(obs.owned & obs.alive),
            vuln_weight=float(config.global_select_vuln_weight),
            margin=float(config.global_select_margin),
            drop_sweeps=int(config.global_select_drop_sweeps),
            drop_deadband=float(config.global_select_drop_deadband),
        )
    else:
...
CONFIG_4P = dataclasses.replace(
    ...
    # sample130: threat-aware early-defense source reservation (all 4P modes).
    enable_threat_reserve_4p=True,
    threat_reserve_window=6,
    threat_reserve_margin=3.0,
    threat_reserve_prod_frac=0.5,
    threat_reserve_start=0,
    threat_reserve_limit=200,
)
...
# sample149: code-baked 2P combo. params.json may be skipped under raw Kaggle
# exec when __file__ is undefined, so experiment flags must live in the base.
_CONFIG_2P_BASE = dataclasses.replace(
    ProducerLiteConfig(),
    enable_global_select_2p=True,
    global_select_vuln_weight=0.6,
    global_select_margin=1.0,
    global_select_drop_sweeps=2,
    global_select_drop_deadband=0.25,
    enable_true_one_ply_2p=True,
    true_one_ply_2p_start=34,
    true_one_ply_2p_limit=400,
    true_one_ply_2p_hard_bad_net=-42.0,
    true_one_ply_2p_recapture_ratio=1.25,
    true_one_ply_2p_recapture_net=-12.0,
)
CONFIG_2P = _ow_apply_config_overrides(_CONFIG_2P_BASE, _OW_STRATEGY_PARAMS.get("config_2p"))
