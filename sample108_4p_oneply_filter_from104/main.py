...
class ProducerLiteConfig:
    ...
    mini_rollout_top_k: int = 36
    mini_rollout_window: int = 10
    mini_rollout_hold_bonus: float = 0.72
    mini_rollout_fail_penalty: float = 2.2
    mini_rollout_source_strip_penalty: float = 0.55
    mini_rollout_nonleader_enemy_penalty: float = 0.18
    # --- sample105: per-player best-response threat for mini-rollout --------
    one_ply_response_frac: float = 0.65
    one_ply_response_min_ships: float = 8.0
    one_ply_response_eta_cap: float = 12.0
...
def _apply_mini_rollout_adjustment(
    *,
    ...
    per_player_response = torch.zeros(idx.shape[0], dtype=dtype, device=device)
    ...
    if response_frac > 0.0:
        ...
        for enemy_id in range(int(player_count)):
            ...
            e_eta_masked = torch.where(can_respond, e_eta, torch.full_like(e_eta, float("inf")))
            best_src = torch.argmin(e_eta_masked, dim=1)
            best_eta = e_eta_masked.gather(1, best_src.unsqueeze(1)).squeeze(1)
            best_ships = e_ships[best_src] * response_frac
            valid = torch.isfinite(best_eta) & (best_ships >= response_min)
    ...
    local_enemy = per_player_response
    ...
    toxic_target = (
        (neutral_target | enemy_target)
        & hold_bad
        & (prod_t >= 2.0)
        & (future_hold < -6.0)
    )
    local = torch.where(toxic_target, torch.full_like(local, float("-inf")), local)
