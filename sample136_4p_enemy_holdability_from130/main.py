...
class ProducerLiteConfig:
    ...
    # sample136: enemy-capture holdability filter (vulture). Forbid ONLY enemy-target
    # captures we cannot hold -- after taking the planet, combat-aware net enemy
    # arrivals within the window must not exceed our surviving ships + reinforcement
    # + production + margin. Holdable captures (weakened/cheap enemies) stay allowed,
    # so we keep vulture wins while cutting the premature over-commits that cause the
    # t32-40 collapse. Hard -inf mask on bad enemy captures only.
    enable_enemy_holdability_4p: bool = False
    enemy_holdability_window: int = 6
    enemy_holdability_margin: float = 4.0
    enemy_holdability_start: int = 0
    enemy_holdability_limit: int = 460
...
def _apply_enemy_holdability_filter(
    *,
    score: Tensor,
    cand_send: Tensor,
    cand_eta: Tensor,
    ...
    """Forbid only the enemy-target captures we cannot hold (vulture filter).

    For each candidate capturing an enemy planet, estimate the ships that survive on
    the planet after we take it, then require that combat-aware net enemy arrivals in
    the post-capture window do not exceed that plus our reinforcement, production and
    a margin. Holdable captures (weakened/cheap enemy planets) are kept; un-holdable
    premature attacks are masked to ``-inf``. Neutral and own targets are untouched.
    """
    if int(player_count) < 4 or not bool(config.enable_enemy_holdability_4p):
        return score
    step = _obs_step(obs_tensors)
    if step < int(config.enemy_holdability_start) or step > int(config.enemy_holdability_limit):
        return score
    arrivals = getattr(garrison_status, "arrivals_by_owner", None)
    gar = getattr(garrison_status, "ships", None)
    if arrivals is None or gar is None:
        return score
    P = int(obs.P)
    if P <= 0:
        return score
    ...
    dtype = score.dtype
    pid = int(obs.player_id)
    H = int(arrivals.shape[-2]) - 1
    if H <= 0:
        return score
    W = max(1, min(int(config.enemy_holdability_window), H))

    tgt = cand_tgt_slot.clamp(0, max(P - 1, 0))
    active = cand_active.any(dim=-1) & torch.isfinite(score) & (~cand_is_def)
    enemy_tgt = active & obs.is_enemy[tgt]
    if not bool(enemy_tgt.any()):
        return score

    send = cand_send[:, 0].to(dtype)
    eta = cand_eta[:, 0].to(dtype).clamp(min=1.0)
    k_arr = torch.ceil(eta).to(torch.long).clamp(1, H)            # [C] our arrival step
    gar_at = gar.to(dtype)[tgt, k_arr]                            # [C] enemy garrison we fight
    surviving = send - gar_at                                    # [C] ships left after capture

    steps = torch.arange(1, H + 1, device=device).view(1, H)
    start = k_arr.view(-1, 1)
    end = (k_arr + W).clamp(max=H).view(-1, 1)
    win = (steps > start) & (steps <= end)                       # [C, H] strictly after our arrival
    arr_t = arrivals[tgt, 1:, :].to(dtype)                       # [C, H, A]
    A = int(arr_t.shape[-1])
    per_owner = (arr_t * win.unsqueeze(-1)).sum(dim=1)           # [C, A]
    owner_ids = torch.arange(A, device=device).view(1, A)
    enemy_po = torch.where(owner_ids != pid, per_owner, torch.zeros_like(per_owner))
    k_top = min(2, A)
    top_vals, _ = torch.topk(enemy_po, k=k_top, dim=1)
    top2 = top_vals[:, 1] if k_top > 1 else torch.zeros_like(top_vals[:, 0])
    net_enemy = (top_vals[:, 0] - top2).clamp(min=0.0)           # [C] combat-aware counterattack
    my_po = per_owner[:, pid] if pid < A else torch.zeros(int(per_owner.shape[0]), dtype=dtype, device=device)
    prod_t = prod.to(dtype)[tgt]
    margin = float(config.enemy_holdability_margin)

    hold_capacity = surviving + my_po + prod_t * float(W)
    unholdable = enemy_tgt & ((surviving <= 0.0) | (hold_capacity + margin < net_enemy))
    return torch.where(unholdable, torch.full_like(score, float("-inf")), score)
...
def plan_lite_waves(
    *,
    ...
    score = _apply_enemy_holdability_filter(
        score=score,
        cand_send=cand_send,
        cand_eta=cand_eta,
        cand_tgt_slot=cand_tgt_slot,
...
CONFIG_4P = dataclasses.replace(
    ...
    # sample136: forbid only un-holdable enemy captures (keep vulture captures).
    enable_enemy_holdability_4p=True,
    enemy_holdability_window=6,
    enemy_holdability_margin=4.0,
    enemy_holdability_start=0,
    enemy_holdability_limit=460,
)
