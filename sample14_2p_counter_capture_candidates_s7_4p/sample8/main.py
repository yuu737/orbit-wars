...
class ProducerLiteConfig:
    ...
    counter_capture_enabled: bool = False
    counter_capture_turns: int = 220
    counter_capture_extra_targets: int = 5
    counter_capture_eta_slack: float = 7.0
    counter_capture_min_value: float = 8.0
    counter_capture_min_enemy_ships: float = 6.0
...
def _merge_extra_targets(
    *, target_idx: Tensor, target_exists: Tensor, extra_idx: Tensor, P: int, device
) -> tuple[Tensor, Tensor]:
...
def _add_counter_capture_targets(
    *,
    obs,
    cache,
    prod: Tensor,
    ...
    if (
        not bool(config.counter_capture_enabled)
        or int(player_count) != 2
        or int(step) >= int(config.counter_capture_turns)
        or int(obs.P) <= 0
    ...
    enemy_sources = obs.is_enemy & obs.alive & (obs.ships >= float(config.counter_capture_min_enemy_ships))
    if not bool(neutral.any()) or not bool(enemy_sources.any()):
        return target_idx, target_exists

    d0 = cache.cross_dist[0].to(dtype)
    my_ships = torch.where(source_mask, obs.ships.to(dtype), torch.zeros(P, dtype=dtype, device=device))
    enemy_ships = torch.where(enemy_sources, obs.ships.to(dtype), torch.zeros(P, dtype=dtype, device=device))
    my_speed = fleet_speed(my_ships.clamp(min=1.0)).clamp(min=1e-6)
    enemy_speed = fleet_speed(enemy_ships.clamp(min=1.0)).clamp(min=1e-6)

    my_eta_matrix = d0 / my_speed.view(P, 1)
    enemy_eta_matrix = d0 / enemy_speed.view(P, 1)
    my_eta = torch.where(source_mask.view(P, 1), my_eta_matrix, torch.full_like(my_eta_matrix, float("inf"))).amin(dim=0)
    enemy_eta = torch.where(enemy_sources.view(P, 1), enemy_eta_matrix, torch.full_like(enemy_eta_matrix, float("inf"))).amin(dim=0)

    value = prod.to(dtype) * 5.0 + obs.ships.to(dtype) * 0.04
    contested = (
        neutral
        & torch.isfinite(enemy_eta)
        & torch.isfinite(my_eta)
        & (enemy_eta <= my_eta + float(config.counter_capture_eta_slack))
        & (value >= float(config.counter_capture_min_value))
    )
    if not bool(contested.any()):
        return target_idx, target_exists

    # Prefer valuable neutrals that the enemy can reach soon and we can still contest.
    pref = value + (my_eta - enemy_eta).clamp(min=-10.0, max=20.0) * 0.08 - my_eta * 0.01
    pref = torch.where(contested, pref, torch.full_like(pref, float("-inf")))
    cap = max(1, min(int(config.counter_capture_extra_targets), P))
    extra_idx, extra_exists = _candidate_indices(pref, contested, cap)
...
def plan_lite_waves(
    *,
    ...
    target_idx, target_exists = _add_counter_capture_targets(
        obs=obs,
        cache=cache,
        prod=prod,
        config=config,
