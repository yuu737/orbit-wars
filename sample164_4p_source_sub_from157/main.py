...
class ProducerLiteConfig:
    ...
    # --- sample164: 4P source-SUBSTITUTION (re-route instead of drop) ---
    # Before the guard's drop sweep, re-route a wave whose source it would have to bare:
    # keep the attack on the same target but launch from a DIFFERENT source that is NOT
    # left third-party-exposed (clears ROI, has budget, stays safe after funding, not a
    # reinforced/target planet). Keeps activity (no drop -> avoids the passivity that
    # capped sample157/158/160); only un-routable waves fall through to the drop sweep.
    enable_source_sub_4p: bool = False
    tp_sub_min_score_frac: float = 0.5   # don't re-route to an alt scoring below orig*this
    tp_sub_max_alt: int = 8              # alt sources examined per target
...
def _global_select_4p(
    *, P, W, device, dtype, score, cand_src, cand_send, cand_angle, cand_eta,
    ...
    enable_sub=False, sub_min_score_frac=0.5, sub_max_alt=8,
):
    ...
    # ---- Phase 1.5: SOURCE-SUBSTITUTION (sample164) ----
    # Re-route an exposed wave to a safer source for the SAME target (keep the attack).
    # Only un-routable waves fall through to the Phase-2 drop sweep -> activity preserved.
    need_sub = tp_threat.to(dtype) + float(margin)
    holdable_sub = owned & (ships >= need_sub)
    if bool(enable_sub) and bool(w_fired.any()) and bool(holdable_sub.any()):
        cand_s0 = cand_src[:, 0]
        cand_send0 = cand_send[:, 0]
        cand_active0 = cand_active[:, 0]
        frac = float(sub_min_score_frac)

        # planets that are a target of some fired wave (must not become a source)
        is_target = torch.zeros(P, dtype=torch.bool, device=device)
        ft = w_tgt[w_fired][:, 0].clamp(0, P - 1) if bool(w_fired.any()) else None
        if ft is not None and ft.numel() > 0:
            is_target[ft] = True

        def _committed_sub():
            send = torch.where(w_active & w_fired.view(W, 1), w_send, torch.zeros_like(w_send))
            c = torch.zeros(P, dtype=dtype, device=device)
            c.scatter_add_(0, w_src.reshape(-1), send.reshape(-1))
            return c

        order_sub = torch.argsort(torch.where(w_fired, w_score, torch.full_like(w_score, float("inf"))))
        for wi in order_sub.tolist():
            if not bool(w_fired[wi]) or not bool(w_active[wi, 0]):
                continue
            ga = ships - _committed_sub()
            S = int(w_src[wi, 0])
            # only re-route a wave that bares a *holdable* source against a third party
            if not (bool(holdable_sub[S]) and float(ga[S]) < float(need_sub[S])):
                continue
            tgt_planet = int(w_tgt[wi, 0])
            sib = (
                (cand_tgt_slot == tgt_planet) & cand_active0 & torch.isfinite(score)
                & (score > float(roi_threshold)) & (score >= float(w_score[wi]) * frac)
            )
            sib_idx = torch.nonzero(sib, as_tuple=False).flatten()
            if sib_idx.numel() == 0:
                continue
            order_c = sib_idx[torch.argsort(score[sib_idx], descending=True)]
            chosen = None
            for ci in order_c[: int(sub_max_alt)].tolist():
                Sp = int(cand_s0[ci])
                if Sp == S or bool(defended[Sp]) or bool(is_target[Sp]):
                    continue
                sp_send = float(cand_send0[ci])
                rem_sp = float((ships - _committed_sub())[Sp])          # ships free on S'
                if sp_send > rem_sp:
                    continue
                # S' must stay safe after funding (or be unholdable = nothing to lose)
                if bool(holdable_sub[Sp]) and (rem_sp - sp_send) < float(need_sub[Sp]):
                    continue
                chosen = ci
                break
            if chosen is not None:
                w_src[wi, 0] = cand_s0[chosen]
                w_send[wi, 0] = cand_send0[chosen]
                w_angle[wi, 0] = cand_angle[chosen, 0]
                w_eta[wi, 0] = cand_eta[chosen, 0]
                w_active[wi, 0] = True
                w_score[wi] = score[chosen]
    ...
    *,
    ...
    if (int(player_count) >= 4 and bool(config.enable_thirdparty_guard_4p)
        ...
        wave_entries, leftover = _global_select_4p(
            ...
            enable_sub=bool(config.enable_source_sub_4p),
            sub_min_score_frac=float(config.tp_sub_min_score_frac),
            sub_max_alt=int(config.tp_sub_max_alt),
        )
    else:
...
CONFIG_4P = dataclasses.replace(
    ...
    tp_guard_start=20,
    tp_guard_limit=250,
    # sample164: source-substitution (re-route exposed waves before dropping)
    enable_source_sub_4p=True,
    tp_sub_min_score_frac=0.5,
    tp_sub_max_alt=8,
)
