...
def _eager_load_agents() -> None:
    global _EAGER_LOAD_DONE
    if _EAGER_LOAD_DONE:
        return
    _agent_2p()
    _agent_4p()
    _EAGER_LOAD_DONE = True


try:
    _eager_load_agents()
except Exception:
    # Kaggle may execute from unusual working directories during validation.
    # Fall back to lazy loading so FileNotFoundError remains debuggable at act time.
    _EAGER_LOAD_DONE = False
