...
def competitive_score(
    ...
    *,
    player_id: int,
    leader_weights: Tensor | None = None,
) -> Tensor:
    ...
    if leader_weights is None:
        ...
        weights = leader_weights.to(device=net.device, dtype=net.dtype)
        weights = weights.clone()
        if int(player_id) < int(weights.shape[0]):
            weights[int(player_id)] = 0.0
        opp = (net * weights).sum(dim=-1)
    return me - opp
...
def score_candidates(
    ...
    *,
    ...
    return competitive_score(diff, player_id=int(player_id), leader_weights=leader_weights)
