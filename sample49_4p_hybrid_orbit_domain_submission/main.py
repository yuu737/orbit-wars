...
class ProducerLiteConfig:
    ...
    # --- sample49: PlanetWars/Halite-inspired 4P meta layer ---------------
    # Apply after exact candidate scoring and before greedy assignment.
    # The goal is not to replace the sample opening, but to bias the assignment
    # away from 4P traps: center fights, already-covered targets, and overkill.
    enable_hybrid_meta_4p: bool = True
    hybrid_center_turn_limit: int = 180
    hybrid_center_radius: float = 30.0
    hybrid_center_penalty: float = 0.34
    hybrid_outer_radius: float = 32.0
    hybrid_outer_bonus: float = 0.16
    hybrid_eta_penalty: float = 0.010
    hybrid_prod_bonus: float = 0.020
    hybrid_enemy_pressure_weight: float = 0.014
    hybrid_enemy_pressure_cap: float = 0.55
    hybrid_incoming_credit_ratio: float = 0.72
    hybrid_overkill_ratio: float = 1.34
    hybrid_overkill_penalty: float = 0.46
    hybrid_same_target_fleet_penalty: float = 0.30
...
def _own_incoming_ship_credit(obs) -> Tensor:
    """Approximate own fleets already committed to each planet.

    Orbit Wars observations expose fleet position, angle and ship count, but not
    the explicit target.  This uses a conservative cone test: if a friendly fleet
    is moving almost directly toward a planet, count most of its ships as already
    covering that target.  The value is only used as a soft penalty, not as a hard
    legality rule.
    """
    P = int(obs.P)
    F = int(obs.F)
    ...
    if P <= 0 or F <= 0:
        return torch.zeros(P, dtype=dtype, device=device)
    own_fleet = obs.f_alive & (obs.f_owner == float(obs.player_id)) & (obs.f_ships >= 1.0)
    if not bool(own_fleet.any()):
        return torch.zeros(P, dtype=dtype, device=device)

    fx = obs.f_x.view(F, 1).to(dtype)
    fy = obs.f_y.view(F, 1).to(dtype)
    tx = obs.x.view(1, P).to(dtype)
    ty = obs.y.view(1, P).to(dtype)
    dx = tx - fx
    dy = ty - fy
    dist = torch.sqrt(dx * dx + dy * dy).clamp(min=1e-6)
    desired = torch.atan2(dy, dx)
    diff = torch.abs((desired - obs.f_angle.view(F, 1).to(dtype) + math.pi) % (2.0 * math.pi) - math.pi)

    # Larger bodies are easier to hit, but keep the cone narrow to avoid
    # double-counting fleets that merely pass near a planet.
    body_slack = ((obs.r.view(1, P).to(dtype) + 4.0) / dist).clamp(max=0.24)
    cone = 0.18 + body_slack
    valid = own_fleet.view(F, 1) & obs.alive.view(1, P) & (diff <= cone)
    # Discount very distant fleets; this is just a target-coverage signal.
    distance_weight = (1.0 - dist / 120.0).clamp(min=0.35, max=1.0)
    credit = torch.where(
        valid,
        obs.f_ships.view(F, 1).to(dtype) * distance_weight,
        torch.zeros((F, P), dtype=dtype, device=device),
    )
    return credit.sum(dim=0)


def _floor_need_for_candidates(*, floor: Tensor, cand_tgt_short: Tensor, cand_eta: Tensor, active: Tensor) -> Tensor:
    """Gather capture floor at each candidate's arrival turn."""
    C = int(cand_tgt_short.shape[0])
    dtype = cand_eta.dtype
    device = cand_eta.device
    if C == 0 or int(floor.shape[-1]) <= 0:
        return torch.ones(C, dtype=dtype, device=device)
    T, K = int(floor.shape[0]), int(floor.shape[1])
    short = cand_tgt_short.clamp(0, max(T - 1, 0)).to(torch.long)
    eta0 = cand_eta[:, 0] if cand_eta.ndim == 2 else cand_eta
    k = (eta0.clamp(min=1.0, max=float(K)).ceil().long() - 1).clamp(0, K - 1)
    need = floor.to(dtype=dtype).gather(0, short.view(C, 1).expand(C, K)).gather(1, k.view(C, 1)).squeeze(1)
    return torch.where(active, need.clamp(min=1.0), torch.ones_like(need))


def _apply_hybrid_4p_meta_adjustment(
    *,
    score: Tensor,
    cand_src: Tensor,
    cand_send: Tensor,
    ...
    """4P meta scoring layer inspired by Planet Wars / Halite bots.

    Components:
    - global assignment remains `_greedy_select`; this layer improves its ranking.
    - avoid early center brawls in FFA unless the exact flow score is very good.
    - prefer outer high-production lanes.
    - discount targets already covered by friendly fleets.
    - penalize overkill and highly contestable neutral grabs.
    """
    if int(player_count) < 4 or not bool(config.enable_hybrid_meta_4p):
        return score
    P = int(obs.P)
    if P <= 0 or int(score.shape[0]) == 0:
        return score
    ...
    target_neutral = obs.is_neutral[tgt]
    target_enemy = obs.is_enemy[tgt]
    target_attack = active & (target_neutral | target_enemy)
    target_mine = obs.owned[tgt]

    # 4P FFA usually loses value in early center trades.  Keep it soft so exact
    # tactical captures still fire when their flow score is high.
    center_depth = ((float(config.hybrid_center_radius) - radius[tgt]) / max(float(config.hybrid_center_radius), 1e-6)).clamp(min=0.0, max=1.0)
    center_penalty = torch.where(
        target_attack & (step < int(config.hybrid_center_turn_limit)),
        center_depth * float(config.hybrid_center_penalty),
        torch.zeros_like(score),
    )

    # Outer production lane bias: stronger on neutral captures, weaker on enemy
    # attacks. This complements domain_factory without requiring its mode.
    outer_depth = ((radius[tgt] - float(config.hybrid_outer_radius)) / 38.0).clamp(min=0.0, max=1.0)
    outer_bonus = torch.where(
        active & target_neutral,
        outer_depth * float(config.hybrid_outer_bonus) + prod[tgt].to(dtype).clamp(max=8.0) * float(config.hybrid_prod_bonus),
        torch.zeros_like(score),
    )
    enemy_outer_bonus = torch.where(
        active & target_enemy & (radius[tgt] >= float(config.hybrid_outer_radius) + 4.0),
        outer_depth * float(config.hybrid_outer_bonus) * 0.45,
        torch.zeros_like(score),
    )

    # Arrival-time cost prevents long low-value grabs from displacing safer close
    # captures after the target-shortlist has been expanded by domain modes.
    eta_penalty = torch.where(target_attack, eta * float(config.hybrid_eta_penalty), torch.zeros_like(score))

    # Existing friendly fleets count as soft credit.  Penalize both "already
    # covered" targets and send+incoming overkill.  This is the Halite-style
    # already-heading-fleet correction adapted to Orbit Wars observations.
    need = _floor_need_for_candidates(floor=floor, cand_tgt_short=cand_tgt_short, cand_eta=cand_eta, active=active).to(dtype)
    incoming = _own_incoming_ship_credit(obs).to(dtype)[tgt]
    covered = target_attack & (incoming >= need * float(config.hybrid_incoming_credit_ratio))
    overkill = target_attack & ((incoming + send) > need * float(config.hybrid_overkill_ratio))
    incoming_penalty = torch.where(
        covered,
        ((incoming / need.clamp(min=1.0)) - float(config.hybrid_incoming_credit_ratio)).clamp(min=0.0, max=2.0)
        * float(config.hybrid_same_target_fleet_penalty),
        torch.zeros_like(score),
    )
    overkill_penalty = torch.where(
        overkill,
        (((incoming + send) / need.clamp(min=1.0)) - float(config.hybrid_overkill_ratio)).clamp(min=0.0, max=2.5)
        * float(config.hybrid_overkill_penalty),
        torch.zeros_like(score),
    )

    # Contestability: neutral planets under heavy enemy reach are often poison in
    # 4P unless production/outer-domain value compensates.
    if enemy_mass is None:
        pressure_t = torch.zeros_like(score)
    else:
        pressure_t = enemy_mass.to(dtype)[tgt]
    pressure_raw = (pressure_t - need * 0.35).clamp(min=0.0) * float(config.hybrid_enemy_pressure_weight)
    pressure_penalty = torch.where(
        target_neutral & active,
        pressure_raw.clamp(max=float(config.hybrid_enemy_pressure_cap)),
        torch.zeros_like(score),
    )
    # Do not suppress reinforcement; exact defensive scoring should dominate.
    defensive_relief = torch.where(target_mine & active, torch.full_like(score, 0.06), torch.zeros_like(score))

    adjusted = score + outer_bonus + enemy_outer_bonus + defensive_relief - center_penalty - eta_penalty - incoming_penalty - overkill_penalty - pressure_penalty
    return torch.where(active, adjusted, score)
...
def plan_lite_waves(
    *,
    ...
    score = _apply_hybrid_4p_meta_adjustment(
        score=score,
        cand_src=cand_src,
        cand_send=cand_send,
        cand_tgt_slot=cand_tgt_slot,
