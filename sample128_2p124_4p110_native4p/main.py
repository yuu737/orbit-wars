...
def _player_count_from_raw_obs(obs) -> int:
...
def _purge_orbit_lite_for_2p() -> None:
...
def _agent_2p():
    global _MOD_2P
    if _MOD_2P is not None:
        return _MOD_2P
    root = os.path.join(_HERE, "sample124")
    main_path = os.path.join(root, "main.py")
    if not os.path.exists(main_path):
        raise FileNotFoundError(main_path)
    _purge_orbit_lite_for_2p()
    if root in sys.path:
        sys.path.remove(root)
    sys.path.insert(0, root)
    spec = importlib.util.spec_from_file_location("_sample128_sample124", main_path)
    if spec is None or spec.loader is None:
        raise ImportError(main_path)
    mod = importlib.util.module_from_spec(spec)
    sys.modules["_sample128_sample124"] = mod
...
def agent(obs):
    ...
    if raw_step is not None and int(raw_step or 0) == 0:
        _USE_2P = _player_count_from_raw_obs(obs) < 4
    if _USE_2P is None:
        _USE_2P = _player_count_from_raw_obs(obs) < 4
    if _USE_2P:
