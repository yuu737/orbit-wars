...
class ProducerLiteConfig:
    ...
    # sample134: latent-threat term. The base reserve only sees enemy fleets already
    # in flight, so a planet emptied to expand is still flipped by a NEARBY enemy
    # garrison that launches next turn (observed cause of the t32-40 collapse). Add a
    # fraction of cheap_enemy_pressure (reachable enemy ship mass) to the per-planet
    # threat so reserves also account for latent, not-yet-launched pressure. 0 = off.
    threat_reserve_latent_frac: float = 0.0
    threat_reserve_latent_horizon: float = 14.0
...
def _threat_reserve_source_budget(
    *,
    ...
    # sample134: latent threat from nearby enemy garrisons that could launch this
    # turn (not yet in flight). cheap_enemy_pressure gives reachable enemy ship mass.
    latent_frac = float(config.threat_reserve_latent_frac)
    if latent_frac > 0.0 and cache is not None:
        lat_h = float(config.threat_reserve_latent_horizon) or float(W)
        latent = cheap_enemy_pressure(obs, cache, horizon=lat_h, player_id=pid).to(dtype)
        enemy_arr = enemy_arr + latent * latent_frac
...
CONFIG_4P = dataclasses.replace(
    ...
    # sample134: also reserve against latent (not-yet-launched) nearby enemy pressure.
    threat_reserve_latent_frac=0.25,
)
