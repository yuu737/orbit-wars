...
class ProducerLiteConfig:
    ...
    # --- sample167: 4P opening boost (expand when cornered, don't guard your only base) ---
    # Competition replays: 2/6 4P losses = sitting on 1 planet (opening passivity). The
    # guard makes it worse -- launching from your ONLY planet bares it, so the guard drops
    # the expansion and you never grow. When owned planets <= threshold, switch to a low-ROI
    # plain greedy (expand aggressively, guard OFF) until established, then resume the guard.
    enable_opening_boost_4p: bool = False
    opening_planet_threshold: int = 3
    opening_roi: float = 0.8
...
def plan_lite_waves(
    *,
    ...
    _n_owned = int((obs.owned & obs.alive).sum().item())
    if (int(player_count) >= 4 and bool(config.enable_opening_boost_4p)
            and _n_owned <= int(config.opening_planet_threshold)):
        # cornered (few planets): expand aggressively with NO guard (guard would drop the
        # only expansion launch -> the 1-planet death seen in competition replays).
        wave_entries, leftover = _greedy_select(
            P=P, W=W, device=device, dtype=dtype, score=score,
            cand_src=cand_src, cand_send=cand_send, cand_angle=cand_angle, cand_eta=cand_eta,
            cand_active=cand_active, cand_tgt_slot=cand_tgt_slot, cand_tgt_short=cand_tgt_short,
            cand_is_def=cand_is_def, source_budget=source_budget,
            target_exists=target_exists_for_normal, roi_threshold=float(config.opening_roi),
        )
    elif (int(player_count) >= 4 and bool(config.enable_thirdparty_guard_4p)
...
CONFIG_4P = dataclasses.replace(
    ...
    tp_guard_start=20,
    tp_guard_limit=250,
    # sample167: opening boost (expand when cornered; guard off until established)
    enable_opening_boost_4p=True,
    opening_planet_threshold=3,
    opening_roi=0.8,
)
