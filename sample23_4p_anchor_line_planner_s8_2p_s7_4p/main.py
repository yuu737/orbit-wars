...
class ProducerLiteConfig:
    ...
    # --- 4P anchor line planner -----------------------------------------
    enable_anchor_line_4p: bool = False
    anchor_plan_turns: int = 145
    anchor_route_targets: int = 6
    anchor_bonus: float = 0.95
    anchor_neighbor_bonus: float = 0.45
    anchor_hold_penalty: float = 0.80
    anchor_min_hold_base: float = 38.0
    anchor_min_hold_prod: float = 9.0
...
def _make_anchor_plan_4p(
    *,
    ...
    """Pick one 4P anchor line: anchor first, nearby support second.

    The goal is to create a thick production cluster instead of scattering into
    many thin planets.  The planner only biases target generation/scoring; the
    exact flow scorer still handles launch validity.
    """
    P = int(obs.P)
    ...
    inf_all = torch.full_like(eta_all, float("inf"))
    my_eta = torch.where(source_mask.view(P, 1), eta_all, inf_all).amin(dim=0)

    # Candidate anchors: productive or large neutral, reachable early enough.
    anchor_mask = neutral & (my_eta <= float(K_eta + 8)) & ((prod >= 3.0) | (obs.ships >= 40.0))
    if not bool(anchor_mask.any()):
        return None

    # Prefer anchors with a useful local neighborhood.  This is the core 4P idea:
    # an anchor is good if it creates a cluster, not because it is a single nice planet.
    local_d = d0
    local_mask = neutral.view(1, P) & (local_d <= 48.0)
    local_prod = torch.where(local_mask, prod.view(1, P).to(dtype), torch.zeros(P, P, dtype=dtype, device=device)).sum(dim=1)
    local_count = local_mask.sum(dim=1).to(dtype)
    local_big = torch.where(local_mask & (obs.ships.view(1, P) >= 35.0), torch.ones(P, P, dtype=dtype, device=device), torch.zeros(P, P, dtype=dtype, device=device)).sum(dim=1)

    # Avoid anchors that are likely everybody's immediate knife-fight if possible.
    enemy = obs.alive & obs.is_enemy
    if bool(enemy.any()):
        enemy_eta = torch.where(enemy.view(P, 1), eta_all, inf_all).amin(dim=0)
    else:
        enemy_eta = torch.full((P,), float("inf"), dtype=dtype, device=device)
    contest_penalty = (my_eta - enemy_eta + 1.5).clamp(min=0.0, max=6.0)
    ...
    anchor_idx = int(torch.argmax(anchor_score).item())
    if not torch.isfinite(anchor_score[anchor_idx]):
        return None

    # Route: anchor plus nearby support planets. Support can be neutral now or
    # newly ours later; we keep the ids stable in memory.
    around = neutral & (d0[anchor_idx] <= 52.0)
    around[anchor_idx] = True
    support_score = torch.where(
        around,
        prod.to(dtype) * 6.0 + obs.ships.to(dtype) * 0.03 - d0[anchor_idx] * 0.06,
        torch.full((P,), float("-inf"), dtype=dtype, device=device),
    )
    cap = max(1, min(int(config.anchor_route_targets), P))
    route_idx, route_exists = _candidate_indices(support_score, torch.isfinite(support_score), cap)
    route = [int(x) for x in route_idx[route_exists].detach().cpu().tolist()]
    if anchor_idx not in route:
        route.insert(0, anchor_idx)

    # Expansion targets: after the anchor thickens, the line should point to
    # larger nearby outside planets.
    route_mask = torch.zeros(P, dtype=torch.bool, device=device)
    route_mask[torch.tensor(route, dtype=torch.long, device=device).clamp(0, P - 1)] = True
    outer = neutral & ~route_mask & (d0[anchor_idx] <= 78.0) & ((prod >= 3.0) | (obs.ships >= 55.0))
    outer_score = torch.where(
        outer,
        prod.to(dtype) * 7.0 + obs.ships.to(dtype) * 0.04 - d0[anchor_idx] * 0.045,
        torch.full((P,), float("-inf"), dtype=dtype, device=device),
    )
    outer_list: list[int] = []
    if bool(torch.isfinite(outer_score).any()):
        outer_idx, outer_exists = _candidate_indices(outer_score, torch.isfinite(outer_score), min(3, P))
        outer_list = [int(x) for x in outer_idx[outer_exists].detach().cpu().tolist()]
...
def _anchor_plan_targets(
    *,
    ...
    K_eta: int,
    player_count: int,
) -> tuple[Tensor | None, Tensor | None, int | None, float]:
    if int(player_count) < 4 or not bool(config.enable_anchor_line_4p):
        return None, None, None, 0.0
    step = _obs_step(obs_tensors)
    if step > int(config.anchor_plan_turns):
        return None, None, None, 0.0
    if getattr(memory, "anchor_plan", None) is None or step <= 1:
        memory.anchor_plan = _make_anchor_plan_4p(
            obs=obs,
    ...
    plan = getattr(memory, "anchor_plan", None)
    if not plan:
        return None, None, None, 0.0
    P = int(obs.P)
    ...
    route = [int(t) for t in plan.get("route", []) if 0 <= int(t) < P and bool(obs.alive[int(t)])]
    outer = [int(t) for t in plan.get("outer", []) if 0 <= int(t) < P and bool(obs.alive[int(t)])]
    if not route:
        route = [anchor]

    anchor_owned = bool(obs.owned[anchor])
    anchor_ships = float(obs.ships[anchor].item())
    anchor_prod = float(prod[anchor].item())
    hold = float(config.anchor_min_hold_base) + float(config.anchor_min_hold_prod) * anchor_prod
    mature = 1.0 if anchor_owned and anchor_ships >= hold else 0.0
    targets = route + (outer if mature > 0.0 else [])
    targets = list(dict.fromkeys(targets))
    target_tensor = torch.tensor(targets, dtype=torch.long, device=obs.device) if targets else None
    route_tensor = torch.tensor(route, dtype=torch.long, device=obs.device) if route else None
    return target_tensor, route_tensor, anchor, mature


def _append_anchor_targets(
    target_idx: Tensor,
    target_exists: Tensor,
    anchor_targets: Tensor | None,
) -> tuple[Tensor, Tensor]:
    if anchor_targets is None or anchor_targets.numel() == 0:
        return target_idx, target_exists
    existing = set(int(x) for x in target_idx[target_exists].detach().cpu().tolist())
    extras = [int(x) for x in anchor_targets.detach().cpu().tolist() if int(x) not in existing]
    if not extras:
        return target_idx, target_exists
    extra_idx = torch.tensor(extras, dtype=torch.long, device=target_idx.device)
...
def _apply_anchor_line_adjustment(
    *,
    score: Tensor,
    cand_valid: Tensor,
    cand_src: Tensor,
    ...
    player_count: int,
    anchor_targets: Tensor | None,
    anchor_route: Tensor | None,
    anchor_slot: int | None,
    anchor_mature: float,
) -> Tensor:
    if int(player_count) < 4 or anchor_targets is None or anchor_targets.numel() == 0:
        return score
    P = int(obs.P)
    target_mask = torch.zeros(P, dtype=torch.bool, device=score.device)
    target_mask[anchor_targets.clamp(0, P - 1)] = True
    route_mask = torch.zeros(P, dtype=torch.bool, device=score.device)
    if anchor_route is not None and anchor_route.numel() > 0:
        route_mask[anchor_route.clamp(0, P - 1)] = True

    tgt = cand_tgt_slot.clamp(0, P - 1)
    src = cand_src.squeeze(-1).clamp(0, P - 1)
    selected_target = target_mask[tgt] & cand_valid
    route_target = route_mask[tgt] & cand_valid

    bonus = torch.zeros_like(score)
    bonus = bonus + torch.where(selected_target, torch.full_like(score, float(config.anchor_neighbor_bonus)), torch.zeros_like(score))
    bonus = bonus + torch.where(route_target, torch.full_like(score, float(config.anchor_bonus)), torch.zeros_like(score))

    # Do not immediately drain the chosen anchor before it is thick enough.  This
    # is a soft penalty, not a veto, because sometimes the exact scorer still knows
    # a killing move.
    if anchor_slot is not None and 0 <= int(anchor_slot) < P:
        anchor = int(anchor_slot)
        anchor_prod = prod[anchor].to(score.dtype)
        hold = float(config.anchor_min_hold_base) + float(config.anchor_min_hold_prod) * anchor_prod
        source_is_anchor = src == anchor
        source_after = obs.ships[src].to(score.dtype) - cand_send.squeeze(-1).to(score.dtype)
        too_thin = source_is_anchor & (source_after < hold) & (float(anchor_mature) < 1.0)
        penalty = float(config.anchor_hold_penalty) + ((hold - source_after).clamp(min=0.0) * 0.01)
        bonus = bonus - torch.where(too_thin & cand_valid, penalty.clamp(max=1.4), torch.zeros_like(score))
...
def plan_lite_waves(
    *,
    ...
    anchor_targets, anchor_route, anchor_slot, anchor_mature = _anchor_plan_targets(
        memory=memory,
        obs=obs,
        obs_tensors=obs_tensors,
        cache=cache,
    ...
    target_idx, target_exists = _append_anchor_targets(target_idx, target_exists, anchor_targets)
    ...
    score = _apply_anchor_line_adjustment(
        score=score,
...
class ProducerLiteMemory:
    ...
    def __init__(self) -> None:
        ...
        self.anchor_plan: Optional[dict[str, Any]] = None
